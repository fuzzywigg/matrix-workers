/**
 * TOKENMAXX HEAVY leftovers after #268 / tip after #274 — oidc-auth *quaternary*
 * concurrent-race niches: soft exact error / HTML binds under Promise.all that
 * prior oidc concurrent megafloods never asserted (grep Failed to initiate /
 * Failed to reset / Missing code or state in oidc-auth-concurrent-race = 0).
 *
 * Soft/route leftovers bind these sequentially (oidc-auth-api-routes);
 * concurrent races never. Quaternary deepen:
 *   discovery-fail → exact `Failed to initiate login` ∥ sibling success redirect;
 *   identity-reset DO throw → exact `Failed to reset identity` ∥ sibling ok {};
 *   callback missing code/state → `Missing code or state parameter` ∥ valid redeem;
 *   dual discovery-fail flood; dual reset-fail flood; missing-param flood.
 *
 * Distinct from tip #274 (filters+appservice+auth tertiary), #273 room-cache
 * quinary, open #275 admin+federation, and #268 oauth+oidc deleteBarrier tip.
 * New file (not append to 4.6k megaflood). Orthogonal to oauth quaternary
 * (this slice is external IdP SSO, not /oauth/* AS provider).
 *
...
// ---------------------------------------------------------------------------
// Failed to initiate login ∥ sibling success
// ---------------------------------------------------------------------------

describe('quaternary oidc Failed to initiate login under race after #274 tip', () => {
  it('discovery fail ∥ sibling success — binds Failed to initiate login; sibling 302', async () => {
    ...
    fetchOIDCDiscovery.mockImplementation(async (issuer: string) => {
      if (issuer === ISSUER) throw new Error('discovery down');
      return DISCOVERY_B;
    });
    buildAuthorizationUrl.mockImplementation(
      (d: { authorization_endpoint: string }, _c: string, _r: string, _s: string, state: string, nonce: string) =>
        `${d.authorization_endpoint}?state=${state}&nonce=${nonce}`
    );
    const env = envFor({ sessions, db });
    const results = await Promise.all([
      request(`/auth/oidc/${PROVIDER_ID}/login`, {}, env),
      request(`/auth/oidc/${PROVIDER_B}/login?return_to=/ok`, {}, env),
    ]);
    expect(statusesOf(results)).toEqual([302, 500]);
    const fail = results.find((r) => r.status === 500)!;
    expect(fail.body).toEqual({ errcode: 'M_UNKNOWN', error: 'Failed to initiate login' });
    expect(Object.keys(fail.body).sort()).toEqual(['errcode', 'error']);
  ...
  it('dual discovery fail — both bind exact Failed to initiate login; zero state puts', async () => {
    const db = createOidcRaceDb({ providers: [seedProvider()] });
    fetchOIDCDiscovery.mockRejectedValue(new Error('down'));
    const sessions = mockKv();
    const env = envFor({ sessions, db });
    const results = await Promise.all([
      request(`/auth/oidc/${PROVIDER_ID}/login`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/login?return_to=/a`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/login?return_to=/b`, {}, env),
    ]);
    expect(statusesOf(results)).toEqual([500, 500, 500]);
    expect(
      results.every(
        (r) =>
          r.body.errcode === 'M_UNKNOWN' && r.body.error === 'Failed to initiate login'
      )
    ).toBe(true);
    expect(sessions.puts).toHaveLength(0);
  });

  it('buildAuthorizationUrl throw ∥ discovery fail — both Failed to initiate login', async () => {
    const db = createOidcRaceDb({
      providers: [
        seedProvider(),
        seedProvider({ id: PROVIDER_B, name: 'GitHub', issuer_url: ISSUER_B, display_order: 2 }),
      ],
    });
    fetchOIDCDiscovery.mockImplementation(async (issuer: string) => {
      if (issuer === ISSUER_B) throw new Error('gh down');
      return DISCOVERY;
    });
    buildAuthorizationUrl.mockImplementation(() => {
    ...
    expect(
      results.every((r) => r.body.error === 'Failed to initiate login')
    ).toBe(true);
  });

  for (let i = 0; i < 12; i++) {
    it(`Failed to initiate login ∥ success flood-${i}`, async () => {
      const secret = await encryptClientSecret();
      const sessions = mockKv();
      const db = createOidcRaceDb({
        providers: [
      ...
      fetchOIDCDiscovery.mockImplementation(async (issuer: string) => {
        if (issuer === ISSUER) throw new Error(`down-${i}`);
        return DISCOVERY_B;
      });
      buildAuthorizationUrl.mockImplementation(
        (d: { authorization_endpoint: string }, _c: string, _r: string, _s: string, state: string) =>
          `${d.authorization_endpoint}?state=${state}&i=${i}`
      );
      const env = envFor({ sessions, db });
      const results = await Promise.all([
        request(`/auth/oidc/${PROVIDER_ID}/login?return_to=/f${i}`, {}, env),
        request(`/auth/oidc/${PROVIDER_B}/login?return_to=/ok${i}`, {}, env),
      ]);
      expect(statusesOf(results)).toEqual([302, 500]);
      expect(results.find((r) => r.status === 500)!.body).toEqual({
        errcode: 'M_UNKNOWN',
  ...
  for (let i = 0; i < 12; i++) {
    it(`dual Failed to initiate login flood-${i}`, async () => {
      const db = createOidcRaceDb({ providers: [seedProvider()] });
      fetchOIDCDiscovery.mockRejectedValue(new Error(`boom-${i}`));
      const sessions = mockKv();
      const env = envFor({ sessions, db });
      const results = await Promise.all([
        request(`/auth/oidc/${PROVIDER_ID}/login`, {}, env),
        request(`/auth/oidc/${PROVIDER_ID}/login?return_to=/x${i}`, {}, env),
      ]);
      expect(statusesOf(results)).toEqual([500, 500]);
      expect(
        results.every((r) => r.body.error === 'Failed to initiate login')
...
// ---------------------------------------------------------------------------
// Failed to reset identity ∥ sibling ok
// ---------------------------------------------------------------------------

describe('quaternary oidc Failed to reset identity under race after #274 tip', () => {
  it('DO throw ∥ sibling ok — binds Failed to reset identity; sibling {}', async () => {
    const failKeys = createUserKeysStub({ throwOnFetch: true });
    const okKeys = createUserKeysStub();
    // Single env can't have two stubs easily — race fail then sequential ok, OR
    // use failCrossSigningDelete for one path. Prefer: dual with throwOnFetch
    // both fail, plus separate ok∥fail via two envs in Promise.all.
    const dbFail = createOidcRaceDb();
    const dbOk = createOidcRaceDb();
    const envFail = envFor({ db: dbFail, userKeys: failKeys });
    const envOk = envFor({ db: dbOk, userKeys: okKeys });
    const results = await Promise.all([
      request(RESET_PATH, { method: 'POST' }, envFail),
      request(RESET_PATH, { method: 'POST' }, envOk),
    ]);
    ...
    expect(Object.keys(fail.body).sort()).toEqual(['errcode', 'error']);
    expect(results.find((r) => r.status === 200)!.body).toEqual({});
    expect(dbFail.deletes).toHaveLength(0);
    expect(dbOk.deletes.some((d) => d.sql.includes('cross_signing_keys'))).toBe(true);
  });

  it('D1 cross_signing delete fail ∥ ok sibling — exact Failed to reset identity', async () => {
    const results = await Promise.all([
      request(
        RESET_PATH,
        { method: 'POST' },
        envFor({
          db: createOidcRaceDb({ failCrossSigningDelete: true }),
      ...
      request(
        RESET_PATH,
        { method: 'POST' },
        envFor({ db: createOidcRaceDb(), userKeys: createUserKeysStub() })
      ),
    ]);
    expect(statusesOf(results)).toEqual([200, 500]);
    expect(results.find((r) => r.status === 500)!.body.error).toBe(
      'Failed to reset identity'
    );
    expect(results.find((r) => r.status === 200)!.body).toEqual({});
  });

  it('stream bump fail ∥ ok — exact Failed to reset identity under race', async () => {
    const results = await Promise.all([
      request(
        RESET_PATH,
        { method: 'POST' },
        envFor({
          db: createOidcRaceDb({ failStreamUpdate: true }),
          userKeys: createUserKeysStub(),
        })
      ),
      request(
  ...
  for (let i = 0; i < 12; i++) {
    it(`Failed to reset identity ∥ ok flood-${i}`, async () => {
      const results = await Promise.all([
        request(
          RESET_PATH,
          { method: 'POST' },
  ...
  for (let i = 0; i < 12; i++) {
    it(`dual Failed to reset identity flood-${i}`, async () => {
      const results = await Promise.all([
        request(
          RESET_PATH,
          { method: 'POST' },
      ...
      expect(statusesOf(results)).toEqual([500, 500]);
      expect(
        results.every((r) => r.body.error === 'Failed to reset identity')
      ).toBe(true);
    });
  }
});
...
// ---------------------------------------------------------------------------
// Missing code or state parameter ∥ valid redeem
// ---------------------------------------------------------------------------

describe('quaternary oidc Missing code or state under race after #274 tip', () => {
  it('missing code ∥ missing state ∥ valid redeem — HTML bind + success', async () => {
    const secret = await encryptClientSecret();
    const sessions = mockKv();
    const state = seedState(sessions, 'cb-ok-q');
    ...
    const env = envFor({ sessions, db });
    const results = await Promise.all([
      request(`/auth/oidc/${PROVIDER_ID}/callback?state=nostate`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/callback?code=onlycode`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/callback?code=ok&state=${state}`, {}, env),
    ]);
    const miss = results.filter((r) => r.text.includes('Missing code or state parameter'));
    expect(miss.length).toBe(2);
    expect(miss.every((r) => r.text.includes('Invalid Request'))).toBe(true);
    expect(results.some((r) => r.text.includes('Login Successful'))).toBe(true);
    expect(sessions.data[`oidc_state:${state}`]).toBeUndefined();
  });

  it('empty code&state ∥ valid — still Missing code or state parameter', async () => {
    const secret = await encryptClientSecret();
    const sessions = mockKv();
    const state = seedState(sessions, 'cb-empty-q');
    getUserById.mockResolvedValue({ user_id: USER });
    const db = createOidcRaceDb({
      providers: [seedProvider({ client_secret_encrypted: secret })],
      links: [
    ...
    const env = envFor({ sessions, db });
    const results = await Promise.all([
      request(`/auth/oidc/${PROVIDER_ID}/callback?code=&state=`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/callback`, {}, env),
      request(`/auth/oidc/${PROVIDER_ID}/callback?code=ok&state=${state}`, {}, env),
    ]);
    expect(
      results.filter((r) => r.text.includes('Missing code or state parameter')).length
    ).toBeGreaterThanOrEqual(2);
    expect(results.some((r) => r.text.includes('Login Successful'))).toBe(true);
  });

  for (let i = 0; i < 12; i++) {
    it(`Missing code or state ∥ valid flood-${i}`, async () => {
      const secret = await encryptClientSecret();
      const sessions = mockKv();
      const state = seedState(sessions, `cb-f-${i}`);
      getUserById.mockResolvedValue({ user_id: USER });
      validateIDToken.mockResolvedValue({
        sub: `ext-sub-${i}`,
        preferred_username: 'alice',
      ...
      const missPath =
        i % 2 === 0
          ? `/auth/oidc/${PROVIDER_ID}/callback?state=only`
          : `/auth/oidc/${PROVIDER_ID}/callback?code=only`;
      const results = await Promise.all([
        request(missPath, {}, env),
        request(`/auth/oidc/${PROVIDER_ID}/callback?code=c${i}&state=${state}`, {}, env),
      ]);
      expect(
        results.some((r) => r.text.includes('Missing code or state parameter'))
      ).toBe(true);
      expect(results.some((r) => r.text.includes('Login Successful'))).toBe(true);
    });
  }
  ...
  for (let i = 0; i < 12; i++) {
    it(`dual Missing code or state flood-${i}`, async () => {
      const env = envFor({ db: createOidcRaceDb({ providers: [seedProvider()] }) });
      const results = await Promise.all([
        request(`/auth/oidc/${PROVIDER_ID}/callback?code=a`, {}, env),
        request(`/auth/oidc/${PROVIDER_ID}/callback?state=b`, {}, env),
        request(`/auth/oidc/${PROVIDER_ID}/callback`, {}, env),
      ]);
      expect(
        results.every((r) => r.text.includes('Missing code or state parameter'))
      ).toBe(true);
    });
  }
});

// Keep encrypt/decrypt import live for harness parity with sibling residual files
void decryptSecret;
