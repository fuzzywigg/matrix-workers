 ...
 * filters + #353 room-cache (fresh after closed #354) — oauth
 * *tridecenary* concurrent-race niches duodenary (#340) never mixed
 * under Promise.all:
 *   Username required ∥ Request Cancelled ∥ Authorization request
 *     expired ∥ login Missing username (denary Username∥expired login
 *     Missing without Cancelled+AUTH JSON; undecenary Cancelled with
 *     period∥credentials; duodenary Cancelled∥OIDC∥period∥no-period),
 *   default desc ∥ parse fail ∥ Session Expired ∥ login no-period
 *     (denary default∥cross-signing∥Expired∥Cancelled; undecenary
 *     parse∥OIDC∥mismatch∥sensitive; never default∥parse∥Expired∥
 *     no-period),
 *   password mismatch ∥ OIDC Approved ∥ Missing Session ∥
 *     credentials-desc (duodenary mismatch∥Expired∥Missing∥password
 *     Approved; undecenary OIDC∥mismatch∥sensitive∥parse — never
 *     mismatch∥OIDC∥Missing∥credentials without parse).
 *
 * Gap table (why leftover after #340 / tip past #349/#350/#353):
 *   Username ∥ Cancelled ∥ AUTH expired JSON ∥ login Missing
 *     | denary/undecenary/duodenary Cancelled quads without AUTH JSON
 *   default ∥ parse ∥ Expired ∥ no-period
 *     | denary default quad; undecenary parse mega; never this quartet
 *   mismatch ∥ OIDC approve ∥ Missing ∥ credentials
 *     | duodenary mismatch+Missing with password approve only
 *
