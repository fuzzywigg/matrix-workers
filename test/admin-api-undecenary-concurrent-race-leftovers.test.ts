 ...
 * septendecenary — undecenary *admin* soft+concurrent-race niches unsaturated by
 * octonary IdP Bad-JSON (#342), nonary IdP create/missing/test/delete (#364), and
 * in-flight denary #386 forbid/param softs (self-demote/purge/registration/
 * deactivated/whois). Complementary to denary — does not re-flood those strings.
 *
 * Distinct from senary (#328) Report/Admin-access/login-token/IdP-PUT/keys:
 * Distinct from octonary (#342) IdP PUT updated/No-changes/test-ok/Bad-JSON (IdP-only):
 * Distinct from nonary (#364) IdP POST missing/create-fail/test-fail/deleted/link:
 * Distinct from denary (#386) self-demote/purge/registration-enabled/deactivated/whois:
 *   federation/test No signing keys found exact soft+race;
 *   registration Failed to update config exact soft+race;
 *   Identity provider created successfully soft flood (never its own soft loop in numbered);
 *   non-IdP Bad JSON Could not parse on make-admin∥registration∥users/create soft+race.
 *
...
const IDP_CREATE_BODY = {
  name: 'Undecenary IdP',
  issuer_url: 'https://idp.example.com',
  client_id: 'cid-undecenary',
  client_secret: 'secret-undecenary',
  scopes: 'openid profile',
};

function mockFederationTestFetch(opts: { emptyVerifyKeys?: boolean } = {}) {
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input: RequestInfo | URL) => {
    ...
    if (url.includes('/_matrix/key/v2/server')) {
      return Response.json(
        opts.emptyVerifyKeys === false
          ? { verify_keys: { 'ed25519:a': { key: 'abc' } } }
          : { verify_keys: {} }
      );
    }
...
describe('soft undecenary admin federation-test/config/IdP-ok/Bad-JSON exact after #364/#386', () => {
  for (let i = 0; i < 12; i++) {
    it(`federation/test No signing keys found exact flood-${i}`, async () => {
      mockFederationTestFetch({ emptyVerifyKeys: true });
      const res = await jsonReq('/admin/api/federation/test', {}, createEnv());
  ...
  for (let i = 0; i < 12; i++) {
    it(`registration Failed to update config exact flood-${i}`, async () => {
  ...
  for (let i = 0; i < 12; i++) {
    it(`Identity provider created successfully exact flood-${i}`, async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.message).toBe('Identity provider created successfully');
      expect(db.idpProviders).toHaveLength(1);
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`non-IdP Bad JSON Could not parse exact flood-${i}`, async () => {
      const env = createEnv();
      const paths = [
      ...
      const res = await jsonReq(
        path,
        {
          method: path === '/admin/api/registration' ? 'PUT' : 'POST',
      ...
      expect(res.body.error).toBe('Could not parse request body as JSON');
    });
  }
});

describe('race undecenary admin federation-test/config/IdP-ok/Bad-JSON exact after #364/#386', () => {
  it('No signing keys found∥Failed to update config isolation', async () => {
    mockFederationTestFetch({ emptyVerifyKeys: true });
    const failDO = createAdminDO({ failConfigPut: true });
    const [keys, cfg] = await Promise.all([
      jsonReq('/admin/api/federation/test', {}, createEnv()),
      jsonReq(
        '/admin/api/registration',
        jsonInit('PUT', { enabled: false }),
        createEnv({ adminDO: failDO })
      ),
    ]);
    const tests = keys.body.tests as Array<{ name: string; message: string }>;
    expect(tests.find((t) => t.name === 'Server signing keys')?.message).toBe('No signing keys found');
    expect(cfg.body.raw).toBe('Failed to update config');
  });

  it('IdP created ok∥Failed to create isolation', async () => {
    ...
    const [ok, fail] = await Promise.all([
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'RaceOk', client_id: 'cid-race-ok' }),
        createEnv({ db: okDb })
      ),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'RaceFail', client_id: 'cid-race-fail' }),
        createEnv({ db: failDb })
      ),
    ]);
  ...
  it('make-admin Bad JSON∥registration Bad JSON isolation', async () => {
    const env = createEnv();
    const badInit = {
    ...
    const [makeAdmin, reg] = await Promise.all([
      jsonReq('/admin/api/make-admin', badInit, env),
      jsonReq(
        '/admin/api/registration',
        { ...badInit, method: 'PUT' },
        env
    ...
    expect(makeAdmin.body.error).toBe('Could not parse request body as JSON');
    expect(reg.body.error).toBe('Could not parse request body as JSON');
  });

  it('No signing keys∥Failed to update∥IdP created∥make-admin Bad JSON four-way', async () => {
    mockFederationTestFetch({ emptyVerifyKeys: true });
    const okDb = createAdminDb({ idpProviders: [] });
    const [keys, cfg, idp, bad] = await Promise.all([
      jsonReq('/admin/api/federation/test', {}, createEnv()),
      jsonReq(
        '/admin/api/registration',
        jsonInit('PUT', { enabled: true }),
        createEnv({ adminDO: createAdminDO({ failConfigPut: true }) })
      ),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'FourOk', client_id: 'cid-four' }),
        createEnv({ db: okDb })
      ),
      jsonReq(
        '/admin/api/users/create',
    ...
    expect(tests.find((t) => t.name === 'Server signing keys')?.message).toBe('No signing keys found');
    expect(cfg.body.raw).toBe('Failed to update config');
    expect(idp.body.message).toBe('Identity provider created successfully');
    expect(bad.body.error).toBe('Could not parse request body as JSON');
  });

  for (let i = 0; i < 12; i++) {
    it(`undecenary admin exact race flood-${i}`, async () => {
      mockFederationTestFetch({ emptyVerifyKeys: true });
      const okDb = createAdminDb({ idpProviders: [] });
      ...
      const tests = results[0].body.tests as Array<{ name: string; message: string }>;
      expect(tests.find((t) => t.name === 'Server signing keys')?.message).toBe(
        'No signing keys found'
      );
      expect(results[1].body.raw).toBe('Failed to update config');
      expect(results[2].body.message).toBe('Identity provider created successfully');
      expect(results[3].body.error).toBe('Could not parse request body as JSON');
      expect(results[4].body.error).toBe('Could not parse request body as JSON');
    });
  }
});
