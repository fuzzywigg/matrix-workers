/**
 * TOKENMAXX HEAVY leftovers after #310 duodenary — tridecenary room-cache
 * *concurrent race / TOCTOU* niches not landed by duodenary:
 *   - joinedRaw -Infinity isDm∥null→0 isDm∥invitedRaw NaN→0
 *   - name [] non-DM∥name {} non-DM∥avatar url:false under miss
 *   - topic true stored∥alias 0 stored∥hot sibling
 *   - gen "9foo"→9∥"+0"→0∥ZWSP→0 get∥bump
 *   - meta putThrows A swallow∥bump A∥hot B isolation
 *   - get put-hold A∥deleteHold invalidate mid → LWW empty-or-filled
 *   - getBatch putThrows A swallow∥hot B∥miss C still maps
 *
 * Hibernation tridecenary leftovers live in call-room-hibernation.test.ts
 ...
 * and the hibernation tridecenary describes.
 */
...
// ---------------------------------------------------------------------------
// joinedRaw -Infinity / null / invitedRaw NaN
// ---------------------------------------------------------------------------

describe('race tridecenary joinedRaw -Inf/null + invited NaN after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`-Inf isDm∥null→0 isDm∥invited NaN→0 under parallel miss flood-${i}`, async () => {
      ...
      const db = mockDb({
        [ROOM_A]: { joinedRaw: Number.NEGATIVE_INFINITY, invitedRaw: 0 },
        [ROOM_B]: { joinedRaw: null, invitedRaw: 2 },
        [ROOM_C]: { joinedRaw: 1, invitedRaw: Number.NaN },
      });
      ...
      // -Infinity truthy stays; -Infinity <= 2 → true → isDm
      expect(a?.joinedCount).toBe(Number.NEGATIVE_INFINITY);
      expect(a?.isDm).toBe(true);
      // null falsy → || 0; 0 <= 2 && !name → isDm
      expect(b?.joinedCount).toBe(0);
      expect(b?.invitedCount).toBe(2);
      expect(b?.isDm).toBe(true);
      // NaN falsy → || 0 for invited
...
// ---------------------------------------------------------------------------
// name [] / {} / avatar url:false
// ---------------------------------------------------------------------------

describe('race tridecenary name []/{} + avatar url false after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`name [] non-DM∥name {} non-DM∥url false under miss flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        getBarrier: [
          [metaKey(ROOM_A), 1],
          [metaKey(ROOM_B), 1],
      ...
      // ![] → false → non-DM; !{} → false → non-DM; url false stored
...
// ---------------------------------------------------------------------------
// topic true / alias 0 / hot sibling
// ---------------------------------------------------------------------------

describe('race tridecenary topic true / alias 0 after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`topic true stored∥alias 0 stored∥hot sibling flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
      ...
      const db = mockDb({
        [ROOM_A]: { rawTopicContent: JSON.stringify({ topic: true }), joined: 1 },
...
// ---------------------------------------------------------------------------
// Generation "9foo" / "+0" / ZWSP
// ---------------------------------------------------------------------------

describe('race tridecenary gen 9foo/+0/ZWSP parse after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`9foo→9∥+0→0∥ZWSP→0 get∥bump flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
          [genKey(ROOM_A)]: '9foo',
          [genKey(ROOM_B)]: '+0',
      ...
      // parseInt('9foo')=9; parseInt('+0')=0; parseInt(ZWSP+'5')=NaN→0
      expect(gA).toBe(9);
      expect(bA).toBe(10);
      expect(gB).toBe(0);
      expect(bB).toBe(1);
...
// ---------------------------------------------------------------------------
// meta putThrows A swallow ∥ bump A ∥ hot B
// ---------------------------------------------------------------------------

describe('race tridecenary meta putThrows∥bump∥hot after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`A putThrow swallows∥still returns FreshA∥bump→1∥B hot flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
      ...
      // put throw swallowed; caller still gets D1 metadata
...
// ---------------------------------------------------------------------------
// get put-hold A ∥ deleteHold invalidate mid → LWW
// ---------------------------------------------------------------------------

describe('race tridecenary put-hold∥deleteHold invalidate mid after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`delete-hold mid put-hold LWW empty-or-filled∥peer bump flood-${i}`, async () => {
      ...
      // Force miss via getThrows so D1 refill starts put-hold path
...
describe('race tridecenary getBatch putThrows A∥hot B∥miss C after #310', () => {
  for (let i = 0; i < 8; i++) {
    it(`getBatch A putThrow swallow∥B hit∥C refill all in map flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
          [metaKey(ROOM_B)]: JSON.stringify(freshMeta({ name: 'HotB' })),
        },
      ...
      // Batch path: put .catch() swallows; metadata still lands in Map
