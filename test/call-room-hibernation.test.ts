...
/**
 * TOKENMAXX HEAVY leftovers after #325 — CallRoom hibernation *quindecenary*
 * (tracks mid:0∥peer mute, errorCode:0 falsy success∥join, sessionDescription {}
 * empty sdp∥leave, Uint16Array INVALID_MESSAGE∥join, JSON null INTERNAL_ERROR∥join,
 * answer∥peer mute, offer∥webSocketClose LWW). Quattuordecenary stopped at answer∥leave.
 */

describe('CallRoom hibernation quindecenary sfu/error leftovers after #325', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s15' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '15' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks mid:0 stored∥peer mute isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=mid0' },
        tracks: [{ mid: 0 }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: number };
          } catch {
            return {};
          }
        })
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: 0 });
      const stored = state.storage.map.get('participant:u1|d1') as {
        tracks: Record<string, { mid?: number }>;
      };
      expect(stored.tracks.audio0.mid).toBe(0);
      const muted = state.storage.map.get('participant:u2|d2') as {
        tracks: Record<string, { enabled: boolean }>;
      };
      expect(muted.tracks.audio0.enabled).toBe(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:0 falsy success mid undef∥peer join flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=ec0' },
        tracks: [{ errorCode: 0 }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // errorCode 0 is falsy → success path; mid undefined
      const offerResp = wsA.sent
        .map((s) => {
          try {
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription {} empty sdp∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: {},
        tracks: [{ mid: '15e' }],
      });
      ...
      // {} is truthy → not NO_ANSWER; sdp undefined on offer_response
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; sdp?: string; mid?: string };
          } catch {
            return {};
          }
        })
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: '15e' });
...
describe('CallRoom hibernation quindecenary offer/json leftovers after #325', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s15b' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '15b' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`Uint16Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON null INTERNAL_ERROR∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      ...
      // null parses ok; msg.type throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`answer∥peer mute isolation both settle flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      await state.storage.put(
        'participant:u2|d2',
      ...
      await Promise.all([
        room.webSocketMessage(wsA, JSON.stringify({ type: 'answer', sdp: 'v=ans15' })),
        room.webSocketMessage(
          wsB,
          JSON.stringify({ type: 'mute', trackName: 'audio0', muted: true })
        ),
  ...
  for (let i = 0; i < 8; i++) {
    it(`offer∥webSocketClose LWW absent-or-track flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const ws = new FakeWebSocket();
      ws.serializeAttachment({ participantKey: 'u1|d1' });
      ...
      await Promise.all([
        ...
        room.webSocketClose(ws, 1000, 'bye'),
      ]);

      // close-then-offer may re-persist; offer-then-close clears — accept either
      const stored = state.storage.map.get('participant:u1|d1') as
        | { tracks: Record<string, unknown> }
        | undefined;
      if (stored) {
...
/**
 * TOKENMAXX HEAVY leftovers after #332 — CallRoom hibernation *sexdecenary*
 * (tracks mid:false stored∥peer mute, errorCode:"" falsy success∥join,
 * sessionDescription [] truthy empty sdp∥leave, tracks null INTERNAL_ERROR∥join,
 * Uint32Array INVALID_MESSAGE∥join, JSON {} UNKNOWN_MESSAGE∥join,
 * answer∥webSocketClose LWW, mute∥leave LWW). Quindecenary stopped at
 * offer∥webSocketClose and JSON null.
 */

describe('CallRoom hibernation sexdecenary sfu/error leftovers after #332', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s16' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '16' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks mid:false stored∥peer mute isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=midfalse' },
        tracks: [{ mid: false }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: boolean };
          } catch {
            return {};
          }
        })
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: false });
      const stored = state.storage.map.get('participant:u1|d1') as {
        tracks: Record<string, { mid?: boolean }>;
      };
      expect(stored.tracks.audio0.mid).toBe(false);
      const muted = state.storage.map.get('participant:u2|d2') as {
        tracks: Record<string, { enabled: boolean }>;
      };
      expect(muted.tracks.audio0.enabled).toBe(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:"" falsy success mid undef∥peer join flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=ecempty' },
        tracks: [{ errorCode: '' }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // empty-string errorCode is falsy → success path; mid undefined
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: string };
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription [] truthy empty sdp∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: [],
        tracks: [{ mid: '16e' }],
      });
      ...
      // [] is truthy → not NO_ANSWER; .sdp on array is undefined
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; sdp?: string; mid?: string };
        ...
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: '16e' });
      expect(offerResp?.sdp).toBeUndefined();
      expect(room.participants.has('u2|d2')).toBe(false);
      expect(state.storage.map.has('participant:u2|d2')).toBe(false);
    });
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks null INTERNAL_ERROR∥peer join isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=null-tracks' },
        tracks: null,
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // tracks null → tracks[0] throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
...
describe('CallRoom hibernation sexdecenary offer/json leftovers after #332', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s16b' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '16b' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`Uint32Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON {} UNKNOWN_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      ...
      // {} parses; msg.type undefined → UNKNOWN_MESSAGE default
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'UNKNOWN_MESSAGE',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`answer∥webSocketClose LWW absent-or-settled flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const ws = new FakeWebSocket();
      ws.serializeAttachment({ participantKey: 'u1|d1' });
      ...
      await Promise.all([
        room.webSocketMessage(ws, JSON.stringify({ type: 'answer', sdp: 'v=ans16' })),
        room.webSocketClose(ws, 1000, 'bye'),
      ]);

      // close clears participant; answer may settle before leave — accept either
      if (state.storage.map.has('participant:u1|d1')) {
        expect(renegotiateMock).toHaveBeenCalled();
      } else {
        expect(room.participants.has('u1|d1')).toBe(false);
      }
  ...
  for (let i = 0; i < 8; i++) {
    it(`mute∥leave same participant LWW absent-or-muted flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put(
        'participant:u1|d1',
        storedParticipant('u1', 'd1', { audio0: TRACK })
...
/**
 * TOKENMAXX HEAVY leftovers after #353 / tip c8c1af7 (#364/#366) — CallRoom
 * hibernation *septendecenary* relaunch of closed #370 (tracks mid:true
 * stored∥peer mute, errorCode:false falsy success∥join, sessionDescription 0
 * → NO_ANSWER∥leave, tracks true INTERNAL_ERROR∥join, BigUint64Array
 * INVALID_MESSAGE∥join, JSON [] UNKNOWN_MESSAGE∥join, unmute∥leave LWW,
 * answer∥peer join). Sexdecenary stopped at mute∥leave and JSON {}.
 */

describe('CallRoom hibernation septendecenary sfu/error leftovers after #353', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks mid:true stored∥peer mute isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=midtrue' },
        tracks: [{ mid: true }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      const offerResp = wsA.sent
        ...
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: true });
      const stored = state.storage.map.get('participant:u1|d1') as {
        tracks: Record<string, { mid?: boolean }>;
      };
      expect(stored.tracks.audio0.mid).toBe(true);
      const muted = state.storage.map.get('participant:u2|d2') as {
        tracks: Record<string, { enabled: boolean }>;
      };
      expect(muted.tracks.audio0.enabled).toBe(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:false falsy success mid undef∥peer join flood-${i}`, async () => {
      ...
      // false errorCode is falsy → success path; mid undefined
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: string };
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription 0 → NO_ANSWER∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
      ...
      // 0 is falsy → NO_ANSWER before tracks[0]
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'NO_ANSWER',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks true INTERNAL_ERROR∥peer join isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=tracks-true' },
        tracks: true,
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // tracks true → tracks[0] undefined → .errorCode throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
...
describe('CallRoom hibernation septendecenary offer/json leftovers after #353', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
  ...
  for (let i = 0; i < 8; i++) {
    it(`BigUint64Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      const room = makeRacingRoom(state) as any;
      const buf = new BigUint64Array([1n, 2n]);
  ...
  for (let i = 0; i < 8; i++) {
    it(`unmute∥leave same participant LWW absent-or-enabled flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put(
        'participant:u1|d1',
        storedParticipant('u1', 'd1', {
  ...
  for (let i = 0; i < 8; i++) {
    it(`answer∥peer join isolation flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const wsA = new FakeWebSocket();
      wsA.serializeAttachment({ participantKey: 'u1|d1' });
...
/**
 * TOKENMAXX HEAVY leftovers after #380 — CallRoom hibernation *octodenary*
 * relaunch of closed #379 (tracks mid:null stored∥peer mute, errorCode:null
 * falsy success∥join, sessionDescription false → NO_ANSWER∥leave, tracks false
 * INTERNAL_ERROR∥join, Float64Array INVALID_MESSAGE∥join, JSON 1.5
 * UNKNOWN_MESSAGE∥join, mute∥webSocketClose LWW, offer∥peer join).
 * Septendecenary stopped at mid:true / errorCode:false / BigUint64Array / [].
 */

describe('CallRoom hibernation octodenary sfu/error leftovers after #380', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s18' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '18' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks mid:null stored∥peer mute isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=midnull' },
        tracks: [{ mid: null }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: null };
          } catch {
            return {};
          }
        })
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: null });
      const stored = state.storage.map.get('participant:u1|d1') as {
        tracks: Record<string, { mid?: null }>;
      };
      expect(stored.tracks.audio0.mid).toBeNull();
      const muted = state.storage.map.get('participant:u2|d2') as {
        tracks: Record<string, { enabled: boolean }>;
      };
      expect(muted.tracks.audio0.enabled).toBe(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:null falsy success mid undef∥peer join flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=ecnull' },
        tracks: [{ errorCode: null }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const wsA = new FakeWebSocket();
      ...
      // null errorCode is falsy → success path; mid undefined
      const offerResp = wsA.sent
        .map((s) => {
          try {
            return JSON.parse(s) as { type?: string; mid?: string };
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription false → NO_ANSWER∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
      ...
      // false is falsy → NO_ANSWER before tracks[0]
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'NO_ANSWER',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks false INTERNAL_ERROR∥peer join isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=tracks-false' },
        tracks: false,
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // tracks false → tracks[0] throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
...
describe('CallRoom hibernation octodenary offer/json leftovers after #380', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s18b' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '18b' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON 1.5 UNKNOWN_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      ...
      // 1.5 parses as number; msg.type undefined → UNKNOWN_MESSAGE default
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'UNKNOWN_MESSAGE',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`mute∥webSocketClose LWW absent-or-muted flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put(
        'participant:u1|d1',
        storedParticipant('u1', 'd1', { audio0: TRACK })
  ...
  for (let i = 0; i < 8; i++) {
    it(`offer∥peer join both settle flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const wsA = new FakeWebSocket();
      wsA.serializeAttachment({ participantKey: 'u1|d1' });
      ...
      const offerResp = wsA.sent
        ...
        .find((m) => m.type === 'offer_response');
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: '18b' });
      expect(JSON.parse(wsB.sent[0])).toMatchObject({ type: 'welcome' });
      expect(room.participants.has('u2|d2')).toBe(true);
      expect(room.participants.has('u1|d1')).toBe(true);
    });
...
/**
 * TOKENMAXX HEAVY leftovers after #387 — CallRoom hibernation *novemdenary*
 * (tracks mid:"" stored∥peer mute, errorCode:"0" truthy error∥join,
 * sessionDescription "" → NO_ANSWER∥leave, tracks 0 INTERNAL_ERROR∥join,
 * Int16Array INVALID_MESSAGE∥join, BigInt64Array INVALID_MESSAGE∥join,
 * JSON "true" string UNKNOWN_MESSAGE∥join, unmute∥webSocketClose LWW).
 * Octodenary stopped at mid:null / errorCode:null / Float64Array / 1.5 /
 * mute∥close / offer∥peer join.
 */

describe('CallRoom hibernation novemdenary sfu/error leftovers after #387', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s19' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '19' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:"0" truthy error∥peer join flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=ec0' },
        tracks: [{ errorCode: '0', errorDescription: 'zero-code' }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const wsA = new FakeWebSocket();
      ...
      // non-empty string errorCode is truthy → error path
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: '0',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription "" → NO_ANSWER∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: '',
        tracks: [{ mid: '19z' }],
      });
      ...
      // "" is falsy → NO_ANSWER before tracks[0]
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'NO_ANSWER',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks 0 INTERNAL_ERROR∥peer join isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=tracks-0' },
        tracks: 0,
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // tracks 0 → tracks[0] undefined → track.errorCode throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
...
describe('CallRoom hibernation novemdenary offer/json leftovers after #387', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s19b' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '19b' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`Int16Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`BigInt64Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON "true" string UNKNOWN_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      ...
      // '"true"' parses as string; msg.type undefined → UNKNOWN_MESSAGE default
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'UNKNOWN_MESSAGE',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`unmute∥webSocketClose LWW absent-or-enabled flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put(
        'participant:u1|d1',
        storedParticipant('u1', 'd1', {
...
/**
 * TOKENMAXX HEAVY leftovers after #400 — CallRoom hibernation *vicenary*
 * (tracks mid:[] stored∥peer mute, errorCode:true truthy error∥join,
 * sessionDescription NaN → NO_ANSWER∥leave, tracks "" INTERNAL_ERROR∥join,
 * Int32Array INVALID_MESSAGE∥join, Uint8ClampedArray INVALID_MESSAGE∥join,
 * JSON "false" string UNKNOWN_MESSAGE∥join, leave∥webSocketError LWW).
 * Novemdenary stopped at mid:"" / errorCode:"0" / Int16Array / BigInt64Array /
 * unmute∥close / JSON "true".
 */

describe('CallRoom hibernation vicenary sfu/error leftovers after #400', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s20' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '20' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks mid:[] stored∥peer mute isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=midarr' },
        tracks: [{ mid: [] }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      expect(offerResp).toMatchObject({ type: 'offer_response', mid: [] });
      const stored = state.storage.map.get('participant:u1|d1') as {
        tracks: Record<string, { mid?: unknown }>;
      };
      expect(stored.tracks.audio0.mid).toEqual([]);
      const muted = state.storage.map.get('participant:u2|d2') as {
        tracks: Record<string, { enabled: boolean }>;
      };
      expect(muted.tracks.audio0.enabled).toBe(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`errorCode:true truthy error∥peer join flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=ectrue' },
        tracks: [{ errorCode: true, errorDescription: 'bool-true' }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const wsA = new FakeWebSocket();
      ...
      // boolean true errorCode is truthy → error path (code coerced in send)
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: true,
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`sessionDescription NaN → NO_ANSWER∥peer leave deletes flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: Number.NaN,
        tracks: [{ mid: '20z' }],
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // NaN is falsy → NO_ANSWER before tracks[0]
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'NO_ANSWER',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`tracks "" INTERNAL_ERROR∥peer join isolation flood-${i}`, async () => {
      addTracksMock.mockResolvedValueOnce({
        sessionDescription: { type: 'answer', sdp: 'v=tracks-empty-str' },
        tracks: '',
      });
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      ...
      // tracks "" → tracks[0] undefined → track.errorCode throws → INTERNAL_ERROR
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'INTERNAL_ERROR',
      });
...
describe('CallRoom hibernation vicenary offer/json leftovers after #400', () => {
  beforeEach(() => {
    addTracksMock.mockReset();
    closeTracksMock.mockReset();
    createSessionMock.mockReset();
    renegotiateMock.mockReset();
    createSessionMock.mockResolvedValue({ sessionId: 'sess-s20b' });
    renegotiateMock.mockResolvedValue(undefined);
    addTracksMock.mockResolvedValue({
      sessionDescription: { type: 'answer', sdp: 'v=a' },
      tracks: [{ mid: '20b' }],
  ...
  for (let i = 0; i < 8; i++) {
    it(`Int32Array INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`Uint8ClampedArray INVALID_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON "false" string UNKNOWN_MESSAGE∥valid join isolation flood-${i}`, async () => {
      const state = new RacingState();
      const wsA = new FakeWebSocket();
      const wsB = new FakeWebSocket();
      state.sockets = [wsA, wsB];
      ...
      // '"false"' parses as string; msg.type undefined → UNKNOWN_MESSAGE default
      expect(JSON.parse(wsA.sent[0])).toMatchObject({
        type: 'error',
        code: 'UNKNOWN_MESSAGE',
      });
  ...
  for (let i = 0; i < 8; i++) {
    it(`leave∥webSocketError same participant LWW absent flood-${i}`, async () => {
      const state = new RacingState();
      await state.storage.put('participant:u1|d1', storedParticipant('u1', 'd1'));
      const ws = new FakeWebSocket();
      ws.serializeAttachment({ participantKey: 'u1|d1' });
      ...
      await Promise.all([
        room.webSocketMessage(ws, JSON.stringify({ type: 'leave' })),
        room.webSocketError(ws, new Error('s20-err')),
      ]);
