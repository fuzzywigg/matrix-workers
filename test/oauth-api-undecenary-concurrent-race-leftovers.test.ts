/**
 * TOKENMAXX HEAVY leftovers after #315 denary / tip past #306 — oauth
 * *undecenary* concurrent-race niches denary never mixed under Promise.all:
 *   sensitive-operation warning ∥ UIA Invalid (WITH period) ∥ login
 *     Invalid (NO period) ∥ Request Approved (octonary sensitive∥Missing/
 *     Expired only; nonary period∥no-period without sensitive/Approved),
 *   Authorization request expired JSON ∥ UIA Invalid period ∥ Request
 *     Cancelled ∥ credentials-desc (denary expired∥Username required∥
 *     Missing; never period+cancel+credentials with expired JSON),
 *   OIDC-only Request Approved ∥ password mismatch ∥ sensitive warning ∥
 *     parse mega (nonary OIDC approve∥parse∥Username∥cancel; denary
 *     mismatch without OIDC approve+sensitive).
 *
 * Gap table (why leftover after #315):
 *   sensitive ∥ UIA period ∥ login no-period ∥ Approved
 *     | octonary sensitive∥soft bodies; nonary period pair only
 *   expired JSON ∥ UIA period ∥ Cancelled ∥ credentials-desc
 *     | denary expired∥Username required∥Missing
 *   OIDC approve ∥ password mismatch ∥ sensitive ∥ parse
 *     | nonary OIDC mega without mismatch+sensitive
 *
 * Distinct from #315 oauth denary, #306 nonary, #303 octonary.
...
// ---------------------------------------------------------------------------
// sensitive ∥ UIA period ∥ login no-period ∥ Request Approved
// ---------------------------------------------------------------------------

describe('undecenary oauth sensitive∥period∥no-period∥Approved after #315', () => {
  it('sensitive warning ∥ UIA Invalid period ∥ login no-period ∥ Request Approved under race', async () => {
    const cache = mockKv();
  ...
  it('sensitive ∥ period ∥ no-period ∥ Username required ∥ Approved penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-bad');
    ...
    const results = await Promise.all([
      ...
      request(
        '/oauth/authorize/uia',
        formInit({ session: 'uia-bad', username: 'nobody', password: 'x' }),
        env
      ),
      request(
        '/oauth/authorize',
  ...
  for (let i = 0; i < 8; i++) {
    it(`sensitive ∥ period ∥ no-period ∥ Approved flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
...
describe('undecenary oauth expired∥period∥Cancelled∥credentials after #315', () => {
  it('Authorization request expired ∥ UIA period ∥ Request Cancelled ∥ credentials-desc under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-bad');
  ...
  it('expired ∥ period ∥ Cancelled ∥ credentials ∥ Missing Session penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-bad');
    seedUiaSession(cache, 'uia-cancel');
  ...
  for (let i = 0; i < 8; i++) {
    it(`expired ∥ period ∥ Cancelled ∥ credentials flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      seedUiaSession(cache, `uia-bad-${i}`);
...
// ---------------------------------------------------------------------------
// OIDC-only approve ∥ password mismatch ∥ sensitive ∥ parse mega
// ---------------------------------------------------------------------------

describe('undecenary oauth OIDC-approve∥mismatch∥sensitive∥parse after #315', () => {
  it('OIDC-only Request Approved ∥ password mismatch ∥ sensitive ∥ parse under race', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
      .mockRejectedValueOnce(new Error('undecenary-parse'));
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    ...
    // parse reject steals one POST under Promise.all — Approved marks
    // either OIDC or alice-parse completed_stages (success does not delete).
    const stages = ['m.oauth', 'm.login.oauth', 'org.matrix.cross_signing_reset'];
    const oidcStages = JSON.parse(cache.data['uia_session:uia-oidc']).completed_stages;
    const parseStages = JSON.parse(cache.data['uia_session:uia-parse']).completed_stages;
    expect(
      stages.every((s) => oidcStages.includes(s)) || stages.every((s) => parseStages.includes(s))
    ).toBe(true);
    expect(JSON.parse(cache.data['uia_session:uia-mm']).completed_stages).toEqual([]);
  });

  it('OIDC approve ∥ mismatch ∥ sensitive ∥ parse ∥ Expired body penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
      .mockRejectedValueOnce(new Error('undecenary-parse-penta'));
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
    seedUiaSession(cache, 'uia-mm');
  ...
  for (let i = 0; i < 8; i++) {
    it(`OIDC approve ∥ mismatch ∥ sensitive ∥ parse flood-${i}`, async () => {
      const spy = vi
        .spyOn(HonoRequest.prototype, 'parseBody')
        .mockRejectedValueOnce(new Error(`undecenary-parse-${i}`));
      const cache = mockKv();
      seedClient(cache, 'cid-1');
  ...
  it('sensitive∥period∥expired∥OIDC-approve∥mismatch mega race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-bad');
    ...
    expect(results[2].body).toMatchObject({ error_description: AUTH_REQUEST_EXPIRED });
