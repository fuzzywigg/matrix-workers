 ...
 * niches unsaturated by senary keys-claim/openid/3pid-early exact (#328).
 ...
 *
 * Distinct from #324–#330 oauth/media/filters/crypto siblings. Tests-only. example.com only.
...
// ---------------------------------------------------------------------------
// Septenary soft floods + concurrent races after tip #328 / post-#317–#330 wave
// ---------------------------------------------------------------------------

describe('soft septenary federation invite/PDU/media/auth exact after #328', () => {
  for (let i = 0; i < 12; i++) {
...
describe('race septenary federation invite/PDU/media/auth exact after #328', () => {
  beforeEach(() => {
    federationOrigin = FED_ORIGIN;
    verifyRemoteSignature.mockResolvedValue(true);
    verifyContentHash.mockResolvedValue(false);
  ...
  for (let i = 0; i < 8; i++) {
    it(`septenary federation exact race flood-${i}`, async () => {
      const inviteEnv = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const sendEnv = makeEnv(
        createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
      );
