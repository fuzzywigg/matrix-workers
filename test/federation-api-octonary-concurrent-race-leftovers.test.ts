 ...
 * octonary *federation* soft+concurrent-race niches unsaturated by senary early-3pid
 * (#328) and septenary invite/PDU/media/auth (#335). Skip FederationCatchupWorkflow
 * and knock (own leftovers file).
 *
 * Distinct from senary (#328) device_keys/one_time_keys/3pid-early/openid:
 * Distinct from septenary (#335) invite v1/v2 / PDU hash-structure-sender / media/auth:
 *   exchange_third_party_invite late exact Room not found /
 *     No third party invite found with matching token /
 *     Invalid third party invite content /
 *     Could not verify third party invite signature under soft+race
 *     (complete signed block past senary early gates; never soft-flooded in numbered).
 *
...
function dbWithBadJsonInvite() {
  const eid = '$3pid-bad:example.com';
...
function dbWithUnverifiableInvite() {
  const eid = '$3pid-bad-sig:example.com';
  const roomState = new Map<string, string>([
    [stateKey(ROOM, 'm.room.third_party_invite', TOKEN), eid],
  ]);
  return createFedDb({
    events: [
      {
        ...
        event_type: 'm.room.third_party_invite',
        state_key: TOKEN,
        content: JSON.stringify({ public_key: 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' }),
        origin_server_ts: 1000,
        depth: 1,
        auth_events: '[]',
        prev_events: '[]',
...
describe('soft octonary federation late-3pid exact after #328/#335', () => {
  for (let i = 0; i < 12; i++) {
    it(`3pid late Room not found exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ rooms: [] }));
      const res = await req(
        'PUT',
  ...
  for (let i = 0; i < 12; i++) {
    it(`3pid late No third party invite exact flood-${i}`, async () => {
  ...
  for (let i = 0; i < 12; i++) {
    it(`3pid late Invalid third party invite content exact flood-${i}`, async () => {
      const env = makeEnv(dbWithBadJsonInvite());
      const res = await req(
        'PUT',
        `/_matrix/federation/v1/exchange_third_party_invite/${encodeURIComponent(ROOM)}`,
        env,
      ...
      expect((res.body as { error: string }).error).toBe('Invalid third party invite content');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`3pid late Could not verify signature exact flood-${i}`, async () => {
      const env = makeEnv(dbWithUnverifiableInvite());
      const res = await req(
        'PUT',
        `/_matrix/federation/v1/exchange_third_party_invite/${encodeURIComponent(ROOM)}`,
        env,
      ...
      expect((res.body as { errcode: string }).errcode).toBe('M_FORBIDDEN');
      expect((res.body as { error: string }).error).toBe(
        'Could not verify third party invite signature'
      );
    });
  }
});

describe('race octonary federation late-3pid exact after #328/#335', () => {
  it('Room not found∥No third party invite isolation', async () => {
    const [missRoom, missTok] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v1/exchange_third_party_invite/${encodeURIComponent(ROOM)}`,
    ...
    expect((missRoom.body as { error: string }).error).toBe('Room not found');
    expect((missTok.body as { error: string }).error).toBe(
      'No third party invite found with matching token'
    );
  });

  it('Invalid content∥Could not verify isolation', async () => {
    const [badJson, badSig] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v1/exchange_third_party_invite/${encodeURIComponent(ROOM)}`,
        makeEnv(dbWithBadJsonInvite()),
        completeSigned3pid()
      ),
      req(
        'PUT',
        `/_matrix/federation/v1/exchange_third_party_invite/${encodeURIComponent(ROOM)}`,
        makeEnv(dbWithUnverifiableInvite()),
        completeSigned3pid()
      ),
    ]);
    expect((badJson.body as { error: string }).error).toBe('Invalid third party invite content');
    expect((badSig.body as { error: string }).error).toBe(
      'Could not verify third party invite signature'
    );
  });

  it('late four-way Room∥No-tok∥Invalid∥Unverifiable', async () => {
    ...
    expect((a.body as { error: string }).error).toBe('Room not found');
    expect((b.body as { error: string }).error).toBe(
      'No third party invite found with matching token'
    );
    expect((c.body as { error: string }).error).toBe('Invalid third party invite content');
    expect((d.body as { error: string }).error).toBe(
      'Could not verify third party invite signature'
    );
  });

  it('No third party∥early Event must be membership still isolated', async () => {
    const env = makeEnv(createFedDb());
    ...
    expect((late.body as { error: string }).error).toBe(
      'No third party invite found with matching token'
    );
  ...
  for (let i = 0; i < 12; i++) {
    it(`octonary federation late-3pid race flood-${i}`, async () => {
      ...
      expect(statusesOf(results)).toEqual([400, 403, 403, 404]);
      expect((results[0].body as { error: string }).error).toBe('Room not found');
