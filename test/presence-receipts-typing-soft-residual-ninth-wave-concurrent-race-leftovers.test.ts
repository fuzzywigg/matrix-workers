 ...
 *   never Gone/AppearOffline/ExtendedAway/Occupied/Quiet invalid pins,
 *   never ZWSP / EM-space / trailing-LF whitespace (`\u200bonline`,
 *     `\u2003online`, `offline\n`),
 *   never typing root-string `"NULL"` / root-object `{}` → Missing typing ∥ boolean ok,
 ...
 *   oNlInE/OfFlInE/uNaVaIlAbLe + Gone/AppearOffline/Quiet + ZWSP/EM/LF ∥ ok
 *   typing root-string/object Missing typing + ban-membership Not a member ∥ ok
...
// ---------------------------------------------------------------------------
// oNlInE / OfFlInE / uNaVaIlAbLe + Gone/AppearOffline/Quiet + ZWSP/EM/LF whitespace ∥ ok
// ---------------------------------------------------------------------------
...
describe('presence ninth-wave concurrent case/whitespace soft after #371', () => {
  const cases: Array<{ label: string; body: unknown; error: string }> = [
    ...
    {
      label: 'AppearOffline-title',
      body: { presence: 'AppearOffline' },
      error: 'Invalid presence state: AppearOffline. Must be one of: online, offline, unavailable',
    },
    {
    ...
    {
      label: 'Occupied-title',
      body: { presence: 'Occupied' },
      error: 'Invalid presence state: Occupied. Must be one of: online, offline, unavailable',
    },
    {
      label: 'Quiet-title',
      body: { presence: 'Quiet' },
      error: 'Invalid presence state: Quiet. Must be one of: online, offline, unavailable',
    },
    {
    ...
    {
      label: 'em-space-whitespace',
      body: { presence: '\u2003online' },
      error: 'Invalid presence state: \u2003online. Must be one of: online, offline, unavailable',
    },
    {
      label: 'trailing-LF-whitespace',
      body: { presence: 'offline\n' },
  ...
  it('oNlInE ∥ OfFlInE ∥ Gone ∥ Quiet ∥ offline under race', async () => {
    const env = createEnv();
    const results = await Promise.all([
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
      ...
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
        jsonInit('PUT', { presence: 'Gone' }),
        env
      ),
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
        jsonInit('PUT', { presence: 'Quiet' }),
        env
      ),
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
    ...
    expect(
      ...
      [
        ...
        'Invalid presence state: Gone. Must be one of: online, offline, unavailable',
        'Invalid presence state: Quiet. Must be one of: online, offline, unavailable',
      ].sort()
    );
  });
});
...
describe('typing ninth-wave concurrent root-string-object + ban-membership soft after #371', () => {
  ...
  it('root-object {} Missing typing ∥ boolean false under race', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
      typingReq(
  ...
  for (let i = 0; i < 6; i++) {
    it(`typing root-string-object / ban-mem flood-${i} ∥ ok`, async () => {
      const roomDO = createRoomDOStub();
...
describe('presence/receipts/typing ninth-wave cross-domain soft isolation after #371', () => {
  ...
  for (let i = 0; i < 6; i++) {
    it(`cross-domain ninth-wave soft isolation flood-${i}`, async () => {
      const roomDO = createRoomDOStub();
      const env = createEnv({ roomDO });
      const badPresence = (['oNlInE', 'OfFlInE', 'Gone'] as const)[i % 3];
