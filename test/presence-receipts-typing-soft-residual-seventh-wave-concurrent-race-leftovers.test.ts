 ...
 * residual *presence + receipts + typing* soft→*concurrent-race* seventh-wave
 * binds unsaturated by #301/#305/#318/#327/#334 + open sixth-wave #341:
 *   never oNLINE/OfFline/UNAVAILAble/Online\\r/leading-space pins,
 *   never Invisible/Present/HIDDEN/BUSY/dnD invalid-case pins,
 *   never typing root-object `{}` / missing-key → Missing typing ∥ boolean ok,
 *   never receipt case-mutated m.read.PRIVATE/m.Fully_Read/M.read.private/
 *     m.Read.Private/M.Read under soft residual PA.
 *
 * Gap table (why leftover after #334 fifth + #341 sixth claims):
 *   oNLINE/OfFline/UNAVAILAble + Invisible/Present/HIDDEN + CR-trailing/
 *     leading-space ∥ ok
 *   typing root-object/missing-key Missing typing ∥ ok
 *   receipt residual case-mutated Invalid type matrix ∥ m.read ok
 *   cross-domain: oNLINE + root-object typing + m.read.PRIVATE ∥ oks
 *
...
// ---------------------------------------------------------------------------
// oNLINE / OfFline / Invisible / Present residual case softs ∥ ok
// ---------------------------------------------------------------------------

describe('presence seventh-wave concurrent residual case soft after #336', () => {
  const cases: Array<{ label: string; body: unknown; error: string }> = [
    ...
    {
      label: 'trailing-cr',
      body: { presence: 'Online\r' },
      error: 'Invalid presence state: Online\r. Must be one of: online, offline, unavailable',
    },
    {
      label: 'leading-space',
      body: { presence: ' online' },
  ...
  it('oNLINE ∥ OfFline ∥ Invisible ∥ HIDDEN ∥ offline under race', async () => {
    const env = createEnv();
    const results = await Promise.all([
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
    ...
    expect(
      ...
      [
        ...
        'Invalid presence state: HIDDEN. Must be one of: online, offline, unavailable',
      ].sort()
    );
  });
});
...
// ---------------------------------------------------------------------------
// Typing root-object {} / root-number Missing typing ∥ boolean ok
// ---------------------------------------------------------------------------

describe('typing seventh-wave concurrent root-object/number soft after #336', () => {
  it('root-object {} Missing typing ∥ boolean true under race', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
      typingReq(typingPath(USER_ENC), jsonInit('PUT', {}), env),
  ...
  it('root-object + typing:null ∥ two oks under race', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
      typingReq(typingPath(USER_ENC), jsonInit('PUT', {}), env),
    ...
    expect(
      results
        .filter((r) => r.status === 400)
        .every((r) => r.body.errcode === 'M_MISSING_PARAM')
    ).toBe(true);
  });

  for (let i = 0; i < 6; i++) {
    it(`typing root-object Missing flood-${i} ∥ ok`, async () => {
      const roomDO = createRoomDOStub();
      const env = createEnv({ roomDO });
      const results = await Promise.all([
        typingReq(typingPath(USER_ENC), jsonInit('PUT', { not_typing: true }), env),
        typingReq(
          typingPath(USER_ENC),
...
describe('receipts seventh-wave concurrent residual case-mutated Invalid type after #336', () => {
  ...
  for (let i = 0; i < 6; i++) {
    it(`receipt residual case soft flood-${i}`, async () => {
      const badType = (
        ['m.read.PRIVATE', 'm.Fully_Read', 'M.read.private', 'm.Read.Private', 'M.Read'] as const
      )[i % 5];
      const env = createEnv();
      const results = await Promise.all([
...
describe('presence/receipts/typing seventh-wave cross-domain soft isolation after #336', () => {
  ...
  it('oNLINE + root-object typing + m.read.PRIVATE ∥ three successes', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
      presenceReq(
  ...
  for (let i = 0; i < 6; i++) {
    it(`cross-domain seventh-wave soft isolation flood-${i}`, async () => {
      ...
      const env = createEnv({ roomDO });
      const badPresence =
        i % 3 === 0 ? 'oNLINE' : i % 3 === 1 ? 'Invisible' : 'BUSY';
      const badType = (
        ['m.read.PRIVATE', 'm.Fully_Read', 'M.Read'] as const
      )[i % 3];
      const results = await Promise.all([
        presenceReq(
          `/_matrix/client/v3/presence/${USER_ENC}/status`,
