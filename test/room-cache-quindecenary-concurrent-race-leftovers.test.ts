/**
 * TOKENMAXX HEAVY leftovers after #325 quattuordecenary / tip main — quindecenary
 * room-cache *concurrent race / TOCTOU* niches not landed by quattuordecenary:
 *   - joinedRaw [] isDm∥{} non-DM∥invitedRaw false→0
 *   - name -1 non-DM∥avatar url:[]∥avatar url:{} under miss
 *   - topic null stored∥alias [] stored∥hot sibling
 *   - gen CR"8"→8∥"7."→7∥VT"4"→4 get∥bump
 *   - meta deleteHold invalidate A mid∥miss-refill A∥bump B
 *   - get put-hold A∥invalidateBatch mid A∥getBatch hot C
 *   - getBatch getThrows A+B dual-miss∥deleteThrows inv A∥hot C
 *
 * Hibernation quindecenary leftovers live in call-room-hibernation.test.ts
 ...
 * and the hibernation quindecenary describes.
 */
...
// ---------------------------------------------------------------------------
// joinedRaw [] / {} / invitedRaw false
// ---------------------------------------------------------------------------

describe('race quindecenary joinedRaw []/{} + invited false after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`[] isDm∥{} non-DM∥invited false→0 under parallel miss flood-${i}`, async () => {
      ...
      const db = mockDb({
        ...
        [ROOM_B]: { joinedRaw: {}, invitedRaw: 1 },
        [ROOM_C]: { joinedRaw: 1, invitedRaw: false },
      });
      ...
      expect(a?.joinedCount).toEqual([]);
      expect(a?.isDm).toBe(true);
      // {} truthy stays; ToNumber({})=NaN <= 2 → false → non-DM
      expect(b?.joinedCount).toEqual({});
      expect(b?.invitedCount).toBe(1);
      expect(b?.isDm).toBe(false);
      // false falsy → || 0 for invited
...
// ---------------------------------------------------------------------------
// name -1 / avatar url:[] / avatar url:{}
// ---------------------------------------------------------------------------

describe('race quindecenary name -1 + avatar url []/{} after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`name -1 non-DM∥url []∥url {} under miss flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        getBarrier: [
          [metaKey(ROOM_A), 1],
          [metaKey(ROOM_B), 1],
      ...
      const db = mockDb({
        [ROOM_A]: { rawNameContent: JSON.stringify({ name: -1 }), joined: 1 },
      ...
      // -1 truthy → !name false → non-DM; url []/{} stored as-is
      expect(a?.name).toBe(-1);
      expect(a?.isDm).toBe(false);
      expect(b?.avatar).toEqual([]);
      expect(b?.isDm).toBe(true);
      expect(c?.avatar).toEqual({});
      expect(c?.isDm).toBe(true);
...
// ---------------------------------------------------------------------------
// topic null / alias [] / hot sibling
// ---------------------------------------------------------------------------

describe('race quindecenary topic null / alias [] after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`topic null∥alias []∥hot sibling flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
      ...
      const db = mockDb({
        [ROOM_A]: {
          rawTopicContent: JSON.stringify({ topic: null }),
          rawAliasContent: JSON.stringify({ alias: [] }),
          joined: 1,
        },
        [ROOM_B]: { rawTopicContent: JSON.stringify({ topic: null }), joined: 2 },
      });
      ...
      expect(a?.canonicalAlias).toEqual([]);
      expect(a?.isDm).toBe(true);
...
// ---------------------------------------------------------------------------
// Generation CR"8" / "7." / VT"4"
// ---------------------------------------------------------------------------

describe('race quindecenary gen CR/dot/VT parse after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`CR8→8∥7.→7∥VT4→4 get∥bump flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
          [genKey(ROOM_A)]: '\r8',
          [genKey(ROOM_B)]: '7.',
      ...
      // parseInt('\\r8')=8; parseInt('7.')=7; parseInt('\\v4')=4
      expect(gA).toBe(8);
      expect(bA).toBe(9);
      expect(gB).toBe(7);
      expect(bB).toBe(8);
      ...
      expect(ctl.data[genKey(ROOM_B)]).toBe('8');
...
describe('race quindecenary deleteHold inv∥miss-refill∥bump after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`deleteHold mid∥A refills FreshA∥bump B→1 flood-${i}`, async () => {
      ...
      const bumpP = bumpRoomCacheGeneration(kv, ROOM_B);

      await vi.waitFor(() => {
        expect(ctl.events.some((e) => e.includes(`delete-wait:${metaKey(ROOM_A)}`))).toBe(true);
      });

      const [a, gen] = await Promise.all([getP, bumpP]);
      expect(a?.name).toBe('FreshA');
      expect(gen).toBe(1);
      // delete still held — put may have overwritten stale key
      expect(JSON.parse(ctl.data[metaKey(ROOM_A)]).name).toBe('FreshA');

      releaseDelete(metaKey(ROOM_A));
      await invP;
      // delete after put may clear FreshA or no-op if put won race after release
...
describe('race quindecenary put-hold∥invBatch mid∥batch hot after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`put-hold A∥invBatch mid∥batch C hot isolation flood-${i}`, async () => {
      ...
      const getP = getRoomMetadata(kv, db, ROOM_A);
      const invBatchP = invalidateBatchRoomCache(kv, [ROOM_A]);
      ...
      const [inv, batch] = await Promise.all([invBatchP, batchP]);
      ...
      const a = await getP;
      // put after delete: either filled HoldA or empty if delete raced after release
...
describe('race quindecenary getBatch dual getThrow∥deleteThrow∥hot after #325', () => {
  for (let i = 0; i < 8; i++) {
    it(`batch A+B getThrow miss-refill∥inv deleteThrow∥C hot flood-${i}`, async () => {
      const { kv, ctl } = createRacingKv({
        data: {
          [metaKey(ROOM_A)]: JSON.stringify(freshMeta({ name: 'StaleA' })),
        ...
        getThrows: [metaKey(ROOM_A), metaKey(ROOM_B)],
