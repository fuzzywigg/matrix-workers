/**
 * TOKENMAXX HEAVY leftovers after #342 / tip #350 — nonary *admin* soft+concurrent-race
 * niches unsaturated by octonary IdP PUT updated/No-changes/test-ok/Bad-JSON (#342).
 * Relaunch of closed #351 onto tip after #350; lands closed septenary #335 niches
 * that never merged (complementary to #342).
 *
 ...
 *   IdP POST missing-param exact name,issuer_url,client_id,client_secret soft+race;
 *   Failed to create identity provider insert-fail soft+race (never soft-flooded in numbered);
 *   IdP POST /test discovery-fail success:false exact under soft+race with create-ok;
 *   Identity provider deleted ∥ User link removed success exact under race with create-fail.
 *
...
const IDP_CREATE_BODY = {
  ...
  issuer_url: 'https://idp.example.com',
  client_id: 'cid-sept',
  client_secret: 'secret-sept',
  scopes: 'openid profile',
};

describe('soft nonary admin IdP create/missing/test exact after #342', () => {
  for (let i = 0; i < 12; i++) {
    it(`IdP missing-param exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [
        { name: 'only-name' },
        { name: 'n', issuer_url: 'https://idp.example.com' },
  ...
  for (let i = 0; i < 12; i++) {
    it(`Failed to create identity provider exact flood-${i}`, async () => {
      const db = createAdminDb({ failIdpInsert: true, idpProviders: [] });
  ...
  for (let i = 0; i < 12; i++) {
    it(`IdP test discovery-fail exact soft flood-${i}`, async () => {
      ...
      const env = createEnv({ db });
      const res = await jsonReq(
        `/admin/api/idp/providers/badidp${i}/test`,
        jsonInit('POST', {}),
        env
      );
      expect(res.status).toBe(400);
  ...
  for (let i = 0; i < 8; i++) {
    it(`Identity provider deleted exact soft flood-${i}`, async () => {
      const db = createAdminDb({
        idpProviders: [
          {
            id: `del${i}`,
      ...
      const env = createEnv({ db });
      const res = await jsonReq(
        `/admin/api/idp/providers/del${i}`,
        jsonInit('DELETE'),
        env
      );
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.message).toBe('Identity provider deleted');
    });
  }

  for (let i = 0; i < 8; i++) {
    it(`User link removed exact soft flood-${i}`, async () => {
      const db = createAdminDb({
        idpProviders: [
          {
            id: 'idp1',
      ...
      const env = createEnv({ db });
      const res = await jsonReq(
        `/admin/api/idp/providers/idp1/links/${100 + i}`,
        jsonInit('DELETE'),
        env
      );
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.message).toBe('User link removed');
    });
  }
});

describe('race nonary admin IdP create/missing/test/delete after #342', () => {
  it('missing-param∥Failed to create isolation', async () => {
    const failDb = createAdminDb({ failIdpInsert: true, idpProviders: [] });
    const missEnv = createEnv({ db: createAdminDb({ idpProviders: [] }) });
    const failEnv = createEnv({ db: failDb });
    const [missing, fail] = await Promise.all([
      jsonReq('/admin/api/idp/providers', jsonInit('POST', { name: 'x' }), missEnv),
      jsonReq('/admin/api/idp/providers', jsonInit('POST', IDP_CREATE_BODY), failEnv),
    ]);
  ...
  it('Failed to create∥create ok isolation', async () => {
    const failDb = createAdminDb({ failIdpInsert: true, idpProviders: [] });
    ...
    const [fail, ok] = await Promise.all([
      ...
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'OkRace', client_id: 'cid-ok' }),
        createEnv({ db: okDb })
      ),
    ]);
    expect(fail.status).toBe(500);
  ...
  it('IdP test discovery-fail∥create ok isolation', async () => {
    const testDb = createAdminDb({
      idpProviders: [
        {
          id: 'badrace',
          name: 'Bad',
    ...
    const okDb = createAdminDb({ idpProviders: [] });
    const [testFail, ok] = await Promise.all([
      jsonReq(
        '/admin/api/idp/providers/badrace/test',
        jsonInit('POST', {}),
        createEnv({ db: testDb })
      ),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'FromTestRace' }),
        createEnv({ db: okDb })
      ),
    ]);
  ...
  it('Identity provider deleted∥User link removed∥Failed to create triple', async () => {
    const delDb = createAdminDb({
      idpProviders: [
        {
          id: 'todel',
          name: 'Del',
    ...
    const failDb = createAdminDb({ failIdpInsert: true, idpProviders: [] });
    const [del, link, fail] = await Promise.all([
      jsonReq('/admin/api/idp/providers/todel', jsonInit('DELETE'), createEnv({ db: delDb })),
      jsonReq(
        '/admin/api/idp/providers/idp1/links/77',
        jsonInit('DELETE'),
        createEnv({ db: linkDb })
      ),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', IDP_CREATE_BODY),
        createEnv({ db: failDb })
      ),
    ]);
    ...
    expect(link.body.message).toBe('User link removed');
    expect(fail.status).toBe(500);
    expect(fail.body.error).toBe('Failed to create identity provider');
  });

  it('missing-param∥Identity provider not found GET∥test discovery-fail triple', async () => {
    const testDb = createAdminDb({
      idpProviders: [
        {
          id: 'badtrip',
    ...
    const env = createEnv({ db: testDb });
    const [missing, notFound, testFail] = await Promise.all([
      jsonReq('/admin/api/idp/providers', jsonInit('POST', { client_id: 'only' }), env),
      jsonReq('/admin/api/idp/providers/gone-sept', {}, env),
      jsonReq('/admin/api/idp/providers/badtrip/test', jsonInit('POST', {}), env),
    ]);
    expect(missing.body.error).toBe(
      'Missing required parameter: name, issuer_url, client_id, and client_secret are required'
    );
    expect(notFound.status).toBe(404);
    expect(notFound.body.error).toBe('Identity provider not found');
    expect(testFail.status).toBe(400);
    expect(testFail.body.success).toBe(false);
  });

  for (let i = 0; i < 8; i++) {
    it(`nonary admin IdP exact race flood-${i}`, async () => {
      const failDb = createAdminDb({ failIdpInsert: true, idpProviders: [] });
      const delDb = createAdminDb({
        idpProviders: [
          {
      ...
      const results = await Promise.all([
        jsonReq(
          '/admin/api/idp/providers',
          jsonInit('POST', { name: `m${i}` }),
          createEnv({ db: createAdminDb({ idpProviders: [] }) })
        ),
        jsonReq(
          '/admin/api/idp/providers',
          jsonInit('POST', { ...IDP_CREATE_BODY, client_id: `f${i}` }),
          createEnv({ db: failDb })
        ),
        jsonReq(`/admin/api/idp/providers/rf${i}`, jsonInit('DELETE'), createEnv({ db: delDb })),
        jsonReq(
      ...
      expect(results[1].body.error).toBe('Failed to create identity provider');
      expect(results[2].body.message).toBe('Identity provider deleted');
