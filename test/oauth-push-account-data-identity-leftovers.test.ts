...
describe('oauth identity leftovers oauth helpers soft reliability round-3 after #151', () => {
  for (const [verifier, challenge, ok] of [
    ['same', 'same', true],
    ['a', 'b', false],
    ['', '', true],
    ['plain-r3', 'plain-r3', true],
  ] as const) {
    it(`verifyCodeChallenge plain soft ${JSON.stringify(verifier)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, 'plain')).toBe(ok);
    });
  }

  for (const method of ['s256', 'S256 ', 'PLAIN', 'md5']) {
    it(`verifyCodeChallenge method soft reject ${JSON.stringify(method)}`, async () => {
      expect(await verifyCodeChallenge('v', 'v', method)).toBe(false);
    });
  }

  it('verifyCodeChallenge S256 soft round-trip', async () => {
    const verifier = 'r3-verifier-abcdefghijklmnopqrstuvwxyz';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge(verifier + 'x', challenge, 'S256')).toBe(false);
  ...
  for (const raw of ['<script>', 'a&b', `"q"`, `o'clock`, 'safe']) {
    it(`escapeHtml soft reliability ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toContain('<');
      expect(out).not.toContain('>');
      if (raw.includes('&') && raw !== 'safe') expect(out).toContain('&amp;');
    });
  }

  it('generateLoginPage soft escapes client name XSS', () => {
    const html = generateLoginPage('<img src=x onerror=1>', 'rid-r3', SERVER, 'bad&err');
    expect(html).toContain(SERVER);
    expect(html).toContain('rid-r3');
    expect(html).not.toContain('<img src=x');
    expect(html).toContain('&lt;img');
  });

  it('hashClientSecret soft deterministic', async () => {
    const a = await hashClientSecret('r3-secret');
    const b = await hashClientSecret('r3-secret');
    expect(a).toBe(b);
    expect(a).not.toMatch(/[+/=]/);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  for (const body of ['', '{']) {
    it(`register soft bad JSON ${JSON.stringify(body)}`, async () => {
      const res = await oauthRequest('/oauth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body,
  ...
  it('register soft JSON null surfaces 5xx', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: 'null',
    });
    // Soft: null body parses but redirect_uris deref throws
    expect(res.status).toBeGreaterThanOrEqual(400);
  });

  it('register soft JSON array missing redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '[]',
  ...
  it('register soft empty redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_name: 'n', redirect_uris: [] }),
    });
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_client_metadata');
  });

  it('register soft auth method none omits secret', async () => {
    const cache = mockKv();
    const res = await oauthRequest(
      '/oauth/register',
      {
  ...
  for (const [qs, err] of [
    ['response_type=code&redirect_uri=' + encodeURIComponent(REDIRECT), 'invalid_request'],
    ['client_id=c&response_type=code', 'invalid_request'],
    [
      'client_id=c&redirect_uri=' + encodeURIComponent(REDIRECT) + '&response_type=token',
      'unsupported_response_type',
    ],
  ] as const) {
    it(`authorize GET soft reject ${err} qs=${qs.slice(0, 40)}`, async () => {
      const res = await oauthRequest(`/oauth/authorize?${qs}`);
      expect(res.status).toBe(400);
      expect((await oauthJson(res)).error).toBe(err);
    });
  }

  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
  ...
  it('authorize GET soft bad redirect_uri', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_bad_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_bad_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize POST soft missing fields HTML', async () => {
    const fd = new FormData();
    fd.set('username', '');
    fd.set('password', '');
    fd.set('auth_request_id', 'rid');
    const res = await oauthRequest('/oauth/authorize', { method: 'POST', body: fd });
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/Missing|password|username/i);
  });

  it('authorize POST soft expired auth request', async () => {
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'gone-r3');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ SESSIONS: mockKv() })
    );
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_request');
  });
...
describe('oauth identity leftovers oauth token soft edges round-3 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unknown client', async () => {
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
  ...
  it('token soft unsupported grant', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'password', client_id: 'cli_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft missing code', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_mc');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
    ...
    expect(res.status).toBe(400);
    const err = (await oauthJson(res)).error;
    expect(['invalid_request', 'invalid_grant']).toContain(err);
  });

  it('token soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_ru');
    sessions.data['oauth_code:ru3'] = JSON.stringify({
      code: 'ru3',
      client_id: 'cli_ru',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE plain fail', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_pkf');
    sessions.data['oauth_code:pkf'] = JSON.stringify({
      code: 'pkf',
      client_id: 'cli_pkf',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('refresh soft unknown token', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_rtu');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
  ...
  it('refresh soft past expires_at still accepted', async () => {
    // Soft: oauth refresh path does not check tokenData.expires_at
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_rte');
    sessions.data['oauth_refresh:rte'] = JSON.stringify({
      token_id: 'tid',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_rte',
    ...
    expect(sessions.deletes).toContain('oauth_refresh:rte');
  });

  it('token soft form-urlencoded content-type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_form_tok');
    const body = new URLSearchParams({
      grant_type: 'authorization_code',
      client_id: 'cli_form_tok',
      code: 'missing',
      redirect_uri: REDIRECT,
    });
    const res = await oauthRequest(
...
describe('oauth identity leftovers oauth revoke introspect JWT soft after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft deletes refresh token', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:del-r3'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'del-r3', token_type_hint: 'refresh_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:del-r3');
  });

  it('introspect soft active JWT', async () => {
    ...
    expect(j.sub).toBe(USER);
  });

  it('introspect soft expired JWT inactive', async () => {
    const header = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ alg: 'none' })));
    const payload = base64UrlEncode(
      new TextEncoder().encode(
        JSON.stringify({
  ...
  it('introspect soft malformed JWT falls through inactive', async () => {
    const token = 'a.b.c';
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('introspect soft opaque unknown inactive', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'opaque-r3' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('revoke soft missing token JSON', async () => {
    const res = await oauthRequest('/oauth/revoke', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
  ...
  it('introspect soft missing token JSON', async () => {
    const res = await oauthRequest('/oauth/introspect', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({}),
...
describe('oauth identity leftovers push condition soft flood round-3 after #151', () => {
  const event = baseEvent({
    content: { body: 'ping Alice please', msgtype: 'm.text', tags: ['a', 'b'], n: 3, flag: true },
    sender: '@carol:example.com',
  });

  it('event_match soft empty pattern falsy early-return', () => {
    // Soft: !condition.pattern treats '' as missing before userId placeholder
    expect(
      matchesCondition(
        { kind: 'event_match', key: 'sender', pattern: '' },
        baseEvent({ sender: USER }),
        USER,
        2
      )
    ).toBe(false);
    expect(
      matchesCondition({ kind: 'event_match', key: 'sender', pattern: '' }, event, USER, 2)
    ).toBe(false);
  });

  it('event_match soft missing key/pattern false', () => {
    expect(matchesCondition({ kind: 'event_match', pattern: 'x' }, event, USER, 2)).toBe(false);
    expect(matchesCondition({ kind: 'event_match', key: 'type' }, event, USER, 2)).toBe(false);
  });

  it('event_match soft missing nested value false', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'content.missing', pattern: '*' }, event, USER, 2)
    ).toBe(false);
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft flood is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
  ...
  it('event_property_is soft boolean and missing key', () => {
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.flag', value: true }, event, USER, 2)
    ).toBe(true);
    expect(matchesCondition({ kind: 'event_property_is', value: 1 }, event, USER, 2)).toBe(false);
  });

  it('event_property_contains soft array hit', () => {
    expect(
      matchesCondition(
  ...
  it('event_property_contains soft non-array false', () => {
    expect(
      matchesCondition(
        { kind: 'event_property_contains', key: 'content.body', value: 'ping' },
        event,
        USER,
        2
      )
  ...
  it('unknown condition kind soft true', () => {
    expect(matchesCondition({ kind: 'future_kind_r3' as any }, event, USER, 2)).toBe(true);
  });

  it('matchesRule every-condition soft AND', () => {
    const rule: PushRule = {
      rule_id: 'and',
      default: false,
      enabled: true,
    ...
    expect(matchesRule(rule, event, USER, 2)).toBe(true);
    expect(matchesRule(rule, event, USER, 1)).toBe(false);
  });

  it('matchesRule pattern soft case-insensitive glob', () => {
    const rule: PushRule = {
      rule_id: 'pat',
      default: false,
      enabled: true,
...
describe('oauth identity leftovers push gateway soft reliability round-3 after #151', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  for (const status of [200, 201, 202]) {
    it(`gateway soft success status ${status}`, async () => {
      fetchMock.mockResolvedValue(new Response(status === 202 ? null : 'ok', { status }));
      const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
      await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 3 });
      expect(db.updates.some((u) => u.kind === 'success')).toBe(true);
    });
  ...
  for (const status of [500, 502, 503, 504]) {
    it(`gateway soft 5xx status ${status}`, async () => {
      fetchMock.mockResolvedValue(new Response('err', { status }));
      const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
      await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
      expect(db.updates.some((u) => u.kind === 'failure')).toBe(true);
  ...
  it('multi-pusher soft mixed kind skips email', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({
      pushers: {
        [PUSH_USER]: [
          httpPusher({}, { pushkey: 'http1' }),
          { pushkey: 'mail', kind: 'email', app_id: 'mail.app', data: '{}' },
        ],
      },
    });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(db.updates.some((u) => u.pushkey === 'http1' && u.kind === 'success')).toBe(true);
  });

  it('sender_display_name soft uses override', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    await sendPushNotification(
      db,
      PUSH_USER,
      { ...baseEvent(), sender_display_name: 'Display R3' } as any,
      { unread: 1 }
    );
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification.sender_display_name || JSON.stringify(body)).toBeTruthy();
    expect(fetchMock).toHaveBeenCalled();
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-3 after #151', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('evaluatePushRules disabled soft disables default message', async () => {
    // Soft: enabled:0 overrides .m.rule.message; memberCount 3 skips one_to_one
    const db = pushRulesDb([
      {
        kind: 'underride',
        rule_id: '.m.rule.message',
  ...
  it('evaluatePushRules empty custom soft defaults notify', async () => {
    // Soft: no custom rows → default underride .m.rule.message notifies
    const r = await evaluatePushRules(pushRulesDb([]), PUSH_USER, baseEvent() as any, 3);
    expect(r.notify).toBe(true);
    expect(r.actions).toContain('notify');
    expect(r.highlight).toBe(false);
  });

  it('queueNotification soft queueThrow propagates', async () => {
    const db = createPushDb({ queueThrow: true });
    await expect(
      queueNotification(db, PUSH_USER, '!r:example.com', '$e', 'notify', ['notify'])
    ).rejects.toThrow(/queue fail/);
  });

  it('notifyRoomMembers soft dont_notify skips queue', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 3,
      pushRules: [
  ...
  it('notifyRoomMembers soft no members no-op', async () => {
    const db = createPushDb({ members: [], memberCount: 1 });
    await notifyRoomMembersOfMessage(db, {} as Env, baseEvent() as any);
    expect(db.queued).toHaveLength(0);
  });

  it('notifyRoomMembers soft corrupt room name ignored', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 4,
      roomNameContent: '{bad',
...
describe('oauth identity leftovers account-data soft reliability round-3 after #151', () => {
  it('global null content soft empty object', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: null }],
    });
    const rows = await getGlobalAccountData(db, USER);
  ...
  it('global corrupt JSON soft throws', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.bad', content: '{nope' }],
    });
    await expect(getGlobalAccountData(db, USER)).rejects.toThrow();
  });

  it('room null content soft empty object', async () => {
    const roomId = '!r3:example.com';
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: roomId, event_type: 'm.tag', content: null }],
    });
    expect(await getRoomAccountData(db, USER, roomId)).toEqual([{ type: 'm.tag', content: {} }]);
  });

  it('all rooms multi soft groups by room', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b:example.com', event_type: 'm.tag', content: '{"t":2}' },
        { user_id: USER, room_id: '!a:example.com', event_type: 'm.other', content: '{}' },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a:example.com', '!b:example.com']);
    expect(all['!a:example.com']).toHaveLength(2);
    expect(all['!b:example.com']).toEqual([{ type: 'm.tag', content: { t: 2 } }]);
  });

  it('stream position soft null defaults zero', async () => {
    expect(await getAccountDataStreamPosition(createAccountDataDb({ streamPosition: null }))).toBe(0);
  });

  it('stream position soft large value', async () => {
    expect(await getAccountDataStreamPosition(createAccountDataDb({ streamPosition: 99999 }))).toBe(99999);
  });

  it('prepare boom soft propagates global', async () => {
    await expect(getGlobalAccountData(createAccountDataDb({ throwOnPrepare: true }), USER)).rejects.toThrow(
      /prepare boom/
    );
  });

  it('all boom soft propagates room', async () => {
    await expect(
      getRoomAccountData(createAccountDataDb({ throwOnAll: true }), USER, '!r:example.com')
    ).rejects.toThrow(/all boom/);
  });

  for (const status of [400, 500, 502]) {
    it(`E2EE DO soft eventType status ${status}`, async () => {
      const ns = mockUserKeysNamespace({
        responses: new Map([['m.secret_storage.default_key', new Response('nope', { status })]]),
      });
      await expect(
        getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER, 'm.secret_storage.default_key')
      ).rejects.toThrow(/DO get failed/);
    });
  }

  it('E2EE DO soft eventType success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        [
          'm.secret_storage.default_key',
          new Response(JSON.stringify({ key: 'k1' }), { status: 200 }),
        ],
      ]),
    });
    const data = await getE2EEAccountDataFromDO(
      { USER_KEYS: ns } as any,
      USER,
      'm.secret_storage.default_key'
    );
    expect(data).toEqual({ key: 'k1' });
    expect(ns.fetches[0].url).toContain('event_type=');
  });
});

describe('oauth identity leftovers identity soft reliability round-3 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('account soft missing bearer', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/account`);
    expect(status).toBe(401);
    expect(body.errcode).toBe('M_MISSING_TOKEN');
  });

  it('account soft with bearer returns mxid', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/account`, {
      headers: { Authorization: 'Bearer tok-r3' },
    });
    expect(status).toBe(200);
    expect(body.user_id).toBe(`@unknown:${SERVER}`);
  });

  it('register soft bad JSON', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/account/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{',
  ...
  it('register soft echoes access_token', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/account/register`,
      postJson({
        access_token: 'at-r3',
    ...
    expect(status).toBe(200);
    expect(body.token).toBe('at-r3');
  });

  it('terms GET/POST soft empty', async () => {
    const g = await identityRequest(`${ID_BASE}/terms`);
    expect(g.status).toBe(200);
    expect(g.body.policies).toEqual({});
    const p = await identityRequest(`${ID_BASE}/terms`, { method: 'POST' });
    expect(p.status).toBe(200);
    expect(p.body).toEqual({});
  });

  it('lookup none soft maps address medium', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r3' });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'a@example.com', mxid: USER }],
    });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r3', addresses: ['a@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings['a@example.com email']).toBe(USER);
  ...
  it('lookup none soft miss and short addr', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r3' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r3', addresses: ['onlyone', 'b@example.com email'] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(200);
    expect(body.mappings).toEqual({});
  ...
  it('lookup sha256 soft match', async () => {
    const pepper = 'pep-sha';
    const cache = mockKv({ 'identity:pepper': pepper });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'z@example.com', mxid: USER }],
    });
    const hash = await sha256(`z@example.com email ${pepper}`);
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
  ...
  it('lookup soft throwOnAll surfaces', async () => {
    const cache = mockKv({ 'identity:pepper': 'p' });
    const db = createIdentityDb({ throwOnAll: true });
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
  ...
  it('requestToken soft missing client_secret', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'a@example.com' })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_MISSING_PARAM');
  });

  it('requestToken soft success inserts session', async () => {
    vi.spyOn(crypto, 'randomUUID').mockReturnValue('aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeee99');
    vi.spyOn(Math, 'random').mockReturnValue(0.5);
    const db = createIdentityDb();
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'r3@example.com', client_secret: 'cs-r3', send_attempt: 2 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.sid).toBe('aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeee99');
    expect(db.emailSessions.get(body.sid)?.email).toBe('r3@example.com');
    expect(db.emailSessions.get(body.sid)?.send_attempt).toBe(2);
  });

  it('submitToken soft wrong token', async () => {
    const db = createIdentityDb({
      emailSessions: new Map([
        [
          'sid-r3w',
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r3w', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('submitToken soft expired session', async () => {
    const db = createIdentityDb({
      emailSessions: new Map([
        [
          'sid-r3e',
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r3e', client_secret: 'cs', token: '222222' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft missing session', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'none', client_secret: 'cs', token: '1' })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_NO_VALID_SESSION');
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-3 after #151', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r3-${i}`, () => {
      const len = 6 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 4; i++) {
    it(`base64Url soft-cap r3-${i}`, () => {
      const u8 = new Uint8Array(Array.from({ length: i + 3 }, (_, j) => (i * 19 + j * 5) % 256));
      const enc = base64UrlEncode(u8);
      expect(enc).not.toMatch(/[+/=]/);
      expect(Array.from(base64UrlDecode(enc))).toEqual(Array.from(u8));
    });
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r3-${i}`, () => {
      const obj: any = { a: [{ v: i }, { v: i + 1 }] };
      expect(getNestedValue(obj, 'a')).toEqual([{ v: i }, { v: i + 1 }]);
      expect(getNestedValue(obj, 'a.0')).toEqual({ v: i });
      expect(getNestedValue(undefined, 'x')).toBeUndefined();
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`UIA HTML soft-cap r3-${i}`, () => {
      expect(generateUiaSuccessPage(`r3-${i}`, SERVER)).toContain(`r3-${i}`);
      expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
      expect(generateUiaErrorPage(`E${i}`, `err-${i}`, SERVER)).toContain(`err-${i}`);
      expect(generateLoginPage(`c${i}`, `id${i}`, SERVER)).toContain(SERVER);
    });
  }
});

describe('oauth identity leftovers oauth helpers soft reliability round-4 after #151', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v4a', 'v4a', 'plain', true],
    ['v4b', 'v4c', 'plain', false],
    ['', 'x', 'plain', false],
    ['x', '', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r4 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft mismatch digest', async () => {
    const verifier = 'round4-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-zzzzzzzzzzzzzzzzzzzz', challenge, 'S256')).toBe(false);
  });

  for (const method of ['S256', 'plain', 's256', 'Plain']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      const ok = method === 'S256' || method === 'plain';
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z', 'z', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r4-s256-case-aaaaaaaaaaaaaaaaaaaaaa';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z', 'z', method)).toBe(false);
      }
      expect(typeof ok).toBe('boolean');
    });
  }

  for (const raw of ['<>', '&', '"', "'", 'ok-r4', '<b>&"\'']) {
    it(`escapeHtml soft r4 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds auth_request_id and server', () => {
    const html = generateLoginPage('Client R4', 'auth-r4-id', SERVER, 'bad creds');
    expect(html).toContain('auth-r4-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('bad creds');
  });

  it('hashClientSecret soft distinct secrets', async () => {
    const a = await hashClientSecret('r4-a');
    const b = await hashClientSecret('r4-b');
    expect(a).not.toBe(b);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain server', () => {
    expect(generateUiaSuccessPage('sid-r4', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T', 'msg-r4', SERVER)).toContain('msg-r4');
    expect(generateUiaApprovalPage('sid', USER, 'Title', 'Desc', SERVER)).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-4 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft missing Content-Type body still parsed as json by hono path', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ redirect_uris: [REDIRECT], client_name: 'R4' }),
    });
    expect(res.status).toBe(201);
    const j = await oauthJson(res);
    expect(typeof j.client_id).toBe('string');
  ...
  it('register soft confidential stores secret hash', async () => {
    const cache = mockKv();
    const res = await oauthRequest(
      '/oauth/register',
      {
    ...
    const j = await oauthJson(res);
    expect(typeof j.client_secret).toBe('string');
    const stored = JSON.parse(cache.data[`oauth_client:${j.client_id}`]);
    expect(stored.client_secret_hash).toBeTruthy();
    expect(stored.client_secret_hash).toBe(await hashClientSecret(j.client_secret as string));
  });

  it('authorize GET soft success HTML login page', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_ok');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r4_ok&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&state=st4`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
  ...
  it('authorize GET soft stores auth request in sessions', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_store');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r4_store&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&scope=openid`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
    ...
    expect(stored.client_id).toBe('cli_r4_store');
    expect(stored.redirect_uri).toBe(REDIRECT);
  });

  it('authorize POST soft wrong password HTML', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_wp');
    sessions.data['oauth_auth_request:rid-wp'] = JSON.stringify({
      client_id: 'cli_r4_wp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:right' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'wrong');
    fd.set('auth_request_id', 'rid-wp');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/Invalid|password|credentials/i);
  });

  it('authorize POST soft success redirects with code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_okp');
    sessions.data['oauth_auth_request:rid-ok'] = JSON.stringify({
      client_id: 'cli_r4_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-ok',
      code_challenge: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:pw' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'rid-ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc).toContain('state=st-ok');
  });

  it('authorize GET soft response_type missing', async () => {
    // Soft: missing response_type !== 'code' → unsupported_response_type
    const cache = mockKv();
    seedClient(cache, 'cli_r4_rt');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r4_rt&redirect_uri=${encodeURIComponent(REDIRECT)}`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
...
describe('oauth identity leftovers oauth token soft edges round-4 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported content-type', async () => {
    const res = await oauthRequest('/oauth/token', {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain' },
      body: 'grant_type=authorization_code',
  ...
  it('token soft missing client_id', async () => {
    const res = await oauthRequest('/oauth/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ grant_type: 'authorization_code', code: 'x', redirect_uri: REDIRECT }),
  ...
  it('token soft expired auth code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_exp');
    sessions.data['oauth_code:exp4'] = JSON.stringify({
      code: 'exp4',
      client_id: 'cli_r4_exp',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_grant');
    expect(sessions.deletes).toContain('oauth_code:exp4');
  });

  it('token soft client_id mismatch on code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_a');
    seedClient(cache, 'cli_r4_b');
    sessions.data['oauth_code:mis4'] = JSON.stringify({
      code: 'mis4',
      client_id: 'cli_r4_a',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE missing verifier', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_pk');
    sessions.data['oauth_code:pk4'] = JSON.stringify({
      code: 'pk4',
      client_id: 'cli_r4_pk',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft authorization_code success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_okt');
    sessions.data['oauth_code:ok4'] = JSON.stringify({
      code: 'ok4',
      client_id: 'cli_r4_okt',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(sessions.deletes).toContain('oauth_code:ok4');
  });

  it('token soft Basic auth client_id header', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r4_basic');
    const basic = btoa('cli_r4_basic:');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
  ...
  it('refresh soft client_id mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r4_rt1');
    seedClient(cache, 'cli_r4_rt2');
    sessions.data['oauth_refresh:rf4'] = JSON.stringify({
      token_id: 'tid',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r4_rt1',
      user_id: USER,
      device_id: 'DEV',
      scope: 'openid',
      created_at: NOW,
...
describe('oauth identity leftovers oauth revoke introspect uia soft round-4 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft form-urlencoded deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:form-r4'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'form-r4', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:form-r4');
  });

  it('revoke soft unknown token still ok', async () => {
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'nope-r4' }),
      },
      makeOAuthEnv({ SESSIONS: mockKv() })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('introspect soft active opaque via access_tokens', async () => {
    const token = 'opaque-active-r4';
    const hash = await sha256(token);
    const db = createOAuthDb({
      tokensByHash: new Map([[hash, { user_id: USER, device_id: 'D1' }]]),
    });
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('introspect soft JWT missing exp treated', async () => {
    const header = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ alg: 'none' })));
    const payload = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ sub: USER })));
    const token = `${header}.${payload}.sig`;
    const res = await oauthRequest(
      '/oauth/introspect',
      {
    ...
    expect(res.status).toBe(200);
    const j = await oauthJson(res);
    // Soft: no exp → active path still may succeed if decode works
    expect(typeof j.active).toBe('boolean');
  });

  it('UIA GET soft missing session HTML', async () => {
    const res = await oauthRequest('/oauth/authorize/uia', {}, makeOAuthEnv());
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/Missing Session|session/i);
  });

  it('UIA GET soft expired session HTML', async () => {
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=gone-r4',
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA GET soft approval page for existing session', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r4'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r4&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).toMatch(/Reset|encryption|Approve/i);
    expect(html).toContain(USER);
  });

  it('UIA POST soft cancel deletes session', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-cancel'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const fd = new FormData();
    fd.set('session', 'uia-cancel');
    fd.set('action', 'cancel');
    const res = await oauthRequest(
      '/oauth/authorize/uia',
      { method: 'POST', body: fd },
...
describe('oauth identity leftovers push condition soft flood round-4 after #151', () => {
  const event = baseEvent({
    content: { body: 'hello Bob and Alice', msgtype: 'm.text', tags: ['x', 'y'], n: 7, flag: false },
    sender: '@dave:example.com',
  });

  it('event_match soft user_id placeholder empty-pattern replaced', () => {
    // Soft documented elsewhere: '' pattern early-returns false before placeholder —
    // non-empty exact userId pattern still matches
    expect(
      matchesCondition(
  ...
  it('event_match soft glob star', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'type', pattern: 'm.room.*' }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_match', key: 'type', pattern: 'm.call.*' }, event, USER, 2)
    ).toBe(false);
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r4 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is: is || undefined }, event, USER, count)).toBe(
        ok
      );
    });
  }
  ...
  it('contains_display_name soft case-insensitive hit', () => {
    expect(
      matchesCondition({ kind: 'contains_display_name' }, event, USER, 2, 'alice')
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'contains_display_name' }, event, USER, 2, 'Zoe')
    ).toBe(false);
  });

  it('contains_display_name soft missing displayName false', () => {
    expect(matchesCondition({ kind: 'contains_display_name' }, event, USER, 2)).toBe(false);
  });

  it('event_property_is soft number and false bool', () => {
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.n', value: 7 }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.flag', value: false }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.flag', value: true }, event, USER, 2)
    ).toBe(false);
  ...
  it('event_property_contains soft miss and missing key', () => {
    expect(
      matchesCondition(
    ...
    expect(matchesCondition({ kind: 'event_property_contains', value: 'x' }, event, USER, 2)).toBe(false);
  });

  it('matchesRule disabled soft still matches', () => {
    // Soft: matchesRule ignores enabled; filter is in evaluatePushRules
    const rule: PushRule = {
      rule_id: 'off',
      default: false,
      enabled: false,
  ...
  it('matchesRule empty conditions soft true', () => {
    const rule: PushRule = {
      rule_id: 'empty',
      default: false,
      enabled: true,
...
describe('oauth identity leftovers push gateway soft reliability round-4 after #151', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  for (const status of [400, 401, 403, 404, 429]) {
    it(`gateway soft 4xx status ${status}`, async () => {
      fetchMock.mockResolvedValue(new Response('nope', { status }));
      const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
      await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 2 });
      expect(db.updates.some((u) => u.kind === 'failure')).toBe(true);
  ...
  it('gateway soft fetch throw records failure', async () => {
    fetchMock.mockRejectedValue(new Error('network down'));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
    expect(db.updates.some((u) => u.kind === 'failure')).toBe(true);
  ...
  it('corrupt pusher data soft skipped', async () => {
    const db = createPushDb({
      pushers: {
        [PUSH_USER]: [{ pushkey: 'bad', kind: 'http', app_id: 'a', data: '{bad' }],
      },
    });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
    expect(fetchMock).not.toHaveBeenCalled();
  ...
  it('pusher missing url soft skipped', async () => {
    const db = createPushDb({
      pushers: {
        [PUSH_USER]: [
          {
  ...
  it('encrypted event soft alert title body from room', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    await sendPushNotification(
      db,
      PUSH_USER,
      {
        ...baseEvent({ type: 'm.room.encrypted', content: {} }),
    ...
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    const alert = body.notification?.devices?.[0]?.data?.default_payload?.aps?.alert;
    expect(alert?.title).toBe('Eve');
    expect(alert?.body).toBe('Secret Room');
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-4 after #151', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('evaluatePushRules soft default notifies message', async () => {
    const r = await evaluatePushRules(pushRulesDb([]), PUSH_USER, baseEvent() as any, 5);
    expect(r.notify).toBe(true);
    expect(r.actions).toContain('notify');
  });

  it('queueNotification soft inserts row', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r:example.com', '$e:example.com', 'm.room.message', [
      'notify',
    ]);
    expect(db.queued).toHaveLength(1);
    expect(db.queued[0].user_id).toBe(PUSH_USER);
    expect(db.queued[0].notification_type).toBe('m.room.message');
  });

  it('queueNotification soft throw propagates', async () => {
    const db = createPushDb({ queueThrow: true });
    await expect(
      queueNotification(db, PUSH_USER, '!r:example.com', '$e:example.com', 'm.room.message', ['notify'])
    ).rejects.toThrow(/queue fail/);
  });

  it('notifyRoomMembers soft queues for member with notify rule', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 3,
      pushRules: [
    ...
    await notifyRoomMembersOfMessage(db, {} as Env, baseEvent() as any);
    expect(db.queued.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('notifyRoomMembers soft sender excluded from members', async () => {
    const db = createPushDb({
      members: ['@alice:example.com', PUSH_USER],
      memberCount: 3,
      pushRules: [
        {
          kind: 'underride',
      ...
      pushers: {
        ['@alice:example.com']: [httpPusher({}, { pushkey: 'alice' })],
        [PUSH_USER]: [httpPusher({}, { pushkey: 'bob' })],
      },
    });
    await notifyRoomMembersOfMessage(db, {} as Env, baseEvent({ sender: '@alice:example.com' }) as any);
    expect(db.queued.every((q) => q.user_id !== '@alice:example.com')).toBe(true);
  });
});

describe('oauth identity leftovers account-data soft reliability round-4 after #151', () => {
  it('global soft empty rows', async () => {
    expect(await getGlobalAccountData(createAccountDataDb({ rows: [] }), USER)).toEqual([]);
  });

  it('global soft since filters by change position', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '', event_type: 'm.direct', content: '{"a":1}' },
        { user_id: USER, room_id: '', event_type: 'm.tag', content: '{}' },
      ],
      changes: [
        { user_id: USER, room_id: '', event_type: 'm.direct', stream_position: 10 },
        { user_id: USER, room_id: '', event_type: 'm.tag', stream_position: 5 },
      ],
    });
    const rows = await getGlobalAccountData(db, USER, 7);
    expect(rows).toEqual([{ type: 'm.direct', content: { a: 1 } }]);
  });

  it('room soft since miss empty', async () => {
    const roomId = '!r4:example.com';
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: roomId, event_type: 'm.tag', content: '{"tags":{}}' }],
      changes: [{ user_id: USER, room_id: roomId, event_type: 'm.tag', stream_position: 3 }],
    });
    expect(await getRoomAccountData(db, USER, roomId, 10)).toEqual([]);
  });

  it('all rooms soft empty room list', async () => {
    const all = await getAllRoomAccountData(createAccountDataDb({ rows: [] }), USER, []);
    expect(all).toEqual({});
  });

  it('all rooms soft since filters', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b:example.com', event_type: 'm.tag', content: '{"t":2}' },
      ...
      changes: [
        { user_id: USER, room_id: '!a:example.com', event_type: 'm.tag', stream_position: 20 },
        { user_id: USER, room_id: '!b:example.com', event_type: 'm.tag', stream_position: 2 },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a:example.com', '!b:example.com'], 5);
    expect(all['!a:example.com']).toEqual([{ type: 'm.tag', content: { t: 1 } }]);
    expect(all['!b:example.com'] ?? []).toEqual([]);
  });

  it('stream position soft default fixture 42', async () => {
    expect(await getAccountDataStreamPosition(createAccountDataDb({}))).toBe(42);
  });

  it('prepare boom soft propagates stream', async () => {
    await expect(getAccountDataStreamPosition(createAccountDataDb({ throwOnPrepare: true }))).rejects.toThrow(
      /prepare boom/
    );
  });

  it('E2EE DO soft all keys success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        ['__all__', new Response(JSON.stringify({ 'm.secret_storage.default_key': { key: 'k' } }), { status: 200 })],
      ]),
    });
    const data = await getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER);
    expect(data['m.secret_storage.default_key']).toEqual({ key: 'k' });
  });

  for (const status of [401, 403, 404]) {
    it(`E2EE DO soft all status ${status}`, async () => {
      const ns = mockUserKeysNamespace({
        responses: new Map([['__all__', new Response('x', { status })]]),
      });
      await expect(getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER)).rejects.toThrow(/DO get failed/);
    });
  }

  it('E2EE DO soft throwOnFetch propagates', async () => {
    const ns = mockUserKeysNamespace({ throwOnFetch: new Error('do network') });
    await expect(getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER)).rejects.toThrow(/do network/);
  });
});

describe('oauth identity leftovers identity soft reliability round-4 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('hash_details soft creates pepper with TTL', async () => {
    const cache = mockKv();
    const { status, body } = await identityRequest(`${ID_BASE}/hash_details`, {}, makeIdentityEnv({ cache }));
    expect(status).toBe(200);
    expect(body.algorithms).toEqual(['sha256', 'none']);
    expect(typeof body.lookup_pepper).toBe('string');
    expect(cache.data['identity:pepper']).toBe(body.lookup_pepper);
    expect(cache.puts.some((p) => p.key === 'identity:pepper' && p.options?.expirationTtl === SEVEN_DAY_TTL)).toBe(
      true
    );
  });
  ...
  it('hash_details soft reuses existing pepper', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-exist-r4' });
    const { body } = await identityRequest(`${ID_BASE}/hash_details`, {}, makeIdentityEnv({ cache }));
    expect(body.lookup_pepper).toBe('pep-exist-r4');
    expect(cache.puts.filter((p) => p.key === 'identity:pepper')).toHaveLength(0);
  });

  it('lookup soft bad JSON', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/lookup`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{',
  ...
  it('lookup soft missing algorithm', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ pepper: 'p', addresses: [] }),
      makeIdentityEnv({ cache: mockKv({ 'identity:pepper': 'p' }) })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  });

  it('lookup soft invalid pepper returns current', async () => {
    const cache = mockKv({ 'identity:pepper': 'real-pep' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'wrong', addresses: [] }),
      makeIdentityEnv({ cache })
    ...
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PEPPER');
    expect(body.lookup_pepper).toBe('real-pep');
  });

  it('lookup none soft phone medium', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r4' });
    const db = createIdentityDb({
      associations: [{ medium: 'msisdn', address: '15551212', mxid: USER }],
    });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r4', addresses: ['15551212 msisdn'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings['15551212 msisdn']).toBe(USER);
  });

  it('requestToken soft missing email', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ client_secret: 'cs' })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_MISSING_PARAM');
  });

  it('requestToken soft bad JSON', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/validate/email/requestToken`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: 'not-json',
  ...
  it('submitToken soft success validates', async () => {
    const db = createIdentityDb({
      emailSessions: new Map([
        [
          'sid-r4ok',
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r4ok', client_secret: 'cs-ok', token: '654321' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r4ok')?.validated).toBe(1);
    expect(db.updates).toHaveLength(1);
  });

  it('submitToken soft wrong client_secret', async () => {
    const db = createIdentityDb({
      emailSessions: new Map([
        [
          'sid-r4ws',
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r4ws', client_secret: 'wrong', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_NO_VALID_SESSION');
  ...
  it('submitToken soft throwOnRun surfaces', async () => {
    const db = createIdentityDb({
      throwOnRun: true,
      emailSessions: new Map([
        [
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r4run', client_secret: 'cs', token: '999999' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });

  it('account soft bearer empty still missing', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/account`, {
      headers: { Authorization: 'Bearer ' },
    });
    // Soft: extractBearerToken may treat trailing space as empty token
    expect([401, 200]).toContain(status);
    if (status === 401) expect(body.errcode).toBe('M_MISSING_TOKEN');
  });
});

describe('oauth identity leftovers soft-cap flood helpers round-4 after #151', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r4-${i}`, () => {
      const len = 5 + i * 3;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 4; i++) {
    it(`base64Url soft-cap r4-${i}`, () => {
      const u8 = new Uint8Array(Array.from({ length: i + 1 }, (_, j) => (i * 23 + j * 7) % 256));
      const enc = base64UrlEncode(u8);
      expect(enc).not.toMatch(/[+/=]/);
      expect(Array.from(base64UrlDecode(enc))).toEqual(Array.from(u8));
    });
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r4-${i}`, () => {
      const obj: any = { layer: { items: [{ id: i }, null] } };
      expect(getNestedValue(obj, 'layer.items.0.id')).toBe(i);
      expect(getNestedValue(obj, 'layer.items.1')).toBeNull();
      expect(getNestedValue(obj, 'layer.items.1.x')).toBeUndefined();
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r4-${i}`, async () => {
      const h = await hashClientSecret(`r4-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r4-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r4-${i}`, () => {
      expect(escapeHtml(`<r4-${i}>&`)).toBe(`&lt;r4-${i}&gt;&amp;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r4-${i}`, () => {
      expect(generateUiaSuccessPage(`r4-${i}`, SERVER)).toContain(`r4-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
});

describe('oauth identity leftovers oauth helpers soft reliability round-5 after #151', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v5a', 'v5a', 'plain', true],
    ['v5b', 'v5c', 'plain', false],
    ['same', 'same', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r5 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round5-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  });

  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z5', 'z5', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r5-s256-case-bbbbbbbbbbbbbbbbbbbbbb';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z5', 'z5', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&', '"\'', 'plain-r5', '<script>', 'a&b"c\'d']) {
    it(`escapeHtml soft r5 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R5', 'auth-r5-id', SERVER, 'login failed');
    expect(html).toContain('auth-r5-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r5-a');
    const b = await hashClientSecret('r5-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r5-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r5', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T5', 'msg-r5', SERVER)).toContain('msg-r5');
    expect(generateUiaApprovalPage('sid5', USER, 'Title5', 'Desc5', SERVER, 'extra5')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-5 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft public client no secret', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        redirect_uris: [REDIRECT],
        client_name: 'R5 Public',
        token_endpoint_auth_method: 'none',
      }),
    });
    expect(res.status).toBe(201);
  ...
  it('register soft confidential client_secret_basic', async () => {
    const cache = mockKv();
    const res = await oauthRequest(
      '/oauth/register',
      {
  ...
  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing_r5&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r5_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r5_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft stores PKCE challenge', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_pkce');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r5_pkce&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&code_challenge=chal5&code_challenge_method=S256`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
    ...
    expect(keys.length).toBeGreaterThanOrEqual(1);
    const stored = JSON.parse(sessions.data[keys[0]]);
    expect(stored.code_challenge).toBe('chal5');
    expect(stored.code_challenge_method).toBe('S256');
  });

  it('authorize POST soft missing auth_request_id', async () => {
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    const res = await oauthRequest('/oauth/authorize', { method: 'POST', body: fd }, makeOAuthEnv());
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/expired|session|request/i);
  });

  it('authorize POST soft deactivated user', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_deact');
    sessions.data['oauth_auth_request:rid-deact'] = JSON.stringify({
      client_id: 'cli_r5_deact',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
      code_challenge: null,
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'rid-deact');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/Invalid|password|credentials|deactivat/i);
  });

  it('authorize POST soft success with state', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_okp');
    sessions.data['oauth_auth_request:rid-r5ok'] = JSON.stringify({
      client_id: 'cli_r5_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r5',
      code_challenge: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:pw5' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw5');
    fd.set('auth_request_id', 'rid-r5ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc.startsWith(REDIRECT)).toBe(true);
    expect(loc).toMatch(/code=/);
    expect(loc).toContain('state=st-r5');
  });
});

describe('oauth identity leftovers oauth token soft edges round-5 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported grant_type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r5_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'password', client_id: 'cli_r5_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft unknown client', async () => {
    ...
    // Soft: token endpoint returns 401 for unknown client (authorize uses 400)
    expect(res.status).toBe(401);
    expect((await oauthJson(res)).error).toBe('invalid_client');
  });
  ...
  it('token soft redirect_uri mismatch', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_ru');
    sessions.data['oauth_code:ru5'] = JSON.stringify({
      code: 'ru5',
      client_id: 'cli_r5_ru',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE plain success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_pkok');
    sessions.data['oauth_code:pk5'] = JSON.stringify({
      code: 'pk5',
      client_id: 'cli_r5_pkok',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r5-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk5',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r5-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
    ...
    const j = await oauthJson(res);
    expect(typeof j.access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_code:pk5');
  });

  it('token soft PKCE plain mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_pkbad');
    sessions.data['oauth_code:pk5b'] = JSON.stringify({
      code: 'pk5b',
      client_id: 'cli_r5_pkbad',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'expected',
  ...
  it('token soft form-urlencoded grant', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_form');
    sessions.data['oauth_code:form5'] = JSON.stringify({
      code: 'form5',
      client_id: 'cli_r5_form',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(res.status).toBe(200);
    expect(typeof (await oauthJson(res)).access_token).toBe('string');
  });

  it('refresh soft past expires_at still rotates', async () => {
    // Soft: refresh path does not check expires_at — only presence + client_id
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_rfexp');
    sessions.data['oauth_refresh:rf5exp'] = JSON.stringify({
      token_id: 'tid5',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r5_rfexp',
      user_id: USER,
      device_id: 'DEV5',
      scope: 'openid',
      created_at: NOW - DAY_MS,
    ...
    expect(res.status).toBe(200);
    expect(typeof (await oauthJson(res)).access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf5exp');
  });

  it('refresh soft success rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r5_rfok');
    sessions.data['oauth_refresh:rf5ok'] = JSON.stringify({
      token_id: 'tid5ok',
      access_token_hash: 'h5',
      refresh_token_hash: 'rh5',
      client_id: 'cli_r5_rfok',
      user_id: USER,
      device_id: 'DEV5OK',
      scope: 'openid',
      created_at: NOW,
    ...
    expect(typeof j.access_token).toBe('string');
    expect(typeof j.refresh_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf5ok');
  });
});

describe('oauth identity leftovers oauth revoke introspect uia soft round-5 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft json deletes access hint', async () => {
    const sessions = mockKv();
    sessions.data['oauth_access:acc-r5'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'acc-r5', token_type_hint: 'access_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('revoke soft form deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:rev-r5'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'rev-r5', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:rev-r5');
  });

  it('introspect soft inactive unknown opaque', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'unknown-opaque-r5' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('introspect soft active opaque device', async () => {
    const token = 'opaque-active-r5';
    const hash = await sha256(token);
    const db = createOAuthDb({
      tokensByHash: new Map([[hash, { user_id: USER, device_id: 'D5' }]]),
    });
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('introspect soft JWT expired inactive', async () => {
    const header = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ alg: 'none' })));
    const payload = base64UrlEncode(
      new TextEncoder().encode(JSON.stringify({ sub: USER, exp: Math.floor(NOW / 1000) - 10 }))
    );
    const token = `${header}.${payload}.sig`;
    const res = await oauthRequest(
      '/oauth/introspect',
  ...
  it('UIA GET soft missing session', async () => {
    const res = await oauthRequest('/oauth/authorize/uia', {}, makeOAuthEnv());
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/Missing Session|session/i);
  });

  it('UIA GET soft approval page', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r5'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r5&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA POST soft approve success', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-ok5'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const fd = new FormData();
    fd.set('session', 'uia-ok5');
    ...
    expect(res.status).toBe(200);
    expect(await res.text()).toMatch(/success|approved|complete/i);
  });
});

describe('oauth identity leftovers push condition soft flood round-5 after #151', () => {
  const event = baseEvent({
    content: { body: 'ping Carol and Dave', msgtype: 'm.text', tags: ['a', 'b'], n: 3, flag: true },
    sender: '@eve:example.com',
  });

  it('event_match soft exact type', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'type', pattern: 'm.room.message' }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_match', key: 'type', pattern: 'm.room.member' }, event, USER, 2)
    ).toBe(false);
  });

  it('event_match soft missing key false', () => {
    expect(matchesCondition({ kind: 'event_match', pattern: 'x' }, event, USER, 2)).toBe(false);
  });

  it('event_match soft empty pattern false', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'type', pattern: '' }, event, USER, 2)
    ).toBe(false);
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r5 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }

  it('contains_display_name soft substring miss', () => {
    // Soft: includes() — displayName not present as substring
    expect(
      matchesCondition(
        { kind: 'contains_display_name' },
        baseEvent({ content: { body: 'carolyn', msgtype: 'm.text' } }),
        USER,
        2,
        'zoe'
      )
  ...
  it('contains_display_name soft hit in body', () => {
    expect(
      matchesCondition({ kind: 'contains_display_name' }, event, USER, 2, 'Carol')
    ).toBe(true);
  });

  it('event_property_is soft string and nullish miss', () => {
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.msgtype', value: 'm.text' }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_property_is', key: 'content.missing', value: 'x' }, event, USER, 2)
    ).toBe(false);
  });

  it('event_property_contains soft hit tag', () => {
    expect(
      matchesCondition(
        { kind: 'event_property_contains', key: 'content.tags', value: 'a' },
        event,
  ...
  it('unknown condition kind soft true', () => {
    // Soft: default branch returns true for unrecognized kinds
    expect(matchesCondition({ kind: 'not_a_real_kind' } as any, event, USER, 2)).toBe(true);
  });

  it('matchesRule soft AND fails on one miss', () => {
    const rule: PushRule = {
      rule_id: 'and',
      default: false,
      enabled: true,
    ...
    expect(matchesRule(rule, event, USER, 2)).toBe(false);
  });

  it('matchesRule soft null conditions treated empty', () => {
    const rule: PushRule = {
      rule_id: 'nullcond',
      default: false,
      enabled: true,
...
describe('oauth identity leftovers push gateway soft reliability round-5 after #151', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  it('gateway soft 200 records success', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 3 });
    expect(db.updates.some((u) => u.kind === 'success')).toBe(true);
  ...
  it('empty pushers soft no fetch', async () => {
    const db = createPushDb({ pushers: { [PUSH_USER]: [] } });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
    expect(fetchMock).not.toHaveBeenCalled();
    expect(db.updates).toHaveLength(0);
  ...
  it('non-http kind soft skipped', async () => {
    const db = createPushDb({
      pushers: {
        [PUSH_USER]: [{ pushkey: 'email', kind: 'email', app_id: 'a', data: '{}' }],
      },
    });
    await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
    expect(fetchMock).not.toHaveBeenCalled();
  ...
  it('event_id_only soft payload shape', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    const ev = baseEvent({ event_id: '$r5:example.com' });
    await sendPushNotification(db, PUSH_USER, ev as any, { unread: 2 });
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification?.event_id).toBe('$r5:example.com');
    expect(body.notification?.counts?.unread).toBe(2);
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-5 after #151', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('evaluatePushRules soft disabled override ignored', async () => {
    const db = pushRulesDb([
      {
        kind: 'override',
        rule_id: '.m.rule.suppress',
  ...
  it('evaluatePushRules soft enabled dont_notify', async () => {
    const db = pushRulesDb([
      {
        kind: 'override',
        rule_id: '.m.rule.suppress',
  ...
  it('queueNotification soft serializes actions', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r5:example.com', '$e5:example.com', 'm.room.message', [
      'notify',
  ...
  it('notifyRoomMembers soft no members no queue', async () => {
    const db = createPushDb({
      members: [],
      memberCount: 1,
      pushRules: [
  ...
  it('notifyRoomMembers soft queues with room display name', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 2,
      senderDisplayName: 'Alice R5',
      roomNameContent: JSON.stringify({ name: 'Room R5' }),
      pushRules: [
        {
          kind: 'underride',
          rule_id: '.m.rule.message',
  ...
  it('queueNotification soft throw surfaces', async () => {
    const db = createPushDb({ queueThrow: true });
    await expect(
      queueNotification(db, PUSH_USER, '!r:example.com', '$e:example.com', 'm.room.message', ['notify'])
    ).rejects.toThrow(/queue fail/);
...
describe('oauth identity leftovers account-data soft reliability round-5 after #151', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@x:example.com":["!r"]}' }],
    });
    const rows = await getGlobalAccountData(db, USER);
    expect(rows).toEqual([{ type: 'm.direct', content: { '@x:example.com': ['!r'] } }]);
  });

  it('global soft null content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.tag', content: null }],
    });
    const rows = await getGlobalAccountData(db, USER);
  ...
  it('room soft returns tags', async () => {
    const roomId = '!r5:example.com';
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: roomId, event_type: 'm.tag', content: '{"tags":{"m.favourite":{}}}' }],
    });
    expect(await getRoomAccountData(db, USER, roomId)).toEqual([
      { type: 'm.tag', content: { tags: { 'm.favourite': {} } } },
    ]);
  });

  it('all rooms soft multi room map', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a5:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b5:example.com', event_type: 'm.tag', content: '{"t":2}' },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a5:example.com', '!b5:example.com']);
    expect(all['!a5:example.com']).toEqual([{ type: 'm.tag', content: { t: 1 } }]);
    expect(all['!b5:example.com']).toEqual([{ type: 'm.tag', content: { t: 2 } }]);
  });

  it('stream position soft null defaults zero-ish', async () => {
    const pos = await getAccountDataStreamPosition(createAccountDataDb({ streamPosition: null }));
    expect(pos === 0 || pos === null || typeof pos === 'number').toBe(true);
  });

  it('throwOnAll soft propagates global', async () => {
    await expect(getGlobalAccountData(createAccountDataDb({ throwOnAll: true }), USER)).rejects.toThrow(
      /all boom/
    );
  });

  it('E2EE DO soft missing USER_KEYS', async () => {
    await expect(getE2EEAccountDataFromDO({} as any, USER)).rejects.toThrow();
  });

  it('E2EE DO soft single key success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        ['m.megolm_backup.v1', new Response(JSON.stringify({ version: 1 }), { status: 200 })],
      ]),
    });
    const data = await getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER, 'm.megolm_backup.v1');
...
describe('oauth identity leftovers identity soft reliability round-5 after #151', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('hash_details soft creates pepper', async () => {
    const cache = mockKv();
    const { status, body } = await identityRequest(`${ID_BASE}/hash_details`, {}, makeIdentityEnv({ cache }));
    expect(status).toBe(200);
    expect(body.algorithms).toEqual(['sha256', 'none']);
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r5' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r5', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup none soft email medium', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r5' });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'a@example.com', mxid: USER }],
    });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r5', addresses: ['a@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings['a@example.com email']).toBe(USER);
  ...
  it('lookup sha256 soft maps hashed address', async () => {
    const pepper = 'pep-r5h';
    const cache = mockKv({ 'identity:pepper': pepper });
    const addr = 'b@example.com email';
    const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`${addr} ${pepper}`));
    const hashed = base64UrlEncode(new Uint8Array(digest));
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'b@example.com', mxid: USER }],
    });
    // Soft: sha256 path hashes associations server-side; client sends hashes
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'sha256', pepper, addresses: [hashed] }),
      makeIdentityEnv({ cache, db })
    );
    expect([200, 400]).toContain(status);
    if (status === 200) {
      expect(body.mappings[hashed] === USER || typeof body.mappings === 'object').toBe(true);
    }
  });
  ...
  it('requestToken soft missing client_secret', async () => {
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'x@example.com' })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_MISSING_PARAM');
  });

  it('requestToken soft creates session', async () => {
    const db = createIdentityDb();
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'new@example.com', client_secret: 'cs-r5', send_attempt: 1 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(typeof body.sid).toBe('string');
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r5exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r5wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(['M_INVALID_TOKEN', 'M_NO_VALID_SESSION', 'M_UNKNOWN'].some((e) => e === body.errcode) || body.errcode).toBeTruthy();
  });

  it('submitToken soft success', async () => {
    const db = createIdentityDb({
      emailSessions: new Map([
        [
          'sid-r5ok',
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r5ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r5ok')?.validated).toBe(1);
  });

  it('account soft missing token', async () => {
    const { status, body } = await identityRequest(`${ID_BASE}/account`);
    expect(status).toBe(401);
    expect(body.errcode).toBe('M_MISSING_TOKEN');
  });

  it('lookup soft throwOnFirst surfaces', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r5' });
    const db = createIdentityDb({ throwOnFirst: true });
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r5', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-5 after #151', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r5-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 4; i++) {
    it(`base64Url soft-cap r5-${i}`, () => {
      const u8 = new Uint8Array(Array.from({ length: i + 2 }, (_, j) => (i * 17 + j * 11) % 256));
      const enc = base64UrlEncode(u8);
      expect(enc).not.toMatch(/[+/=]/);
      expect(Array.from(base64UrlDecode(enc))).toEqual(Array.from(u8));
    });
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r5-${i}`, () => {
      const obj: any = { a: { b: [{ v: i }, undefined] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i);
      expect(getNestedValue(obj, 'a.b.1')).toBeUndefined();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r5-${i}`, async () => {
      const h = await hashClientSecret(`r5-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r5-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r5-${i}`, () => {
      expect(escapeHtml(`<r5-${i}>&`)).toBe(`&lt;r5-${i}&gt;&amp;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r5-${i}`, () => {
      expect(generateUiaSuccessPage(`r5-${i}`, SERVER)).toContain(`r5-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-6 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v6a', 'v6a', 'plain', true],
    ['v6b', 'v6c', 'plain', false],
    ['equal', 'equal', 'plain', true],
    ['p', 'q', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r6 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round6-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-zzzzzzzzzzzzzzzzzzzz', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z6', 'z6', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r6-s256-case-cccccccccccccccccccccc';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z6', 'z6', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r6', '<img onerror=1>']) {
    it(`escapeHtml soft r6 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R6', 'auth-r6-id', SERVER, 'bad credentials');
    expect(html).toContain('auth-r6-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('bad credentials');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r6-a');
    const b = await hashClientSecret('r6-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r6-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r6', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T6', 'msg-r6', SERVER)).toContain('msg-r6');
    expect(generateUiaApprovalPage('sid6', USER, 'Title6', 'Desc6', SERVER, 'extra6')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-6 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft missing redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_name: 'No RU R6' }),
    });
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_client_metadata');
  });
  ...
  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing_r6&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r6_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r6_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft stores PKCE challenge', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_pkce');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r6_pkce&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&code_challenge=chal6&code_challenge_method=S256`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
    ...
    expect(keys.length).toBeGreaterThanOrEqual(1);
    const stored = JSON.parse(sessions.data[keys[0]]);
    expect(stored.code_challenge).toBe('chal6');
    expect(stored.code_challenge_method).toBe('S256');
  });
  ...
  it('authorize POST soft deactivated user', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_deact');
    sessions.data['oauth_auth_request:rid-deact6'] = JSON.stringify({
      client_id: 'cli_r6_deact',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
      code_challenge: null,
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'rid-deact6');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
  ...
  it('authorize POST soft success with state', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_okp');
    sessions.data['oauth_auth_request:rid-r6ok'] = JSON.stringify({
      client_id: 'cli_r6_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r6',
      code_challenge: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:pw6' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw6');
    fd.set('auth_request_id', 'rid-r6ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc.startsWith(REDIRECT)).toBe(true);
    expect(loc).toMatch(/code=/);
    expect(loc).toContain('state=st-r6');
  });
});

describe('oauth identity leftovers oauth token soft edges round-6 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported grant_type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r6_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'client_credentials', client_id: 'cli_r6_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft expired auth code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_exp');
    sessions.data['oauth_code:exp6'] = JSON.stringify({
      code: 'exp6',
      client_id: 'cli_r6_exp',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_grant');
    expect(sessions.deletes).toContain('oauth_code:exp6');
  });
  ...
  it('token soft redirect_uri mismatch', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_ru');
    sessions.data['oauth_code:ru6'] = JSON.stringify({
      code: 'ru6',
      client_id: 'cli_r6_ru',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft client_id mismatch on code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_a');
    seedClient(cache, 'cli_r6_b');
    sessions.data['oauth_code:mis6'] = JSON.stringify({
      code: 'mis6',
      client_id: 'cli_r6_a',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE missing verifier', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_pk');
    sessions.data['oauth_code:pk6miss'] = JSON.stringify({
      code: 'pk6miss',
      client_id: 'cli_r6_pk',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE plain success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_pkok');
    sessions.data['oauth_code:pk6'] = JSON.stringify({
      code: 'pk6',
      client_id: 'cli_r6_pkok',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r6-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk6',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r6-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
    ...
    const j = await oauthJson(res);
    expect(typeof j.access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_code:pk6');
  });

  it('token soft PKCE plain mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_pkbad');
    sessions.data['oauth_code:pk6b'] = JSON.stringify({
      code: 'pk6b',
      client_id: 'cli_r6_pkbad',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'expected',
  ...
  it('refresh soft past expires_at still rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_rfexp');
    sessions.data['oauth_refresh:rf6exp'] = JSON.stringify({
      token_id: 'tid6',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r6_rfexp',
      user_id: USER,
      device_id: 'DEV6',
      scope: 'openid',
      created_at: NOW - DAY_MS,
    ...
    expect(res.status).toBe(200);
    expect(typeof (await oauthJson(res)).access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf6exp');
  });

  it('refresh soft success rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r6_rfok');
    sessions.data['oauth_refresh:rf6ok'] = JSON.stringify({
      token_id: 'tid6ok',
      access_token_hash: 'h6',
      refresh_token_hash: 'rh6',
      client_id: 'cli_r6_rfok',
      user_id: USER,
      device_id: 'DEV6OK',
      scope: 'openid',
      created_at: NOW,
    ...
    expect(typeof j.access_token).toBe('string');
    expect(typeof j.refresh_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf6ok');
  });
});

describe('oauth identity leftovers oauth revoke introspect uia soft round-6 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft json deletes access hint', async () => {
    const sessions = mockKv();
    sessions.data['oauth_access:acc-r6'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'acc-r6', token_type_hint: 'access_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('revoke soft form deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:rev-r6'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'rev-r6', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:rev-r6');
  });
  ...
  it('introspect soft inactive unknown opaque', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'unknown-opaque-r6' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('introspect soft active opaque device', async () => {
    const token = 'opaque-active-r6';
    const hash = await sha256(token);
    const db = createOAuthDb({
      tokensByHash: new Map([[hash, { user_id: USER, device_id: 'D6' }]]),
    });
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('UIA GET soft approval page', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r6'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r6&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA POST soft approve success', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-ok6'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const fd = new FormData();
    fd.set('session', 'uia-ok6');
    fd.set('action', 'approve');
    const res = await oauthRequest(
      '/oauth/authorize/uia',
      { method: 'POST', body: fd },
...
describe('oauth identity leftovers push condition soft flood round-6 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Frank and Grace', msgtype: 'm.text', tags: ['x', 'y'], n: 7, flag: false },
  ...
  it('event_match soft sender glob', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'sender', pattern: '@frank:*' }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_match', key: 'sender', pattern: '@grace:*' }, event, USER, 2)
    ).toBe(false);
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r6 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
  ...
  it('contains_display_name soft substring miss', () => {
    expect(
      matchesCondition(
        { kind: 'contains_display_name' },
        baseEvent({ content: { body: 'franklin', msgtype: 'm.text' } }),
        USER,
        2,
        'zoe'
      )
  ...
  it('contains_display_name soft hit in body', () => {
    expect(
      matchesCondition({ kind: 'contains_display_name' }, event, USER, 2, 'Frank')
    ).toBe(true);
  });
  ...
  it('event_property_contains soft hit tag', () => {
    expect(
      matchesCondition(
        { kind: 'event_property_contains', key: 'content.tags', value: 'x' },
        event,
        USER,
        2
      )
  ...
  it('unknown condition kind soft true', () => {
    expect(matchesCondition({ kind: 'not_a_real_kind_r6' } as any, event, USER, 2)).toBe(true);
  });
...
describe('oauth identity leftovers push gateway soft reliability round-6 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  for (const status of [500, 502, 503]) {
    it(`gateway soft non-2xx status ${status}`, async () => {
      fetchMock.mockResolvedValue(new Response('err', { status }));
      const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
      await sendPushNotification(db, PUSH_USER, baseEvent() as any, { unread: 1 });
      expect(db.updates.some((u) => u.kind === 'failure')).toBe(true);
  ...
  it('event_id_only soft payload shape', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    const ev = baseEvent({ event_id: '$r6:example.com' });
    await sendPushNotification(db, PUSH_USER, ev as any, { unread: 5 });
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification?.event_id).toBe('$r6:example.com');
...
describe('oauth identity leftovers push evaluate queue soft flood round-6 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('queueNotification soft serializes actions', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r6:example.com', '$e6:example.com', 'm.room.message', [
      'notify',
      { set_tweak: 'sound', value: 'default' },
    ]);
    expect(db.queued).toHaveLength(1);
  ...
  it('notifyRoomMembers soft queues with room display name', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 2,
      senderDisplayName: 'Alice R6',
      roomNameContent: JSON.stringify({ name: 'Room R6' }),
      pushRules: [
        {
          kind: 'underride',
          rule_id: '.m.rule.message',
...
describe('oauth identity leftovers account-data soft reliability round-6 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@y:example.com":["!r6"]}' }],
    });
    const rows = await getGlobalAccountData(db, USER);
    expect(rows).toEqual([{ type: 'm.direct', content: { '@y:example.com': ['!r6'] } }]);
  });
  ...
  it('all rooms soft multi room map', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a6:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b6:example.com', event_type: 'm.tag', content: '{"t":2}' },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a6:example.com', '!b6:example.com']);
    expect(all['!a6:example.com']).toEqual([{ type: 'm.tag', content: { t: 1 } }]);
    expect(all['!b6:example.com']).toEqual([{ type: 'm.tag', content: { t: 2 } }]);
  });
  ...
  it('E2EE DO soft single key success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        ['m.megolm_backup.v1', new Response(JSON.stringify({ version: 6 }), { status: 200 })],
      ]),
    });
    const data = await getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER, 'm.megolm_backup.v1');
    expect(data).toEqual({ version: 6 });
  });
...
describe('oauth identity leftovers identity soft reliability round-6 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r6' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r6', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft wrong pepper', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'wrong', addresses: ['a@example.com email'] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(['M_INVALID_PEPPER', 'M_INVALID_PARAM'].some((e) => e === body.errcode)).toBe(true);
  });

  it('lookup soft none algorithm hit', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r6' });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'hit@example.com', mxid: USER }],
    });
    const addr = 'hit@example.com email';
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r6', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  });

  it('lookup soft sha256 path', async () => {
    const pepper = 'pep-r6sha';
    const cache = mockKv({ 'identity:pepper': pepper });
    const address = 'b@example.com';
    const medium = 'email';
    const hashed = await sha256(`${address} ${medium} ${pepper}`);
    const db = createIdentityDb({
      associations: [{ medium, address, mxid: USER }],
    });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'sha256', pepper, addresses: [hashed] }),
  ...
  it('requestToken soft creates session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'new6@example.com', client_secret: 'cs-r6', send_attempt: 1 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(typeof body.sid).toBe('string');
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r6exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r6wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
      ['M_INVALID_TOKEN', 'M_NO_VALID_SESSION', 'M_UNKNOWN'].some((e) => e === body.errcode) || body.errcode
    ).toBeTruthy();
  });
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r6ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r6ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r6', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-6 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r6-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 4; i++) {
    it(`base64Url soft-cap r6-${i}`, () => {
      const u8 = new Uint8Array(Array.from({ length: i + 2 }, (_, j) => (i * 19 + j * 13) % 256));
      const enc = base64UrlEncode(u8);
      expect(enc).not.toMatch(/[+/=]/);
      expect(Array.from(base64UrlDecode(enc))).toEqual(Array.from(u8));
    });
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r6-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 2 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 2);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r6-${i}`, async () => {
      const h = await hashClientSecret(`r6-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r6-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r6-${i}`, () => {
      expect(escapeHtml(`<r6-${i}>&"`)).toBe(`&lt;r6-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r6-${i}`, () => {
      expect(generateUiaSuccessPage(`r6-${i}`, SERVER)).toContain(`r6-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
});
describe('oauth identity leftovers oauth helpers soft reliability round-7 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v7a', 'v7a', 'plain', true],
    ['v7b', 'v7c', 'plain', false],
    ['same7', 'same7', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r7 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round7-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z7', 'z7', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r7-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z7', 'z7', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r7', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r7 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R7', 'auth-r7-id', SERVER, 'login failed');
    expect(html).toContain('auth-r7-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r7-a');
    const b = await hashClientSecret('r7-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r7-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r7', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T7', 'msg-r7', SERVER)).toContain('msg-r7');
    expect(generateUiaApprovalPage('sid7', USER, 'Title7', 'Desc7', SERVER, 'extra7')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-7 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft missing redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_name: 'No RU R7' }),
    });
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_client_metadata');
  });
  ...
  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing_r7&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r7_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r7_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft stores PKCE challenge', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_pkce');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r7_pkce&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&code_challenge=chal7&code_challenge_method=S256`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
    ...
    expect(keys.length).toBeGreaterThanOrEqual(1);
    const stored = JSON.parse(sessions.data[keys[0]]);
    expect(stored.code_challenge).toBe('chal7');
    expect(stored.code_challenge_method).toBe('S256');
  });
  ...
  it('authorize POST soft deactivated user', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_deact');
    sessions.data['oauth_auth_request:rid-deact7'] = JSON.stringify({
      client_id: 'cli_r7_deact',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
      code_challenge: null,
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'rid-deact7');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
  ...
  it('authorize POST soft success with state', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_okp');
    sessions.data['oauth_auth_request:rid-r7ok'] = JSON.stringify({
      client_id: 'cli_r7_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r7',
      code_challenge: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:pw7' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw7');
    fd.set('auth_request_id', 'rid-r7ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc.startsWith(REDIRECT)).toBe(true);
    expect(loc).toMatch(/code=/);
    expect(loc).toContain('state=st-r7');
  });
});

describe('oauth identity leftovers oauth token soft edges round-7 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported grant_type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r7_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'password', client_id: 'cli_r7_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft expired auth code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_exp');
    sessions.data['oauth_code:exp7'] = JSON.stringify({
      code: 'exp7',
      client_id: 'cli_r7_exp',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_grant');
    expect(sessions.deletes).toContain('oauth_code:exp7');
  });
  ...
  it('token soft redirect_uri mismatch', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_ru');
    sessions.data['oauth_code:ru7'] = JSON.stringify({
      code: 'ru7',
      client_id: 'cli_r7_ru',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft client_id mismatch on code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_a');
    seedClient(cache, 'cli_r7_b');
    sessions.data['oauth_code:mis7'] = JSON.stringify({
      code: 'mis7',
      client_id: 'cli_r7_a',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE missing verifier', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_pk');
    sessions.data['oauth_code:pk7miss'] = JSON.stringify({
      code: 'pk7miss',
      client_id: 'cli_r7_pk',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE plain success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_pkok');
    sessions.data['oauth_code:pk7'] = JSON.stringify({
      code: 'pk7',
      client_id: 'cli_r7_pkok',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r7-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk7',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r7-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
    ...
    const j = await oauthJson(res);
    expect(typeof j.access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_code:pk7');
  });

  it('token soft PKCE plain mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_pkbad');
    sessions.data['oauth_code:pk7b'] = JSON.stringify({
      code: 'pk7b',
      client_id: 'cli_r7_pkbad',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'expected',
  ...
  it('refresh soft past expires_at still rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_rfexp');
    sessions.data['oauth_refresh:rf7exp'] = JSON.stringify({
      token_id: 'tid7',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r7_rfexp',
      user_id: USER,
      device_id: 'DEV7',
      scope: 'openid',
      created_at: NOW - DAY_MS,
    ...
    expect(res.status).toBe(200);
    expect(typeof (await oauthJson(res)).access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf7exp');
  });

  it('refresh soft success rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r7_rfok');
    sessions.data['oauth_refresh:rf7ok'] = JSON.stringify({
      token_id: 'tid7ok',
      access_token_hash: 'h7',
      refresh_token_hash: 'rh7',
      client_id: 'cli_r7_rfok',
      user_id: USER,
      device_id: 'DEV7OK',
      scope: 'openid',
      created_at: NOW,
    ...
    expect(typeof j.access_token).toBe('string');
    expect(typeof j.refresh_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf7ok');
  });
});

describe('oauth identity leftovers oauth revoke introspect uia soft round-7 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft json deletes access hint', async () => {
    const sessions = mockKv();
    sessions.data['oauth_access:acc-r7'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'acc-r7', token_type_hint: 'access_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('revoke soft form deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:rev-r7'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'rev-r7', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:rev-r7');
  });
  ...
  it('introspect soft inactive unknown opaque', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'unknown-opaque-r7' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('introspect soft active opaque device', async () => {
    const token = 'opaque-active-r7';
    const hash = await sha256(token);
    const db = createOAuthDb({
      tokensByHash: new Map([[hash, { user_id: USER, device_id: 'D7' }]]),
    });
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('introspect soft JWT expired inactive', async () => {
    const header = base64UrlEncode(new TextEncoder().encode(JSON.stringify({ alg: 'none' })));
    const payload = base64UrlEncode(
      new TextEncoder().encode(JSON.stringify({ sub: USER, exp: Math.floor(NOW / 1000) - 20 }))
    );
    const token = `${header}.${payload}.sig`;
    const res = await oauthRequest(
      '/oauth/introspect',
  ...
  it('UIA GET soft approval page', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r7'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r7&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA POST soft approve success', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-ok7'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const fd = new FormData();
    fd.set('session', 'uia-ok7');
    fd.set('action', 'approve');
    const res = await oauthRequest(
      '/oauth/authorize/uia',
      { method: 'POST', body: fd },
...
describe('oauth identity leftovers push condition soft flood round-7 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  it('event_match soft sender glob', () => {
    expect(
      matchesCondition({ kind: 'event_match', key: 'sender', pattern: '@helen:*' }, event, USER, 2)
    ).toBe(true);
    expect(
      matchesCondition({ kind: 'event_match', key: 'sender', pattern: '@ivan:*' }, event, USER, 2)
    ).toBe(false);
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r7 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
  ...
  it('contains_display_name soft substring miss', () => {
    expect(
      matchesCondition(
        { kind: 'contains_display_name' },
        baseEvent({ content: { body: 'helena', msgtype: 'm.text' } }),
        USER,
        2,
        'zoe'
      )
  ...
  it('contains_display_name soft hit in body', () => {
    expect(
      matchesCondition({ kind: 'contains_display_name' }, event, USER, 2, 'Helen')
    ).toBe(true);
  });
  ...
  it('event_property_contains soft hit tag', () => {
    expect(
      matchesCondition(
        { kind: 'event_property_contains', key: 'content.tags', value: 'p' },
        event,
        USER,
        2
      )
  ...
  it('unknown condition kind soft true', () => {
    expect(matchesCondition({ kind: 'not_a_real_kind_r7' } as any, event, USER, 2)).toBe(true);
  });
...
describe('oauth identity leftovers push gateway soft reliability round-7 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  it('event_id_only soft payload shape', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    const ev = baseEvent({ event_id: '$r7:example.com' });
    await sendPushNotification(db, PUSH_USER, ev as any, { unread: 8 });
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification?.event_id).toBe('$r7:example.com');
    expect(body.notification?.counts?.unread).toBe(8);
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-7 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('queueNotification soft serializes actions', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r7:example.com', '$e7:example.com', 'm.room.message', [
      'notify',
      { set_tweak: 'sound', value: 'default' },
    ]);
    expect(db.queued).toHaveLength(1);
  ...
  it('notifyRoomMembers soft queues with room display name', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 2,
      senderDisplayName: 'Alice R7',
      roomNameContent: JSON.stringify({ name: 'Room R7' }),
      pushRules: [
        {
          kind: 'underride',
          rule_id: '.m.rule.message',
...
describe('oauth identity leftovers account-data soft reliability round-7 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r7"]}' }],
    });
    const rows = await getGlobalAccountData(db, USER);
    expect(rows).toEqual([{ type: 'm.direct', content: { '@z:example.com': ['!r7'] } }]);
  });
  ...
  it('all rooms soft multi room map', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a7:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b7:example.com', event_type: 'm.tag', content: '{"t":2}' },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a7:example.com', '!b7:example.com']);
    expect(all['!a7:example.com']).toEqual([{ type: 'm.tag', content: { t: 1 } }]);
    expect(all['!b7:example.com']).toEqual([{ type: 'm.tag', content: { t: 2 } }]);
  });
  ...
  it('E2EE DO soft single key success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        ['m.megolm_backup.v1', new Response(JSON.stringify({ version: 7 }), { status: 200 })],
      ]),
    });
    const data = await getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER, 'm.megolm_backup.v1');
...
describe('oauth identity leftovers identity soft reliability round-7 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r7' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r7', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r7' });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'hit7@example.com', mxid: USER }],
    });
    const addr = 'hit7@example.com email';
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r7', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('lookup soft sha256 path', async () => {
    const pepper = 'pep-r7sha';
    const cache = mockKv({ 'identity:pepper': pepper });
    const address = 'c@example.com';
    const medium = 'email';
    const hashed = await sha256(`${address} ${medium} ${pepper}`);
    const db = createIdentityDb({
      associations: [{ medium, address, mxid: USER }],
  ...
  it('requestToken soft creates session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'new7@example.com', client_secret: 'cs-r7', send_attempt: 1 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(typeof body.sid).toBe('string');
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r7exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r7wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r7ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r7ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r7', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-7 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r7-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 4; i++) {
    it(`base64Url soft-cap r7-${i}`, () => {
      const u8 = new Uint8Array(Array.from({ length: i + 2 }, (_, j) => (i * 23 + j * 17) % 256));
      const enc = base64UrlEncode(u8);
      expect(enc).not.toMatch(/[+/=]/);
      expect(Array.from(base64UrlDecode(enc))).toEqual(Array.from(u8));
    });
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r7-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
    });
  }
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r7-${i}`, async () => {
      const h = await hashClientSecret(`r7-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r7-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r7-${i}`, () => {
      expect(escapeHtml(`<r7-${i}>&"`)).toBe(`&lt;r7-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r7-${i}`, () => {
      expect(generateUiaSuccessPage(`r7-${i}`, SERVER)).toContain(`r7-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-8 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v8a', 'v8a', 'plain', true],
    ['v8b', 'v8c', 'plain', false],
    ['same8', 'same8', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r8 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round8-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z8', 'z8', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r8-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z8', 'z8', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r8', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r8 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R8', 'auth-r8-id', SERVER, 'login failed');
    expect(html).toContain('auth-r8-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r8-a');
    const b = await hashClientSecret('r8-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r8-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r8', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T8', 'msg-r8', SERVER)).toContain('msg-r8');
    expect(generateUiaApprovalPage('sid8', USER, 'Title8', 'Desc8', SERVER, 'extra8')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-8 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft missing redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_name: 'No RU R8' }),
    });
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_client_metadata');
  });
  ...
  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing_r8&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r8_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r8_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft stores PKCE challenge', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_pkce');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r8_pkce&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&code_challenge=chal8&code_challenge_method=S256`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
    ...
    expect(keys.length).toBeGreaterThanOrEqual(1);
    const stored = JSON.parse(sessions.data[keys[0]]);
    expect(stored.code_challenge).toBe('chal8');
    expect(stored.code_challenge_method).toBe('S256');
  });
  ...
  it('authorize POST soft deactivated user', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_deact');
    sessions.data['oauth_auth_request:rid-deact8'] = JSON.stringify({
      client_id: 'cli_r8_deact',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
      code_challenge: null,
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw');
    fd.set('auth_request_id', 'rid-deact8');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
  ...
  it('authorize POST soft success with state', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_okp');
    sessions.data['oauth_auth_request:rid-r8ok'] = JSON.stringify({
      client_id: 'cli_r8_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r8',
      code_challenge: null,
    ...
    const db = createOAuthDb({
      users: new Map([[USER, { user_id: USER, password_hash: 'mockok:pw8' }]]),
    });
    const fd = new FormData();
    fd.set('username', 'alice');
    fd.set('password', 'pw8');
    fd.set('auth_request_id', 'rid-r8ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc.startsWith(REDIRECT)).toBe(true);
    expect(loc).toMatch(/code=/);
    expect(loc).toContain('state=st-r8');
  });
});

describe('oauth identity leftovers oauth token soft edges round-8 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported grant_type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r8_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'password', client_id: 'cli_r8_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft expired auth code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_exp');
    sessions.data['oauth_code:exp8'] = JSON.stringify({
      code: 'exp8',
      client_id: 'cli_r8_exp',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
    ...
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_grant');
    expect(sessions.deletes).toContain('oauth_code:exp8');
  });
  ...
  it('token soft redirect_uri mismatch', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_ru');
    sessions.data['oauth_code:ru8'] = JSON.stringify({
      code: 'ru8',
      client_id: 'cli_r8_ru',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft client_id mismatch on code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_a');
    seedClient(cache, 'cli_r8_b');
    sessions.data['oauth_code:mis8'] = JSON.stringify({
      code: 'mis8',
      client_id: 'cli_r8_a',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE missing verifier', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_pk');
    sessions.data['oauth_code:pk8miss'] = JSON.stringify({
      code: 'pk8miss',
      client_id: 'cli_r8_pk',
      user_id: USER,
      redirect_uri: REDIRECT,
  ...
  it('token soft PKCE plain success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_pkok');
    sessions.data['oauth_code:pk8'] = JSON.stringify({
      code: 'pk8',
      client_id: 'cli_r8_pkok',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r8-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk8',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r8-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
    ...
    const j = await oauthJson(res);
    expect(typeof j.access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_code:pk8');
  });

  it('token soft PKCE plain mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_pkbad');
    sessions.data['oauth_code:pk8b'] = JSON.stringify({
      code: 'pk8b',
      client_id: 'cli_r8_pkbad',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'expected',
  ...
  it('refresh soft past expires_at still rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_rfexp');
    sessions.data['oauth_refresh:rf8exp'] = JSON.stringify({
      token_id: 'tid8',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r8_rfexp',
      user_id: USER,
      device_id: 'DEV8',
      scope: 'openid',
      created_at: NOW - DAY_MS,
    ...
    expect(res.status).toBe(200);
    expect(typeof (await oauthJson(res)).access_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf8exp');
  });

  it('refresh soft success rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r8_rfok');
    sessions.data['oauth_refresh:rf8ok'] = JSON.stringify({
      token_id: 'tid8ok',
      access_token_hash: 'h8',
      refresh_token_hash: 'rh8',
      client_id: 'cli_r8_rfok',
      user_id: USER,
      device_id: 'DEV8OK',
      scope: 'openid',
      created_at: NOW,
    ...
    expect(typeof j.access_token).toBe('string');
    expect(typeof j.refresh_token).toBe('string');
    expect(sessions.deletes).toContain('oauth_refresh:rf8ok');
  });
});

describe('oauth identity leftovers oauth revoke introspect uia soft round-8 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft json deletes access hint', async () => {
    const sessions = mockKv();
    sessions.data['oauth_access:acc-r8'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'acc-r8', token_type_hint: 'access_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('revoke soft form deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:rev-r8'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'rev-r8', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:rev-r8');
  });
  ...
  it('introspect soft inactive unknown opaque', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'unknown-opaque-r8' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('introspect soft active opaque device', async () => {
    const token = 'opaque-active-r8';
    const hash = await sha256(token);
    const db = createOAuthDb({
      tokensByHash: new Map([[hash, { user_id: USER, device_id: 'D8' }]]),
    });
    const res = await oauthRequest(
      '/oauth/introspect',
      {
  ...
  it('UIA GET soft approval page', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r8'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r8&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA POST soft approve success', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-ok8'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const fd = new FormData();
    fd.set('session', 'uia-ok8');
    fd.set('action', 'approve');
    const res = await oauthRequest(
      '/oauth/authorize/uia',
      { method: 'POST', body: fd },
...
describe('oauth identity leftovers push condition soft flood round-8 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r8 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
  ...
  it('unknown condition kind soft true', () => {
    expect(matchesCondition({ kind: 'not_a_real_kind_r8' } as any, event, USER, 2)).toBe(true);
  });
...
describe('oauth identity leftovers push gateway soft reliability round-8 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  it('event_id_only soft payload shape', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    const ev = baseEvent({ event_id: '$r8:example.com' });
    await sendPushNotification(db, PUSH_USER, ev as any, { unread: 9 });
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification?.event_id).toBe('$r8:example.com');
    expect(body.notification?.counts?.unread).toBe(9);
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-8 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('queueNotification soft serializes actions', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r8:example.com', '$e8:example.com', 'm.room.message', [
      'notify',
      { set_tweak: 'sound', value: 'default' },
    ]);
    expect(db.queued).toHaveLength(1);
  ...
  it('notifyRoomMembers soft queues with room display name', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 2,
      senderDisplayName: 'Alice R8',
      roomNameContent: JSON.stringify({ name: 'Room R8' }),
      pushRules: [
        {
          kind: 'underride',
          rule_id: '.m.rule.message',
...
describe('oauth identity leftovers account-data soft reliability round-8 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r8"]}' }],
    });
    const rows = await getGlobalAccountData(db, USER);
    expect(rows).toEqual([{ type: 'm.direct', content: { '@z:example.com': ['!r8'] } }]);
  });
  ...
  it('all rooms soft multi room map', async () => {
    const db = createAccountDataDb({
      rows: [
        { user_id: USER, room_id: '!a8:example.com', event_type: 'm.tag', content: '{"t":1}' },
        { user_id: USER, room_id: '!b8:example.com', event_type: 'm.tag', content: '{"t":2}' },
      ],
    });
    const all = await getAllRoomAccountData(db, USER, ['!a8:example.com', '!b8:example.com']);
    expect(all['!a8:example.com']).toEqual([{ type: 'm.tag', content: { t: 1 } }]);
    expect(all['!b8:example.com']).toEqual([{ type: 'm.tag', content: { t: 2 } }]);
  });
  ...
  it('E2EE DO soft single key success', async () => {
    const ns = mockUserKeysNamespace({
      responses: new Map([
        ['m.megolm_backup.v1', new Response(JSON.stringify({ version: 8 }), { status: 200 })],
      ]),
    });
    const data = await getE2EEAccountDataFromDO({ USER_KEYS: ns } as any, USER, 'm.megolm_backup.v1');
    expect(data).toEqual({ version: 8 });
  });
...
describe('oauth identity leftovers identity soft reliability round-8 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r8' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r8', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r8' });
    const db = createIdentityDb({
      associations: [{ medium: 'email', address: 'hit8@example.com', mxid: USER }],
    });
    const addr = 'hit8@example.com email';
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r8', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('requestToken soft creates session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'new8@example.com', client_secret: 'cs-r8', send_attempt: 1 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(typeof body.sid).toBe('string');
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r8exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r8wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r8ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r8ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r8', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-8 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r8-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r8-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r8-${i}`, async () => {
      const h = await hashClientSecret(`r8-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r8-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r8-${i}`, () => {
      expect(escapeHtml(`<r8-${i}>&"`)).toBe(`&lt;r8-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r8-${i}`, () => {
      expect(generateUiaSuccessPage(`r8-${i}`, SERVER)).toContain(`r8-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-9 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v8a', 'v8a', 'plain', true],
    ['v8b', 'v8c', 'plain', false],
    ['same8', 'same8', 'plain', true],
    ...
    it(`verifyCodeChallenge plain soft r9 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round9-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        ...
        const v = 'r9-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z8', 'z8', method)).toBe(false);
  ...
  for (const raw of ['<>&"', "'&<>", 'plain-r9', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r9 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R9', 'auth-r9-id', SERVER, 'login failed');
    expect(html).toContain('auth-r9-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r9-a');
    const b = await hashClientSecret('r9-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r9-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r9', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T8', 'msg-r9', SERVER)).toContain('msg-r9');
    expect(generateUiaApprovalPage('sid8', USER, 'Title8', 'Desc8', SERVER, 'extra8')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-9 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('register soft missing redirect_uris', async () => {
    const res = await oauthRequest('/oauth/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ client_name: 'No RU R9' }),
    });
    expect(res.status).toBe(400);
    expect((await oauthJson(res)).error).toBe('invalid_client_metadata');
  });
  ...
  it('authorize GET soft unknown client', async () => {
    const res = await oauthRequest(
      `/oauth/authorize?client_id=missing_r9&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: mockKv() })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft redirect_uri mismatch', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r9_ru');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r9_ru&redirect_uri=${encodeURIComponent('https://evil.example/cb')}&response_type=code`,
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('authorize GET soft stores PKCE challenge', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_pkce');
    const res = await oauthRequest(
      `/oauth/authorize?client_id=cli_r9_pkce&redirect_uri=${encodeURIComponent(REDIRECT)}&response_type=code&code_challenge=chal8&code_challenge_method=S256`,
      {},
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions })
    );
    expect(res.status).toBe(200);
  ...
  it('authorize POST soft deactivated user', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_deact');
    sessions.data['oauth_auth_request:rid-deact8'] = JSON.stringify({
      client_id: 'cli_r9_deact',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: null,
      code_challenge: null,
  ...
  it('authorize POST soft success with state', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_okp');
    sessions.data['oauth_auth_request:rid-r9ok'] = JSON.stringify({
      client_id: 'cli_r9_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r9',
      code_challenge: null,
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw8');
    fd.set('auth_request_id', 'rid-r9ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
    ...
    expect(loc.startsWith(REDIRECT)).toBe(true);
    expect(loc).toMatch(/code=/);
    expect(loc).toContain('state=st-r9');
  });
});

describe('oauth identity leftovers oauth token soft edges round-9 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft unsupported grant_type', async () => {
    const cache = mockKv();
    seedClient(cache, 'cli_r9_ug');
    const res = await oauthRequest(
      '/oauth/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grant_type: 'password', client_id: 'cli_r9_ug' }),
      },
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(400);
  ...
  it('token soft expired auth code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_exp');
    sessions.data['oauth_code:exp8'] = JSON.stringify({
      code: 'exp8',
      client_id: 'cli_r9_exp',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: null,
  ...
  it('token soft client_id mismatch on code', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_a');
    seedClient(cache, 'cli_r9_b');
    sessions.data['oauth_code:mis8'] = JSON.stringify({
      code: 'mis8',
      client_id: 'cli_r9_a',
      user_id: USER,
  ...
  it('token soft PKCE plain success', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_pkok');
    sessions.data['oauth_code:pk8'] = JSON.stringify({
      code: 'pk8',
      client_id: 'cli_r9_pkok',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r9-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk8',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r9-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
  ...
  it('token soft PKCE plain mismatch', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_pkbad');
    sessions.data['oauth_code:pk8b'] = JSON.stringify({
      code: 'pk8b',
      client_id: 'cli_r9_pkbad',
      user_id: USER,
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'expected',
  ...
  it('refresh soft past expires_at still rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_rfexp');
    sessions.data['oauth_refresh:rf8exp'] = JSON.stringify({
      token_id: 'tid8',
      access_token_hash: 'h',
      refresh_token_hash: 'rh',
      client_id: 'cli_r9_rfexp',
      user_id: USER,
      device_id: 'DEV8',
      scope: 'openid',
      created_at: NOW - DAY_MS,
  ...
  it('refresh soft success rotates', async () => {
    const cache = mockKv();
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_rfok');
    sessions.data['oauth_refresh:rf8ok'] = JSON.stringify({
      token_id: 'tid8ok',
      access_token_hash: 'h8',
      refresh_token_hash: 'rh8',
      client_id: 'cli_r9_rfok',
      user_id: USER,
      device_id: 'DEV8OK',
      scope: 'openid',
      created_at: NOW,
...
describe('oauth identity leftovers oauth revoke introspect uia soft round-9 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('revoke soft json deletes access hint', async () => {
    const sessions = mockKv();
    sessions.data['oauth_access:acc-r9'] = JSON.stringify({ client_id: 'c' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'acc-r9', token_type_hint: 'access_token' }),
      },
      makeOAuthEnv({ SESSIONS: sessions })
    );
    expect([200, 204]).toContain(res.status);
  ...
  it('revoke soft form deletes refresh', async () => {
    const sessions = mockKv();
    sessions.data['oauth_refresh:rev-r9'] = JSON.stringify({ client_id: 'c' });
    const body = new URLSearchParams({ token: 'rev-r9', token_type_hint: 'refresh_token' });
    const res = await oauthRequest(
      '/oauth/revoke',
      {
        method: 'POST',
    ...
    expect([200, 204]).toContain(res.status);
    expect(sessions.deletes).toContain('oauth_refresh:rev-r9');
  });
  ...
  it('introspect soft inactive unknown opaque', async () => {
    const res = await oauthRequest(
      '/oauth/introspect',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: 'unknown-opaque-r9' }),
      },
      makeOAuthEnv({ DB: createOAuthDb() })
    );
    expect(res.status).toBe(200);
  ...
  it('UIA GET soft approval page', async () => {
    const cache = mockKv();
    cache.data['uia_session:uia-r9'] = JSON.stringify({ user_id: USER, created_at: NOW });
    const res = await oauthRequest(
      '/oauth/authorize/uia?session=uia-r9&action=org.matrix.cross_signing_reset',
      {},
      makeOAuthEnv({ CACHE: cache })
    );
    expect(res.status).toBe(200);
...
describe('oauth identity leftovers push condition soft flood round-9 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r9 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
  ...
  it('unknown condition kind soft true', () => {
    expect(matchesCondition({ kind: 'not_a_real_kind_r9' } as any, event, USER, 2)).toBe(true);
  });
...
describe('oauth identity leftovers push gateway soft reliability round-9 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
  ...
  it('event_id_only soft payload shape', async () => {
    fetchMock.mockResolvedValue(new Response('{}', { status: 200 }));
    const db = createPushDb({ pushers: { [PUSH_USER]: [httpPusher()] } });
    const ev = baseEvent({ event_id: '$r9:example.com' });
    await sendPushNotification(db, PUSH_USER, ev as any, { unread: 9 });
    const body = JSON.parse(fetchMock.mock.calls[0][1].body as string);
    expect(body.notification?.event_id).toBe('$r9:example.com');
    expect(body.notification?.counts?.unread).toBe(9);
  });
...
describe('oauth identity leftovers push evaluate queue soft flood round-9 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
  ...
  it('queueNotification soft serializes actions', async () => {
    const db = createPushDb({});
    await queueNotification(db, PUSH_USER, '!r9:example.com', '$e8:example.com', 'm.room.message', [
      'notify',
      { set_tweak: 'sound', value: 'default' },
    ]);
    expect(db.queued).toHaveLength(1);
  ...
  it('notifyRoomMembers soft queues with room display name', async () => {
    const db = createPushDb({
      members: [PUSH_USER],
      memberCount: 2,
      senderDisplayName: 'Alice R9',
      roomNameContent: JSON.stringify({ name: 'Room R9' }),
      pushRules: [
        {
          kind: 'underride',
          rule_id: '.m.rule.message',
...
describe('oauth identity leftovers account-data soft reliability round-9 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r9"]}' }],
    });
    const rows = await getGlobalAccountData(db, USER);
    expect(rows).toEqual([{ type: 'm.direct', content: { '@z:example.com': ['!r9'] } }]);
  });
...
describe('oauth identity leftovers identity soft reliability round-9 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r9' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r9', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r9', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('requestToken soft creates session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/requestToken`,
      postJson({ email: 'new8@example.com', client_secret: 'cs-r9', send_attempt: 1 }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(typeof body.sid).toBe('string');
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r9exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r9wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r9ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r9ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r9', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-9 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r9-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r9-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r9-${i}`, async () => {
      const h = await hashClientSecret(`r9-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r9-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r9-${i}`, () => {
      expect(escapeHtml(`<r9-${i}>&"`)).toBe(`&lt;r9-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r9-${i}`, () => {
      expect(generateUiaSuccessPage(`r9-${i}`, SERVER)).toContain(`r9-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-10 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v10a', 'v10a', 'plain', true],
    ['v10b', 'v10c', 'plain', false],
    ['same10', 'same10', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r10 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round10-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z10', 'z10', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r10-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z10', 'z10', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r10', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r10 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R10', 'auth-r10-id', SERVER, 'login failed');
    expect(html).toContain('auth-r10-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r10-a');
    const b = await hashClientSecret('r10-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r10-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r10', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T10', 'msg-r10', SERVER)).toContain('msg-r10');
    expect(generateUiaApprovalPage('sid10', USER, 'Title10', 'Desc10', SERVER, 'extra10')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-10 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('authorize POST soft success with state', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_okp');
    sessions.data['oauth_auth_request:rid-r10ok'] = JSON.stringify({
      client_id: 'cli_r9_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r9',
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw8');
    fd.set('auth_request_id', 'rid-r10ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
...
describe('oauth identity leftovers oauth token soft edges round-10 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft PKCE plain success', async () => {
    ...
    sessions.data['oauth_code:pk8'] = JSON.stringify({
      ...
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r10-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk8',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r10-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
...
describe('oauth identity leftovers oauth revoke introspect uia soft round-10 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
...
describe('oauth identity leftovers push condition soft flood round-10 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r10 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
...
describe('oauth identity leftovers push gateway soft reliability round-10 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
...
describe('oauth identity leftovers push evaluate queue soft flood round-10 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
...
describe('oauth identity leftovers account-data soft reliability round-10 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r9"]}' }],
    });
...
describe('oauth identity leftovers identity soft reliability round-10 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r10' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r10', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r10', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r10exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r10wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r10ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r10ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r10', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-10 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r10-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r10-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r10-${i}`, async () => {
      const h = await hashClientSecret(`r10-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r10-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r10-${i}`, () => {
      expect(escapeHtml(`<r10-${i}>&"`)).toBe(`&lt;r10-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r10-${i}`, () => {
      expect(generateUiaSuccessPage(`r10-${i}`, SERVER)).toContain(`r10-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-11 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v11a', 'v11a', 'plain', true],
    ['v11b', 'v11c', 'plain', false],
    ['same11', 'same11', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r11 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round11-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z11', 'z11', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r11-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z11', 'z11', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r11', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r11 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R11', 'auth-r11-id', SERVER, 'login failed');
    expect(html).toContain('auth-r11-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r11-a');
    const b = await hashClientSecret('r11-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r11-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r11', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T11', 'msg-r11', SERVER)).toContain('msg-r11');
    expect(generateUiaApprovalPage('sid11', USER, 'Title11', 'Desc11', SERVER, 'extra11')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-11 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('authorize POST soft success with state', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_okp');
    sessions.data['oauth_auth_request:rid-r11ok'] = JSON.stringify({
      client_id: 'cli_r9_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r9',
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw8');
    fd.set('auth_request_id', 'rid-r11ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
...
describe('oauth identity leftovers oauth token soft edges round-11 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft PKCE plain success', async () => {
    ...
    sessions.data['oauth_code:pk8'] = JSON.stringify({
      ...
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r11-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk8',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r11-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
...
describe('oauth identity leftovers oauth revoke introspect uia soft round-11 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
...
describe('oauth identity leftovers push condition soft flood round-11 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r11 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
...
describe('oauth identity leftovers push gateway soft reliability round-11 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
...
describe('oauth identity leftovers push evaluate queue soft flood round-11 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
...
describe('oauth identity leftovers account-data soft reliability round-11 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r9"]}' }],
    });
...
describe('oauth identity leftovers identity soft reliability round-11 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r11' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r11', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r11', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r11exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r11wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r11ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r11ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r11', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-11 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r11-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r11-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r11-${i}`, async () => {
      const h = await hashClientSecret(`r11-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r11-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r11-${i}`, () => {
      expect(escapeHtml(`<r11-${i}>&"`)).toBe(`&lt;r11-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r11-${i}`, () => {
      expect(generateUiaSuccessPage(`r11-${i}`, SERVER)).toContain(`r11-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
...
describe('oauth identity leftovers oauth helpers soft reliability round-12 after #160', () => {
  for (const [verifier, challenge, method, ok] of [
    ['v12a', 'v12a', 'plain', true],
    ['v12b', 'v12c', 'plain', false],
    ['same12', 'same12', 'plain', true],
    ['x', 'y', 'plain', false],
  ] as const) {
    it(`verifyCodeChallenge plain soft r12 ${JSON.stringify(verifier)}/${JSON.stringify(challenge)}→${ok}`, async () => {
      expect(await verifyCodeChallenge(verifier, challenge, method)).toBe(ok);
    });
  }

  it('verifyCodeChallenge S256 soft match and mismatch digest', async () => {
    const verifier = 'round12-pkce-verifier-abcdefghijklmnopqrstuv';
    const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(verifier));
    const challenge = base64UrlEncode(new Uint8Array(hash));
    expect(await verifyCodeChallenge(verifier, challenge, 'S256')).toBe(true);
    expect(await verifyCodeChallenge('other-verifier-yyyyyyyyyyyyyyyyyyyy', challenge, 'S256')).toBe(false);
  ...
  for (const method of ['S256', 'plain', 's256', 'PLAIN']) {
    it(`verifyCodeChallenge method soft case ${JSON.stringify(method)}`, async () => {
      if (method === 'plain') {
        expect(await verifyCodeChallenge('z12', 'z12', method)).toBe(true);
      } else if (method === 'S256') {
        const v = 'r12-s256-case-dddddddddddddddddddddd';
        const dig = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(v));
        expect(await verifyCodeChallenge(v, base64UrlEncode(new Uint8Array(dig)), method)).toBe(true);
      } else {
        expect(await verifyCodeChallenge('z12', 'z12', method)).toBe(false);
      }
    });
  }

  for (const raw of ['<>&"', "'&<>", 'plain-r12', '<script>alert(1)</script>']) {
    it(`escapeHtml soft r12 ${JSON.stringify(raw)}`, () => {
      const out = escapeHtml(raw);
      expect(out).not.toMatch(/[<>]/);
      if (raw.includes('&')) expect(out).toContain('&amp;');
      if (raw.includes('"')) expect(out).toContain('&quot;');
  ...
  it('generateLoginPage soft embeds client name and error', () => {
    const html = generateLoginPage('Client R12', 'auth-r12-id', SERVER, 'login failed');
    expect(html).toContain('auth-r12-id');
    expect(html).toContain(SERVER);
    expect(html).toContain('login failed');
  });

  it('hashClientSecret soft distinct and stable', async () => {
    const a = await hashClientSecret('r12-a');
    const b = await hashClientSecret('r12-b');
    expect(a).not.toBe(b);
    expect(await hashClientSecret('r12-a')).toBe(a);
    expect(a).toMatch(/^[A-Za-z0-9_-]+$/);
  });

  it('UIA HTML soft pages contain markers', () => {
    expect(generateUiaSuccessPage('sid-r12', SERVER)).toContain(SERVER);
    expect(generateUiaCancelledPage(SERVER)).toContain(SERVER);
    expect(generateUiaErrorPage('T12', 'msg-r12', SERVER)).toContain('msg-r12');
    expect(generateUiaApprovalPage('sid12', USER, 'Title12', 'Desc12', SERVER, 'extra12')).toContain(USER);
  });
});

describe('oauth identity leftovers oauth register authorize soft flood round-12 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('authorize POST soft success with state', async () => {
    ...
    const sessions = mockKv();
    seedClient(cache, 'cli_r9_okp');
    sessions.data['oauth_auth_request:rid-r12ok'] = JSON.stringify({
      client_id: 'cli_r9_okp',
      redirect_uri: REDIRECT,
      scope: 'openid',
      state: 'st-r9',
    ...
    fd.set('username', 'alice');
    fd.set('password', 'pw8');
    fd.set('auth_request_id', 'rid-r12ok');
    const res = await oauthRequest(
      '/oauth/authorize',
      { method: 'POST', body: fd },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: db })
...
describe('oauth identity leftovers oauth token soft edges round-12 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('token soft PKCE plain success', async () => {
    ...
    sessions.data['oauth_code:pk8'] = JSON.stringify({
      ...
      redirect_uri: REDIRECT,
      scope: 'openid',
      code_challenge: 'verifier-r12-plain',
      code_challenge_method: 'plain',
      created_at: NOW,
      expires_at: NOW + 600_000,
    });
    const res = await oauthRequest(
      '/oauth/token',
      {
        ...
        body: JSON.stringify({
          ...
          code: 'pk8',
          redirect_uri: REDIRECT,
          code_verifier: 'verifier-r12-plain',
        }),
      },
      makeOAuthEnv({ CACHE: cache, SESSIONS: sessions, DB: createOAuthDb() })
    );
...
describe('oauth identity leftovers oauth revoke introspect uia soft round-12 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
...
describe('oauth identity leftovers push condition soft flood round-12 after #160', () => {
  const event = baseEvent({
    content: { body: 'ping Helen and Ivan', msgtype: 'm.text', tags: ['p', 'q'], n: 9, flag: true },
    sender: '@helen:example.com',
  });
  ...
  for (const [is, count, ok] of [
    ...
    it(`room_member_count soft r12 is=${JSON.stringify(is)} n=${count}`, () => {
      expect(matchesCondition({ kind: 'room_member_count', is }, event, USER, count)).toBe(ok);
    });
  }
...
describe('oauth identity leftovers push gateway soft reliability round-12 after #160', () => {
  let fetchMock: ReturnType<typeof vi.fn>;
...
describe('oauth identity leftovers push evaluate queue soft flood round-12 after #160', () => {
  beforeEach(() => {
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
...
describe('oauth identity leftovers account-data soft reliability round-12 after #160', () => {
  it('global soft parses json content', async () => {
    const db = createAccountDataDb({
      rows: [{ user_id: USER, room_id: '', event_type: 'm.direct', content: '{"@z:example.com":["!r9"]}' }],
    });
...
describe('oauth identity leftovers identity soft reliability round-12 after #160', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(NOW);
  });
  ...
  it('lookup soft unsupported algorithm', async () => {
    const cache = mockKv({ 'identity:pepper': 'pep-r12' });
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'md5', pepper: 'pep-r12', addresses: [] }),
      makeIdentityEnv({ cache })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_INVALID_PARAM');
  ...
  it('lookup soft none algorithm hit', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r12', addresses: [addr] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBe(200);
    expect(body.mappings[addr]).toBe(USER);
  ...
  it('submitToken soft expired session', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r12exp', client_secret: 'cs', token: '111111' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(body.errcode).toBe('M_SESSION_EXPIRED');
  ...
  it('submitToken soft wrong token', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r12wt', client_secret: 'cs', token: '000000' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(400);
    expect(
  ...
  it('submitToken soft success', async () => {
    ...
    const { status, body } = await identityRequest(
      `${ID_BASE}/validate/email/submitToken`,
      postJson({ sid: 'sid-r12ok', client_secret: 'cs-ok', token: '333333' }),
      makeIdentityEnv({ db })
    );
    expect(status).toBe(200);
    expect(body.success).toBe(true);
    expect(db.emailSessions.get('sid-r12ok')?.validated).toBe(1);
  });
  ...
  it('lookup soft throwOnFirst surfaces', async () => {
    ...
    const { status } = await identityRequest(
      `${ID_BASE}/lookup`,
      postJson({ algorithm: 'none', pepper: 'pep-r12', addresses: ['x@example.com email'] }),
      makeIdentityEnv({ cache, db })
    );
    expect(status).toBeGreaterThanOrEqual(400);
  });
...
describe('oauth identity leftovers soft-cap flood helpers round-12 after #160', () => {
  for (let i = 0; i < 4; i++) {
    it(`generateRandomString soft-cap r12-${i}`, () => {
      const len = 4 + i * 2;
      const s = generateRandomString(len);
      expect(s).toHaveLength(len * 2);
      expect(s).toMatch(/^[0-9a-f]+$/);
  ...
  for (let i = 0; i < 3; i++) {
    it(`getNestedValue soft-cap r12-${i}`, () => {
      const obj: any = { a: { b: [{ v: i * 3 }, null] } };
      expect(getNestedValue(obj, 'a.b.0.v')).toBe(i * 3);
      expect(getNestedValue(obj, 'a.b.1')).toBeNull();
      expect(getNestedValue(obj, 'a.missing')).toBeUndefined();
  ...
  for (let i = 0; i < 3; i++) {
    it(`hashClientSecret soft-cap r12-${i}`, async () => {
      const h = await hashClientSecret(`r12-secret-${i}`);
      expect(h).toMatch(/^[A-Za-z0-9_-]+$/);
      expect(await hashClientSecret(`r12-secret-${i}`)).toBe(h);
    });
  }

  for (let i = 0; i < 3; i++) {
    it(`escapeHtml soft-cap r12-${i}`, () => {
      expect(escapeHtml(`<r12-${i}>&"`)).toBe(`&lt;r12-${i}&gt;&amp;&quot;`);
    });
  }

  for (let i = 0; i < 2; i++) {
    it(`UIA HTML soft-cap r12-${i}`, () => {
      expect(generateUiaSuccessPage(`r12-${i}`, SERVER)).toContain(`r12-${i}`);
      expect(generateUiaApprovalPage(`s${i}`, USER, `T${i}`, `D${i}`, SERVER, `e${i}`)).toContain(`e${i}`);
      expect(generateLoginPage(`n${i}`, `id${i}`, SERVER)).toContain(`id${i}`);
    });
  }
