/**
 * TOKENMAXX HEAVY leftovers after #297 quaternary / tip past #303 — *admin*
 * *quinary* soft+concurrent-race niches: exact product strings that #265/#295
 * soft-flooded by errcode only (never `.toBe` exact under soft/race):
 *   `Invalid username format` (users/create);
 *   `Username already taken` (users/create duplicate);
 *   `Missing required parameter: username and password required`;
 *   `Missing required parameter: user_ids array required`;
 *   `Missing required parameter: user_id and message required`;
 *   `Report not found` (resolve + unresolve miss);
 *   `Invalid user ID format` (synapse PUT bad MXID);
 *   `Missing required parameter: password required for new user` exact
 *     (leftovers only `.toMatch` regex).
 *
 * Gap table (why leftover after #297):
 *   create/invalid/dup/missing-param | errcode-only soft after #265
 *   bulk-delete / server-notice params | errcode-only soft after #265
 *   Report not found | status/errcode only in leftovers+routes
 *   Invalid user ID format | M_INVALID_USERNAME only
 *   password required for new user | regex match only
 *
 * Distinct from #297 quaternary forbid/deactivated/whois/keys, #279 IdP,
 * admin-mutate write TOCTOU. New file. Tests-only. example.com fixtures.
...
// ---------------------------------------------------------------------------
// Quinary soft floods + concurrent races after tip #297 / #303
// ---------------------------------------------------------------------------

describe('soft quinary admin create/report/notice exact strings after #297', () => {
  for (let i = 0; i < 12; i++) {
    it(`users/create Invalid username format exact flood-${i}`, async () => {
      const env = createEnv();
      const bad = [`Bad Name ${i}!`, `has space`, `UPPER@`, `x/y`, `a:b`, `*star*`][i % 6];
      const res = await jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: bad, password: 'x' }),
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`users/create Username already taken exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: 'bob', password: `pw${i}` }),
        env
      );
      expect(res.status).toBe(400);
  ...
  for (let i = 0; i < 12; i++) {
    it(`users/create username and password required exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [
        {},
        { username: `onlyuser${i}` },
  ...
  for (let i = 0; i < 12; i++) {
    it(`bulk-delete user_ids array required exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [{}, { user_ids: [] }, { user_ids: 'not-array' }, { user_ids: null }] as const;
      const res = await jsonReq(
  ...
  for (let i = 0; i < 12; i++) {
    it(`server-notice user_id and message required exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [
        {},
        { user_id: BOB },
  ...
  for (let i = 0; i < 12; i++) {
    it(`reports resolve Report not found exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
  ...
  for (let i = 0; i < 12; i++) {
    it(`reports unresolve Report not found exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/admin/api/reports/${800 + i}/unresolve`,
  ...
  for (let i = 0; i < 12; i++) {
    it(`synapse PUT Invalid user ID format exact flood-${i}`, async () => {
      const env = createEnv();
      const bad = [`not-an-mxid-${i}`, `bob`, `@nohost`, `:::`, `@`, `plain`];
      const res = await jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent(bad[i % bad.length]!)}`,
  ...
  for (let i = 0; i < 12; i++) {
    it(`synapse PUT password required for new user exact flood-${i}`, async () => {
      const env = createEnv();
      const uid = `@freshq5${i}:example.com`;
      const res = await jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent(uid)}`,
        jsonInit('PUT', { displayname: `NoPass${i}` }),
        env
      );
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
...
describe('race quinary admin create/report/notice exact strings after #297', () => {
  it('Invalid username format∥Username already taken isolation', async () => {
    const env = createEnv();
    const [invalid, taken] = await Promise.all([
      jsonReq(
        '/admin/api/users/create',
    ...
    expect(statusesOf([invalid, taken])).toEqual([400, 400]);
    expect(invalid.body.error).toBe('Invalid username format');
    expect(taken.body.error).toBe('Username already taken');
  });

  it('username and password required∥create success isolation', async () => {
    ...
    const [miss, ok] = await Promise.all([
      ...
      jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: 'carolq5', password: 'Secret1!', display_name: 'Carol' }),
        env
      ),
    ]);
    expect(miss.status).toBe(400);
    expect(miss.body.error).toBe('Missing required parameter: username and password required');
    expect(ok.status).toBe(200);
    expect(ok.body.user_id).toBe('@carolq5:example.com');
  });

  it('user_ids array required∥bulk-delete BOB isolation', async () => {
    const db = createAdminDb();
    const env = createEnv({ db });
    const [miss, ok] = await Promise.all([
    ...
    expect(miss.status).toBe(400);
    expect(miss.body.error).toBe('Missing required parameter: user_ids array required');
    expect(ok.status).toBe(200);
    expect(ok.body.deleted).toBe(1);
  });

  it('user_id and message required∥server-notice success isolation', async () => {
    const db = createAdminDb();
    const env = createEnv({ db });
    const [miss, ok] = await Promise.all([
      ...
      jsonReq(
        '/admin/api/server-notice',
        jsonInit('POST', { user_id: BOB, message: 'hello-q5' }),
        env
      ),
    ]);
    expect(miss.status).toBe(400);
    expect(miss.body.error).toBe('Missing required parameter: user_id and message required');
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
  });

  it('Report not found resolve∥unresolve∥resolve-ok triple', async () => {
    const db = createAdminDb();
    const env = createEnv({ db });
    const [missR, missU, ok] = await Promise.all([
      jsonReq('/admin/api/reports/999/resolve', jsonInit('POST', { note: 'x' }), env),
      jsonReq('/admin/api/reports/998/unresolve', { method: 'POST', headers: AUTH }, env),
      jsonReq('/admin/api/reports/1/resolve', jsonInit('POST', { note: 'done' }), env),
    ]);
    expect(missR.status).toBe(404);
    expect(missR.body.error).toBe('Report not found');
    expect(missU.status).toBe(404);
    expect(missU.body.error).toBe('Report not found');
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
  });

  it('Invalid user ID format∥password required for new user isolation', async () => {
    const env = createEnv();
    const [badId, noPass] = await Promise.all([
      jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent('not-an-mxid')}`,
        jsonInit('PUT', { password: 'Pass1!xx' }),
        env
      ),
      jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent('@nopassq5:example.com')}`,
  ...
  for (let i = 0; i < 10; i++) {
    it(`quinary admin exact race flood-${i}`, async () => {
      ...
      const results = await Promise.all([
        ...
        jsonReq(`/admin/api/reports/${700 + i}/resolve`, jsonInit('POST', {}), env),
        jsonReq(
          `/_synapse/admin/v2/users/${encodeURIComponent(`not-mxid-${i}`)}`,
          jsonInit('PUT', { password: 'Pass1!xx' }),
          env
        ),
      ]);
      expect(results[0]!.body.error).toBe('Invalid username format');
      expect(results[1]!.body.error).toBe('Username already taken');
      expect(results[2]!.body.error).toBe('Missing required parameter: user_ids array required');
      expect(results[3]!.body.error).toBe(
        'Missing required parameter: user_id and message required'
      );
      expect(results[4]!.body.error).toBe('Report not found');
      expect(results[5]!.body.error).toBe('Invalid user ID format');
    });
  }
});
