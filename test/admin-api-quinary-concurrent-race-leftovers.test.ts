/**
 * TOKENMAXX HEAVY leftovers after #310 — quinary *admin* soft+concurrent-race
 * niches unsaturated by quaternary exact-strings (#297 / tip after #279).
 *
 * Distinct from quaternary (#297) self-demote/purge/registration/deactivated/
 * whois/Verified∥No-self-signing:
 *   keys No signature in DB ∥ Signature not in device key object under race;
 *   create exact Invalid username format / Username already taken /
 *     username and password required;
 *   bulk-delete exact user_ids array required;
 *   make/remove-admin exact Missing required parameter: user_id;
 *   Synapse PUT exact Invalid user ID format / password required for new user;
 *   PUT users exact No fields to update;
 *   reset-password exact password / synapse new_password;
 *   server-notice exact user_id and message required;
 *   IdP discovery fail exact under soft+race.
 *
...
describe('soft quinary admin create/keys/param exact strings after #310', () => {
  for (let i = 0; i < 12; i++) {
    it(`keys No signature in DB soft flood-${i}`, async () => {
      const db = createAdminDb({ crossSigningSigs: [] });
      const env = createEnv({ db });
      const res = await jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, env);
      expect(res.status).toBe(200);
      const vs = (res.body.verification_status as Record<string, { verified: boolean; reason: string }>)
        .BOBDEVICE;
      expect(vs).toEqual({ verified: false, reason: 'No signature in DB' });
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`keys Signature not in device key object soft flood-${i}`, async () => {
      ...
      const vs = (res.body.verification_status as Record<string, { verified: boolean; reason: string }>)
        .BOBDEVICE;
      expect(vs).toEqual({ verified: false, reason: 'Signature not in device key object' });
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`create Invalid username format exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: `Bad Name!${i}`, password: 'Secret1!' }),
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`create Username already taken exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: 'bob', password: 'Secret1!' }),
        env
      );
      expect(res.status).toBe(400);
  ...
  for (let i = 0; i < 12; i++) {
    it(`create missing username/password exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [{}, { username: 'x' }, { password: 'y' }, { username: '', password: '' }] as const;
      const res = await jsonReq(
        '/admin/api/users/create',
      ...
      expect(res.body.error).toBe('Missing required parameter: username and password required');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`bulk-delete user_ids array required exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [{}, { user_ids: 'x' }, { user_ids: [] }, { user_ids: null }] as const;
      const res = await jsonReq(
      ...
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: user_ids array required');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`make/remove-admin missing user_id exact flood-${i}`, async () => {
      const env = createEnv();
      const path = i % 2 === 0 ? '/admin/api/make-admin' : '/admin/api/remove-admin';
      const res = await jsonReq(path, jsonInit('POST', {}), env);
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: user_id');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`synapse Invalid user ID format exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent(`not-an-mxid-${i}`)}`,
        jsonInit('PUT', { password: 'Secret1!' }),
        env
      );
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_INVALID_USERNAME');
  ...
  for (let i = 0; i < 12; i++) {
    it(`synapse password required for new user exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent(`@newuser${i}:example.com`)}`,
        jsonInit('PUT', { displayname: 'X' }),
        env
      );
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: password required for new user');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`PUT users No fields to update exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
      ...
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: No fields to update');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`reset-password missing password exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
      ...
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: password');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`synapse reset_password missing new_password exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
      ...
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: new_password');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`server-notice missing params exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [{}, { user_id: BOB }, { message: 'hi' }, { user_id: '', message: '' }] as const;
      const res = await jsonReq(
      ...
      expect(res.status).toBe(400);
      expect(res.body.errcode).toBe('M_MISSING_PARAM');
      expect(res.body.error).toBe('Missing required parameter: user_id and message required');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`IdP discovery fail exact soft flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', {
          name: `BadIdP${i}`,
          issuer_url: 'https://bad-issuer.example.com',
          client_id: 'cid',
          client_secret: 'csec',
        }),
        env
...
describe('race quinary admin create/keys/param exact strings after #310', () => {
  it('keys No signature in DB∥Signature not in device key object isolation', async () => {
    const dbNoSig = createAdminDb({ crossSigningSigs: [] });
    const envNoSig = createEnv({ db: dbNoSig });
    const deviceKeys = mockKv({
      [`device:${BOB}:BOBDEVICE`]: JSON.stringify({
        algorithms: ['m.olm.v1.curve25519-aes-sha2'],
        device_id: 'BOBDEVICE',
    ...
    const envBadObj = createEnv({ deviceKeys });
    const [noDb, noObj] = await Promise.all([
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, envNoSig),
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, envBadObj),
    ]);
    expect(statusesOf([noDb, noObj])).toEqual([200, 200]);
    expect(
      (noDb.body.verification_status as Record<string, { reason: string }>).BOBDEVICE.reason
    ).toBe('No signature in DB');
    expect(
      (noObj.body.verification_status as Record<string, { reason: string }>).BOBDEVICE.reason
    ).toBe('Signature not in device key object');
  });

  it('create Invalid username∥Username already taken isolation', async () => {
    const env = createEnv();
    const [bad, dup] = await Promise.all([
      jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: 'Bad Name!', password: 'Secret1!' }),
        env
      ),
      jsonReq(
        '/admin/api/users/create',
    ...
    expect(statusesOf([bad, dup])).toEqual([400, 400]);
    expect(bad.body.error).toBe('Invalid username format');
    expect(dup.body.error).toBe('Username already taken');
  });

  it('create missing params exact∥create ok isolation', async () => {
    const env = createEnv();
    const [missing, ok] = await Promise.all([
      jsonReq('/admin/api/users/create', jsonInit('POST', { username: 'only' }), env),
      jsonReq(
        '/admin/api/users/create',
        jsonInit('POST', { username: 'carol', password: 'Secret1!', display_name: 'Carol' }),
        env
      ),
    ]);
    expect(missing.status).toBe(400);
    expect(missing.body.error).toBe('Missing required parameter: username and password required');
    expect(ok.status).toBe(200);
    expect(ok.body.user_id).toBe('@carol:example.com');
  });

  it('bulk-delete missing∥make-admin missing user_id isolation', async () => {
    const env = createEnv();
    const [bulk, make] = await Promise.all([
      jsonReq('/admin/api/users/bulk-delete', jsonInit('POST', { user_ids: [] }), env),
      jsonReq('/admin/api/make-admin', jsonInit('POST', {}), env),
    ]);
    expect(statusesOf([bulk, make])).toEqual([400, 400]);
    expect(bulk.body.error).toBe('Missing required parameter: user_ids array required');
    expect(make.body.error).toBe('Missing required parameter: user_id');
  });

  it('remove-admin missing user_id∥self-demote exact isolation', async () => {
    const env = createEnv();
    const [missing, self] = await Promise.all([
      jsonReq('/admin/api/remove-admin', jsonInit('POST', {}), env),
      jsonReq('/admin/api/remove-admin', jsonInit('POST', { user_id: ADMIN }), env),
    ]);
    expect(missing.status).toBe(400);
    expect(missing.body.error).toBe('Missing required parameter: user_id');
    expect(self.status).toBe(403);
    expect(self.body.error).toBe('Cannot remove your own admin privileges');
  });

  it('synapse Invalid user ID∥password required isolation', async () => {
    const env = createEnv();
    const [badId, noPass] = await Promise.all([
      jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent('not-mxid')}`,
        jsonInit('PUT', { password: 'Secret1!' }),
        env
      ),
      jsonReq(
        `/_synapse/admin/v2/users/${encodeURIComponent('@fresh:example.com')}`,
        jsonInit('PUT', { displayname: 'Fresh' }),
        env
      ),
    ]);
    expect(statusesOf([badId, noPass])).toEqual([400, 400]);
    expect(badId.body.error).toBe('Invalid user ID format');
    expect(noPass.body.error).toBe('Missing required parameter: password required for new user');
  });

  it('PUT No fields to update∥display_name ok isolation', async () => {
    const env = createEnv();
    const [empty, ok] = await Promise.all([
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}`, jsonInit('PUT', {}), env),
      jsonReq(
        `/admin/api/users/${encodeURIComponent(BOB)}`,
        jsonInit('PUT', { display_name: 'Bobby' }),
        env
      ),
    ]);
    expect(empty.status).toBe(400);
    expect(empty.body.error).toBe('Missing required parameter: No fields to update');
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
  });

  it('admin reset-password missing∥ok isolation', async () => {
    const env = createEnv();
    const [missing, ok] = await Promise.all([
      jsonReq(
        `/admin/api/users/${encodeURIComponent(BOB)}/reset-password`,
      ...
      jsonReq(
        `/admin/api/users/${encodeURIComponent(BOB)}/reset-password`,
        jsonInit('POST', { password: 'NewPass123!' }),
        env
      ),
    ]);
    expect(missing.status).toBe(400);
    expect(missing.body.error).toBe('Missing required parameter: password');
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
  });

  it('synapse reset_password missing new_password∥ok isolation', async () => {
    const env = createEnv();
    const [missing, ok] = await Promise.all([
      jsonReq(
        `/_synapse/admin/v1/reset_password/${encodeURIComponent(BOB)}`,
      ...
      jsonReq(
        `/_synapse/admin/v1/reset_password/${encodeURIComponent(BOB)}`,
        jsonInit('POST', { new_password: 'NewerPass123!' }),
        env
      ),
    ]);
    expect(missing.status).toBe(400);
    expect(missing.body.error).toBe('Missing required parameter: new_password');
    expect(ok.status).toBe(200);
  });

  it('server-notice missing∥IdP discovery fail isolation', async () => {
    const env = createEnv();
    const [notice, idp] = await Promise.all([
    ...
    expect(statusesOf([notice, idp])).toEqual([400, 400]);
    expect(notice.body.error).toBe('Missing required parameter: user_id and message required');
    expect(idp.body.error).toBe(
      'Failed to fetch OIDC discovery from issuer. Check the issuer URL is correct and accessible.'
    );
  });

  for (let i = 0; i < 10; i++) {
    it(`quinary admin exact race flood-${i}`, async () => {
      const env = createEnv();
      const results = await Promise.all([
        jsonReq(
          '/admin/api/users/create',
          jsonInit('POST', { username: `Bad!${i}`, password: 'x' }),
          env
        ),
        jsonReq('/admin/api/users/bulk-delete', jsonInit('POST', {}), env),
        jsonReq('/admin/api/make-admin', jsonInit('POST', {}), env),
        jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}`, jsonInit('PUT', {}), env),
        jsonReq(
          `/admin/api/users/${encodeURIComponent(BOB)}/reset-password`,
        ...
        jsonReq('/admin/api/server-notice', jsonInit('POST', { message: 'x' }), env),
      ]);
      expect(results.every((r) => r.status === 400)).toBe(true);
      expect(results[0].body.error).toBe('Invalid username format');
      expect(results[1].body.error).toBe('Missing required parameter: user_ids array required');
      expect(results[2].body.error).toBe('Missing required parameter: user_id');
      expect(results[3].body.error).toBe('Missing required parameter: No fields to update');
      expect(results[4].body.error).toBe('Missing required parameter: password');
      expect(results[5].body.error).toBe('Missing required parameter: user_id and message required');
    });
  }
});
