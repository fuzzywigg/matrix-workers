/**
 * TOKENMAXX HEAVY tip-relaunch deepen after merged #357 — niche
 * presence/receipts/typing soft residual *eighth-wave* concurrent-race leftovers
 * (tip past #357 seventh).
 *
 * Unsaturated by waves #301/#305/#318/#327/#334/#341/#357:
 *   never OnLiNe / oFfLiNe / UnAvAiLaBlE / OFFLinE case pins,
 *   never Mobile/Present/DoNotDisturb/DnD/Offlineish invalid pins,
 *   never vertical-tab / NBSP / trailing-CR whitespace (`\vonline`, `\u00a0online`,
 *     `offline\r`),
 *   never typing root-string `"false"` / root-number `0` → Missing typing ∥ boolean ok,
 *   never typing invite-membership Not a member ∥ join ok,
 ...
 *   OnLiNe/oFfLiNe/UnAvAiLaBlE + Mobile/Present/DnD + VT/NBSP/CR whitespace ∥ ok
 *   typing root-string/number Missing typing + invite-membership Not a member ∥ ok
 *   receipt case-mutated Invalid type matrix ∥ m.read ok
 *   cross-domain: OnLiNe + root-string `"false"` typing + m.fully_READ ∥ oks
 *
...
describe('presence eighth-wave concurrent case/whitespace soft after #357', () => {
  const cases: Array<{ label: string; body: unknown; error: string }> = [
    ...
    {
      label: 'Mobile-title',
      body: { presence: 'Mobile' },
      error: 'Invalid presence state: Mobile. Must be one of: online, offline, unavailable',
    },
    {
      label: 'Present-title',
      body: { presence: 'Present' },
      error: 'Invalid presence state: Present. Must be one of: online, offline, unavailable',
    },
    {
      label: 'DoNotDisturb-title',
      body: { presence: 'DoNotDisturb' },
      error: 'Invalid presence state: DoNotDisturb. Must be one of: online, offline, unavailable',
    },
    {
      label: 'DnD-mixed',
      body: { presence: 'DnD' },
    ...
    {
      label: 'Offlineish-title',
      body: { presence: 'Offlineish' },
      error: 'Invalid presence state: Offlineish. Must be one of: online, offline, unavailable',
    },
    {
      label: 'vertical-tab-whitespace',
      body: { presence: '\vonline' },
      error: 'Invalid presence state: \vonline. Must be one of: online, offline, unavailable',
    },
    {
      label: 'nbsp-whitespace',
      body: { presence: '\u00a0online' },
      error: 'Invalid presence state: \u00a0online. Must be one of: online, offline, unavailable',
    },
    {
      label: 'trailing-CR-whitespace',
      body: { presence: 'offline\r' },
      error: 'Invalid presence state: offline\r. Must be one of: online, offline, unavailable',
    },
  ];
  ...
  it('OnLiNe ∥ oFfLiNe ∥ Mobile ∥ DnD ∥ offline under race', async () => {
    const env = createEnv();
    const results = await Promise.all([
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
    ...
    expect(
      ...
      [
        ...
        'Invalid presence state: Mobile. Must be one of: online, offline, unavailable',
...
describe('typing eighth-wave concurrent root-scalar + invite-membership soft after #357', () => {
  ...
  it('root-number 0 Missing typing ∥ boolean false under race', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
      typingReq(
  ...
  it('invite membership Not a member ∥ join ok under race', async () => {
    const roomDO = createRoomDOStub();
    const softRoom = '!invite-mem:example.com';
    const env = createEnv({
      roomDO,
      db: createSharedDb({
        memberships: [
          { room_id: ROOM, user_id: USER, membership: 'join' },
          { room_id: softRoom, user_id: USER, membership: 'invite' },
        ],
      }),
    });
    const results = await Promise.all([
  ...
  it('root-string + invite-membership ∥ two oks under race', async () => {
    const roomDO = createRoomDOStub();
    const softRoom = '!inv:example.com';
    const env = createEnv({
      roomDO,
      db: createSharedDb({
        memberships: [
  ...
  for (let i = 0; i < 6; i++) {
    it(`typing root-scalar / invite-mem flood-${i} ∥ ok`, async () => {
      const roomDO = createRoomDOStub();
      const softRoom = `!inv${i}:example.com`;
      const env = createEnv({
        roomDO,
        db: createSharedDb({
          memberships: [
...
describe('receipts eighth-wave concurrent case-mutated Invalid type soft after #357', () => {
...
// ---------------------------------------------------------------------------
// Cross-domain eighth-wave soft isolation
// ---------------------------------------------------------------------------

describe('presence/receipts/typing eighth-wave cross-domain soft isolation after #357', () => {
  ...
  for (let i = 0; i < 6; i++) {
    it(`cross-domain eighth-wave soft isolation flood-${i}`, async () => {
      const roomDO = createRoomDOStub();
      const env = createEnv({ roomDO });
      const badPresence =
        i % 3 === 0 ? 'OnLiNe' : i % 3 === 1 ? 'oFfLiNe' : 'Mobile';
