/**
 * TOKENMAXX HEAVY tip-relaunch at tip f1983d3 after merged #387
 * room-cache/hibernation octodenary (fresh after closed #388/#384) —
 ...
 *        OAuth approved∥DELETE∥Missing; callback no-code∥keys∥NoPw∥
 *        Invalid; token expired∥delete_devices∥list∥not-found — never
 *        SSO redirect ∥ token soft ∥ keys challenge ∥ Unrecognized;
 *        never SSO callback ∥ OAuth approved ∥ delete_devices challenge ∥
 *        Invalid password; never SSO callback ∥ not-approved ∥ GET list ∥
 *        PUT ok; never token mismatch ∥ DELETE challenge ∥ No password ∥
 *        Missing devices),
 ...
 *
 * Gap table (why leftover after tenth-wave / tip past #387):
 *   SSO redirect ∥ token soft ∥ keys UIA challenge ∥ Unrecognized
 *     | ninth SSO∥token∥not-approved; tenth callback∥keys; eighth
 *       token∥PUT∥Unrecognized
 *   SSO callback ∥ OAuth approved ∥ delete_devices challenge ∥ Invalid
 *     | tenth redirect∥approved∥DELETE; ninth callback∥devices
 *   SSO callback ∥ not-approved ∥ GET list ∥ PUT ok
 *     | ninth not-approved with redirect; tenth callback+PUT; list with
 *       token/delete_devices
 *   token mismatch ∥ DELETE challenge ∥ No password ∥ Missing devices
 *     | ninth token∥keys∥PUT; tenth token∥delete_devices∥list; eighth
 *       approved∥DELETE∥NoPw
 *
...
describe('devices soft residual eleventh-wave SSO∥token∥keys-challenge∥Unrecognized after #366', () => {
  it('SSO redirect Missing∥expired ∥ token missing ∥ keys UIA challenge ∥ Unrecognized under race', async () => {
    ...
    const [ssoMissing, ssoExpired, tokenMissing, challenge, unrec] = await Promise.all([
  ...
  it('SSO redirect ∥ token missing∥expired ∥ keys challenge ∥ Unrecognized ∥ Session user mismatch', async () => {
    const cache = mockKv({
    ...
    const [ssoMissing, tokenMissing, tokenExpired, challenge, unrec, mismatch] =
      await Promise.all([
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO redirect ∥ token soft ∥ keys challenge ∥ Unrecognized flood-${i}`, async () => {
...
// ---------------------------------------------------------------------------
// SSO callback ∥ OAuth approved ∥ delete_devices challenge ∥ Invalid password
// ---------------------------------------------------------------------------

describe('devices soft residual eleventh-wave SSO-callback∥approved∥delete_devices∥Invalid after #366', () => {
  it('SSO callback Missing∥expired ∥ OAuth approved ∥ delete_devices challenge ∥ Invalid password under race', async () => {
    const cache = mockKv({
    ...
    const [missingState, expired, ok, challenge, badPw] = await Promise.all([
  ...
  it('SSO callback Missing∥expired∥no-code ∥ OAuth approved ∥ delete_devices challenge ∥ Invalid password', async () => {
    const cache = mockKv({
      'uia_session:ok': JSON.stringify({
        user_id: USER,
    ...
    const [missingState, expired, noCode, ok, challenge, badPw] = await Promise.all([
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO callback ∥ OAuth approved ∥ delete_devices ∥ Invalid password flood-${i}`, async () => {
...
describe('devices soft residual eleventh-wave SSO-callback∥not-approved∥list∥PUT after #366', () => {
  it('SSO callback Missing∥expired ∥ not-approved ∥ GET list ok ∥ PUT ok under race', async () => {
    const cache = mockKv({
    ...
    const [missingState, expired, pending, list, putOk] = await Promise.all([
      keysRequest(keysEnv, `${SSO_CALLBACK}?code=abc`),
      keysRequest(keysEnv, `${SSO_CALLBACK}?code=abc&state=gone`),
      keysRequest(
        keysEnv,
  ...
  it('SSO callback Missing∥expired∥no-code ∥ not-approved ∥ list ∥ PUT ok ∥ Device not found', async () => {
    const cache = mockKv({
      'uia_session:pend': JSON.stringify({
        user_id: USER,
        completed_stages: [],
    ...
    const [missingState, expired, noCode, pending, list, putOk, notFound] =
      await Promise.all([
        keysRequest(keysEnv, `${SSO_CALLBACK}?code=x`),
        keysRequest(keysEnv, `${SSO_CALLBACK}?code=x&state=missing`),
        keysRequest(keysEnv, `${SSO_CALLBACK}?state=race-nocode`),
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO callback ∥ not-approved ∥ list ∥ PUT flood-${i}`, async () => {
      const cache = mockKv({
...
describe('devices soft residual eleventh-wave token∥DELETE∥NoPw∥Missing after #366', () => {
  it('token mismatch ∥ DELETE challenge ∥ No password ∥ Missing devices under race', async () => {
    const cache = mockKv({
      'uia_session:mm': JSON.stringify({
        user_id: BOB,
        completed_stages: [],
    ...
    const [mismatch, challenge, noPw, missing] = await Promise.all([
      keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', { session: 'mm' })),
  ...
  it('token missing∥mismatch ∥ DELETE challenge ∥ No password ∥ Missing devices ∥ badJson', async () => {
    const cache = mockKv({
      'uia_session:mm': JSON.stringify({
        user_id: BOB,
        completed_stages: [],
    ...
    const [missingTok, mismatch, challenge, noPw, missingDevices, badJson] =
      await Promise.all([
        keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', {})),
        keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', { session: 'mm' })),
        devicesRequest(db, `${DEVICES}/CHALLENGE`, { method: 'DELETE', headers: AUTH }),
    ...
    expect(missingTok.body).toEqual(TOKEN_MISSING_SESSION);
    expect(mismatch.body).toEqual(SESSION_MISMATCH);
    expect(challenge.status).toBe(401);
    expect(noPw.body).toEqual(NO_PASSWORD);
  ...
  for (let i = 0; i < 8; i++) {
    it(`token mismatch ∥ DELETE challenge ∥ No password ∥ Missing devices flood-${i}`, async () => {
      const cache = mockKv({
