/**
 * TOKENMAXX HEAVY tip-relaunch after #340/#357 — oauth *tredecenary*
 * concurrent-race niches duodenary (#340) never mixed under Promise.all:
 *   sensitive ∥ parse ∥ Expired JSON ∥ Session Expired
 *     (duodenary Expired∥Missing∥Approved without sensitive+parse+
 *     expired-JSON; undecenary parse with OIDC/mismatch without
 *     Expired JSON+Session Expired),
 *   password Approved ∥ Cancelled ∥ Username required ∥ login Missing
 *     (duodenary Approved without Cancelled/Username/login-Missing;
 *     Cancelled without password-Approved/Username/Missing),
 *   OIDC Approved ∥ cross-signing ∥ Expired JSON ∥ login no-period
 *     (duodenary OIDC∥period∥no-period without cross-signing+Expired JSON;
 *     mega had OIDC∥cross-signing∥period without Expired JSON∥no-period).
 *
 * Gap table (why leftover after #340 / tip past #357):
 *   sensitive ∥ parse ∥ Expired JSON ∥ Session Expired
 *     | duodenary/undecenary pairs only; single UIA POST for parse
 *   password Approved ∥ Cancelled ∥ Username required ∥ login Missing
 *     | duodenary Approved and Cancelled in separate quads
 *   OIDC ∥ cross-signing ∥ Expired JSON ∥ login no-period
 *     | duodenary mega without Expired JSON+no-period pair
 *
 * Distinct from #340 duodenary, #324 undecenary, #315 denary.
...
describe('tredecenary oauth sensitive∥parse∥ExpiredJSON∥SessionExpired after #340', () => {
  it('sensitive ∥ parse ∥ Expired JSON ∥ Session Expired under race', async () => {
    // Single UIA POST so mockRejectedValueOnce is deterministic under PA.
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
      .mockRejectedValueOnce(new Error('tredecenary-parse'));
    const cache = mockKv();
    ...
    const htmls = htmlOf([results[0], results[1], results[3]]);
  ...
  it('sensitive ∥ parse ∥ Expired JSON ∥ Session Expired ∥ Missing Session penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
      .mockRejectedValueOnce(new Error('tredecenary-parse-penta'));
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-parse');
    ...
    const htmls = htmlOf([results[0], results[1], results[3], results[4]]);
    expect(htmls.some((h) => h.includes(UIA_SENSITIVE))).toBe(true);
    expect(htmls.some((h) => h.includes(UIA_PARSE_BODY))).toBe(true);
    expect(htmls.some((h) => h.includes(UIA_EXPIRED_BODY))).toBe(true);
  ...
  for (let i = 0; i < 8; i++) {
    it(`sensitive ∥ parse ∥ Expired JSON ∥ Session Expired flood-${i}`, async () => {
      const spy = vi
        .spyOn(HonoRequest.prototype, 'parseBody')
        .mockRejectedValueOnce(new Error(`tredecenary-parse-${i}`));
      const cache = mockKv();
      seedClient(cache, 'cid-1');
...
describe('tredecenary oauth Approved∥Cancelled∥Username∥loginMissing after #340', () => {
  it('password Approved ∥ Cancelled ∥ Username required ∥ login Missing under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
  ...
  it('Approved ∥ Cancelled ∥ Username ∥ login Missing ∥ default desc penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-ok');
    seedUiaSession(cache, 'uia-cancel');
  ...
  for (let i = 0; i < 8; i++) {
    it(`Approved ∥ Cancelled ∥ Username ∥ login Missing flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      ...
      const results = await Promise.all([
        ...
        request(
          '/oauth/authorize',
          formInit({ password: 'secret', auth_request_id: `ar-miss-${i}` }),
          env
        ),
      ]);
      const htmls = htmlOf(results);
...
describe('tredecenary oauth OIDC∥cross-signing∥ExpiredJSON∥no-period after #340', () => {
  it('OIDC Approved ∥ cross-signing ∥ Expired JSON ∥ login no-period under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
  ...
  it('OIDC ∥ cross-signing ∥ Expired JSON ∥ no-period ∥ credentials penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
    seedUiaSession(cache, 'uia-xr');
  ...
  for (let i = 0; i < 8; i++) {
    it(`OIDC ∥ cross-signing ∥ Expired JSON ∥ no-period flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
  ...
  it('sensitive∥Approved∥Cancelled∥OIDC∥ExpiredJSON∥no-period mega race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-ok');
    ...
    expect(results[4].body).toMatchObject({ error_description: AUTH_REQUEST_EXPIRED });
    const htmls = htmlOf([results[0], results[1], results[2], results[3], results[5]]);
    expect(htmls.some((h) => h.includes(UIA_SENSITIVE))).toBe(true);
    expect(htmls.some((h) => h.includes(UIA_CROSS_SIGNING_DESC))).toBe(true);
    expect(htmls.some((h) => h.includes('Request Approved'))).toBe(true);
    expect(htmls.some((h) => h.includes('Request Cancelled'))).toBe(true);
