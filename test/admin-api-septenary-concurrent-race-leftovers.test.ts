 ...
 * niches unsaturated by senary report/auth/404/IdP-PUT exact (#328 / tip after #317/#328).
 *
...
describe('soft septenary admin IdP create/missing/test exact after #328', () => {
  for (let i = 0; i < 12; i++) {
...
describe('race septenary admin IdP create/missing/test/delete after #328', () => {
  ...
  for (let i = 0; i < 8; i++) {
    it(`septenary admin IdP exact race flood-${i}`, async () => {
      const failDb = createAdminDb({ failIdpInsert: true, idpProviders: [] });
      const delDb = createAdminDb({
        idpProviders: [
          {
