/**
 * TOKENMAXX HEAVY leftovers after #328 — septenary *admin* soft+concurrent-race
 * niches unsaturated by senary report/auth/404/IdP-PUT exact (#328 / tip after #336).
 *
 * Gap table (why leftover after #328 senary):
 *   IdP create missing params exact
 *     | senary PUT discovery∥GET not-found; never POST create missing-fields soft
 *   IdP create Failed to create identity provider (500)
 *     | zero coverage in admin-api-* concurrent leftovers
 *   IdP DELETE Identity provider not found soft flood
 *     | senary only GET soft; DELETE only raced with PUT discovery
 *   GET users/:id User not found exact
 *     | senary only login-token User not found path
 *   Synapse rooms Room not found exact
 *     | senary only native GET /admin/api/rooms/:id
 *   Synapse reset_password User not found exact
 *     | quinary missing new_password only; never 404 user on synapse path
 *   login-token User is deactivated∥IdP create-fail race
 *     | quaternary soft deactivated alone; never mixed with IdP 500
 *
...
describe('soft septenary admin IdP-create/DELETE/user-room exact after #328', () => {
  for (let i = 0; i < 12; i++) {
    it(`IdP create missing params exact flood-${i}`, async () => {
      const env = createEnv();
      const bodies = [
        {},
        { name: 'x' },
        { name: 'x', issuer_url: 'https://issuer.example.com' },
        { name: 'x', issuer_url: 'https://issuer.example.com', client_id: 'c' },
      ];
  ...
  for (let i = 0; i < 12; i++) {
    it(`IdP create Failed to create identity provider exact flood-${i}`, async () => {
      const env = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
      const res = await jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', {
          ...IDP_CREATE_BODY,
          name: `FailIdP-${i}`,
          issuer_url: `https://issuer${i}.example.com`,
        }),
        env
      );
      expect(res.status).toBe(500);
      expect(res.body.errcode).toBe('M_UNKNOWN');
      expect(res.body.error).toBe('Failed to create identity provider');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`IdP DELETE Identity provider not found exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/admin/api/idp/providers/del-miss-${i}`,
        jsonInit('DELETE'),
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`GET users User not found exact flood-${i}`, async () => {
      const env = createEnv();
      const missing = `@ghostu${i}:example.com`;
      const res = await jsonReq(`/admin/api/users/${encodeURIComponent(missing)}`, {}, env);
      expect(res.status).toBe(404);
      expect(res.body.errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`synapse rooms Room not found exact flood-${i}`, async () => {
      const env = createEnv();
      const missing = `!synroom${i}:example.com`;
      const res = await jsonReq(
        `/_synapse/admin/v1/rooms/${encodeURIComponent(missing)}`,
        {},
        env
      );
      expect(res.status).toBe(404);
  ...
  for (let i = 0; i < 12; i++) {
    it(`synapse reset_password User not found exact flood-${i}`, async () => {
      const env = createEnv();
      const missing = `@synreset${i}:example.com`;
      const res = await jsonReq(
        `/_synapse/admin/v1/reset_password/${encodeURIComponent(missing)}`,
        jsonInit('POST', { new_password: `pw-${i}` }),
        env
      );
      expect(res.status).toBe(404);
      expect(res.body.errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`login-token User is deactivated exact flood-${i}`, async () => {
      const env = createEnv({
        db: createAdminDb({
...
describe('race septenary admin IdP-create/DELETE/user-room exact after #328', () => {
  it('IdP create missing params∥Failed to create isolation', async () => {
    const missEnv = createEnv();
    const failEnv = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
    const [miss, fail] = await Promise.all([
      jsonReq('/admin/api/idp/providers', jsonInit('POST', { name: 'only' }), missEnv),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'FailRace' }),
        failEnv
      ),
    ]);
    expect(miss.status).toBe(400);
    expect(miss.body.error).toBe(
      'Missing required parameter: name, issuer_url, client_id, and client_secret are required'
    );
  ...
  it('IdP create ok∥Failed to create isolation', async () => {
    const okEnv = createEnv();
    const failEnv = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
    const [ok, fail] = await Promise.all([
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'OkRace' }),
        okEnv
      ),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'FailRace2', issuer_url: 'https://issuer-b.example.com' }),
        failEnv
      ),
    ]);
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
    expect(ok.body.id).toBe('idp-opaque-12');
    expect(fail.status).toBe(500);
    expect(fail.body.error).toBe('Failed to create identity provider');
  });

  it('IdP DELETE not found∥create missing params isolation', async () => {
    const env = createEnv();
    const [del, miss] = await Promise.all([
      jsonReq('/admin/api/idp/providers/nope-del', jsonInit('DELETE'), env),
      jsonReq('/admin/api/idp/providers', jsonInit('POST', {}), env),
    ]);
  ...
  it('GET users User not found∥synapse rooms Room not found isolation', async () => {
    const env = createEnv();
    const [userMiss, roomMiss] = await Promise.all([
      jsonReq(`/admin/api/users/${encodeURIComponent('@nouser:example.com')}`, {}, env),
      jsonReq(`/_synapse/admin/v1/rooms/${encodeURIComponent('!nouser:example.com')}`, {}, env),
    ]);
  ...
  it('GET users User not found∥GET users ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq(`/admin/api/users/${encodeURIComponent('@ghost:example.com')}`, {}, env),
  ...
  it('synapse reset_password User not found∥ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq(
        `/_synapse/admin/v1/reset_password/${encodeURIComponent('@ghost:example.com')}`,
        jsonInit('POST', { new_password: 'np' }),
        env
      ),
      jsonReq(
        `/_synapse/admin/v1/reset_password/${encodeURIComponent(BOB)}`,
        jsonInit('POST', { new_password: 'np-ok' }),
        env
      ),
    ]);
    expect(miss.status).toBe(404);
  ...
  it('synapse rooms Room not found∥native room ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq(`/_synapse/admin/v1/rooms/${encodeURIComponent('!missing:example.com')}`, {}, env),
  ...
  it('login-token deactivated∥IdP create fail isolation', async () => {
    const deactEnv = createEnv({
      db: createAdminDb({
        users: [defaultAdmin(), { ...defaultBob(), is_deactivated: 1 }],
      }),
    });
    const failEnv = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
    const [deact, fail] = await Promise.all([
      jsonReq(
        `/admin/api/users/${encodeURIComponent(BOB)}/login-token`,
        jsonInit('POST', {}),
        deactEnv
      ...
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'DeactFail' }),
        failEnv
      ),
    ]);
  ...
  it('IdP DELETE not found∥GET users miss∥synapse room miss∥create fail four-way', async () => {
    const env = createEnv();
    const failEnv = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
    const [del, userMiss, roomMiss, fail] = await Promise.all([
      jsonReq('/admin/api/idp/providers/four-gone', jsonInit('DELETE'), env),
      jsonReq(`/admin/api/users/${encodeURIComponent('@four:example.com')}`, {}, env),
      jsonReq(`/_synapse/admin/v1/rooms/${encodeURIComponent('!four:example.com')}`, {}, env),
      jsonReq(
        '/admin/api/idp/providers',
        jsonInit('POST', { ...IDP_CREATE_BODY, name: 'FourFail' }),
        failEnv
      ),
    ]);
    expect(del.status).toBe(404);
    ...
    expect(userMiss.body.error).toBe('User not found');
    expect(roomMiss.status).toBe(404);
    expect(roomMiss.body.error).toBe('Room not found');
    expect(fail.status).toBe(500);
    expect(fail.body.error).toBe('Failed to create identity provider');
  });
  ...
  for (let i = 0; i < 12; i++) {
    it(`septenary admin exact race flood-${i}`, async () => {
      const env = createEnv();
      const failEnv = createEnv({ db: createAdminDb({ failIdpInsert: true }) });
      const results = await Promise.all([
        jsonReq('/admin/api/idp/providers', jsonInit('POST', { name: `m${i}` }), env),
        jsonReq(`/admin/api/idp/providers/flood-${i}`, jsonInit('DELETE'), env),
        jsonReq(`/admin/api/users/${encodeURIComponent(`@f${i}:example.com`)}`, {}, env),
        jsonReq(
          `/_synapse/admin/v1/rooms/${encodeURIComponent(`!f${i}:example.com`)}`,
          {},
          env
        ),
        jsonReq(
          ...
          jsonInit('POST', {
            ...IDP_CREATE_BODY,
            name: `FailFlood-${i}`,
            issuer_url: `https://flood${i}.example.com`,
          }),
          failEnv
        ),
      ]);
      expect(results[0].status).toBe(400);
      expect(results[0].body.error).toBe(
        'Missing required parameter: name, issuer_url, client_id, and client_secret are required'
      );
      ...
      expect(results[2].body.error).toBe('User not found');
      expect(results[3].status).toBe(404);
      expect(results[3].body.error).toBe('Room not found');
      expect(results[4].status).toBe(500);
      expect(results[4].body.error).toBe('Failed to create identity provider');
    });
  }
});
