...
/**
 * TOKENMAXX HEAVY leftovers after #353 / tip c8c1af7 (#364/#366) —
 * RoomDurableObject hibernation *septendecenary* relaunch of closed #370
 * (timeout ""→0 / "-5" past / "60000" stays, Upgrade "WebSocket " trailing
 * 426∥ping, GET /receipts∥HTTP receipt, WS read∥HTTP typing, webSocketError∥
 * HTTP receipt, broadcast∥WS close, receipt thread∥HTTP typing, JSON
 * "\"x\""/1.5 silent∥HTTP). Sexdecenary stopped at JSON true/0/{}.
 */

describe('RoomDurableObject hibernation septendecenary typing/upgrade leftovers after #353', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`typing timeout ""→0∥"-5" past∥"60000" stays∥peer flood-${i}`, async () => {
      vi.useFakeTimers();
      vi.setSystemTime(330_000);
      const { do: room } = makeRacingRoomDo();

      // "" → Math.min("",120000)=0 → already expired at GET
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
      ...
      const emptyTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(emptyTyping.user_ids).toEqual([]);

      // "-5" → Math.min("-5",120000)=-5 → expiresAt in the past
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@neg:example.com',
            typing: true,
            timeout: '-5',
          }),
        })
      );
      const negTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(negTyping.user_ids).toEqual([]);

      // "60000" → Math.min("60000",120000)=60000 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@str60:example.com',
            typing: true,
            timeout: '60000',
          }),
        })
      ...
      expect(typing.user_ids).toContain('@str60:example.com');
      expect(typing.user_ids).toContain('@a:example.com');
      expect(typing.user_ids).not.toContain('@empty:example.com');
      expect(typing.user_ids).not.toContain('@neg:example.com');
    });
  }

  for (let i = 0; i < 8; i++) {
    it(`Upgrade "WebSocket " trailing 426∥WS ping isolation flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      state.sockets.push(a);
  ...
  for (let i = 0; i < 8; i++) {
    it(`GET /receipts∥HTTP receipt peer isolation flood-${i}`, async () => {
      vi.useFakeTimers();
...
describe('RoomDurableObject hibernation septendecenary receipt/ws leftovers after #353', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`WS read∥HTTP typing both visible flood-${i}`, async () => {
      vi.useFakeTimers();
      ...
      await Promise.all([
        wsMsg(room, a, JSON.stringify({ type: 'read', event_id: '$s17-read' })),
        room.fetch(
          new Request('https://do/typing', {
            method: 'PUT',
            body: JSON.stringify({
  ...
  for (let i = 0; i < 8; i++) {
    it(`webSocketError∥HTTP receipt peer isolation flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      await Promise.all([
        wsError(room, a, new Error('s17-err')),
        room.fetch(
          new Request('https://do/receipt', {
            method: 'PUT',
            body: JSON.stringify({
              user_id: '@c:example.com',
              event_id: '$s17-err-rcpt',
              receipt_type: 'm.read',
            }),
          })
        ),
  ...
  for (let i = 0; i < 8; i++) {
    it(`broadcast∥webSocketClose both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      // b should see broadcast and/or disconnect notice
      expect(b.sent.length).toBeGreaterThan(0);
    });
  }

  for (let i = 0; i < 8; i++) {
    it(`receipt thread PUT∥HTTP typing both settle flood-${i}`, async () => {
      vi.useFakeTimers();
      ...
      expect(state.storage.map.has('receipt:@a:example.com:m.read:$root17')).toBe(true);
      const threaded = state.storage.map.get('receipt:@a:example.com:m.read:$root17') as {
        event_id: string;
      };
      expect(threaded.event_id).toBe('$s17-thr');
      const typing = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(typing.user_ids).toEqual(['@b:example.com']);
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON "\\"x\\""/1.5 WS silent∥HTTP typing flood-${i}`, async () => {
      vi.useFakeTimers();
      vi.setSystemTime(370_000);
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      state.sockets.push(a);
      ...
      await Promise.all([
        wsMsg(room, a, JSON.stringify('x')),
        wsMsg(room, a, JSON.stringify(1.5)),
        room.fetch(
          new Request('https://do/typing', {
            method: 'PUT',
            body: JSON.stringify({
      ...
      // string/number: truthy data but no .type → default silent
      expect(
        a.sent.every((s) => {
          const m = JSON.parse(s) as { type: string };
          return m.type === 'typing';
...
/**
 * TOKENMAXX HEAVY leftovers after #380 — RoomDurableObject hibernation
 * *octodenary* relaunch of closed #379 (timeout "abc"→0 / "120001" clamp /
 * "0" past, Upgrade WEBSOCKET 426∥ping, GET /state∥HTTP typing, WS ping∥HTTP
 * receipt, webSocketClose∥HTTP typing, broadcast∥HTTP receipt, receipt
 * thread∥webSocketError, JSON -1/"null" silent∥HTTP). Septendecenary stopped
 * at ""/"-5"/"60000" / "WebSocket " / JSON "x"/1.5.
 */

describe('RoomDurableObject hibernation octodenary typing/upgrade leftovers after #380', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`typing timeout "abc"→0∥"120001" clamp∥"0" past∥peer flood-${i}`, async () => {
      vi.useFakeTimers();
      vi.setSystemTime(370_000);
      const { do: room } = makeRacingRoomDo();

      // "abc" → Math.min("abc",120000)=NaN → expired at GET
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
      ...
      const abcTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(abcTyping.user_ids).toEqual([]);

      // "120001" → Math.min("120001",120000)=120000 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@clamp:example.com',
            typing: true,
            timeout: '120001',
          }),
        })
      ...
      // "0" → Math.min("0",120000)=0 → already expired
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
      ...
      const zeroTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(zeroTyping.user_ids).toContain('@clamp:example.com');
      expect(zeroTyping.user_ids).not.toContain('@zero:example.com');
      expect(zeroTyping.user_ids).not.toContain('@abc:example.com');
      ...
      expect(typing.user_ids).toContain('@clamp:example.com');
      expect(typing.user_ids).toContain('@a:example.com');
      expect(typing.user_ids).not.toContain('@abc:example.com');
      expect(typing.user_ids).not.toContain('@zero:example.com');
    });
  }
...
describe('RoomDurableObject hibernation octodenary receipt/ws leftovers after #380', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`broadcast∥HTTP receipt both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
  ...
  for (let i = 0; i < 8; i++) {
    it(`receipt thread PUT∥webSocketError both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      await Promise.all([
        ...
        wsError(room, a, new Error('s18-err')),
      ]);

      expect(state.storage.map.has('receipt:@a:example.com:m.read:$root18')).toBe(true);
      const threaded = state.storage.map.get('receipt:@a:example.com:m.read:$root18') as {
        event_id: string;
      };
      expect(threaded.event_id).toBe('$s18-thr');
      expect(
        (room as unknown as { sessions: Map<FakeWebSocket, unknown> }).sessions.has(a)
      ).toBe(false);
      expect(
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON -1/"null" WS silent∥HTTP typing flood-${i}`, async () => {
      ...
      await Promise.all([
        wsMsg(room, a, JSON.stringify(-1)),
        wsMsg(room, a, JSON.stringify('null')),
        room.fetch(
          new Request('https://do/typing', {
            method: 'PUT',
            body: JSON.stringify({
      ...
      // -1: truthy but no .type → silent; "null": string truthy but no .type → silent
      expect(a.sent.every((s) => {
        const m = JSON.parse(s) as { type: string };
        return m.type === 'typing';
      })).toBe(true);
...
/**
 * TOKENMAXX HEAVY leftovers after #387 — RoomDurableObject hibernation
 * *novemdenary* (timeout "-0"→expired / "120000" max stays / "1e5" stays,
 * Upgrade "WEBSOCKET " 426∥ping, GET /receipts∥WS ping, WS read∥HTTP receipt,
 * webSocketClose∥HTTP receipt, broadcast∥WS ping, receipt private∥webSocketError,
 * JSON Infinity/NaN silent∥HTTP). Octodenary stopped at "abc"/"120001"/"0" /
 * WEBSOCKET / state∥typing / ping∥receipt / close∥typing / broadcast∥receipt /
 * thread∥error / JSON -1/"null".
 */

describe('RoomDurableObject hibernation novemdenary typing/upgrade leftovers after #387', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`typing timeout "-0"→expired∥"120000" max∥"1e5" stays∥peer flood-${i}`, async () => {
      vi.useFakeTimers();
      vi.setSystemTime(410_000);
      const { do: room } = makeRacingRoomDo();

      // "-0" → Math.min("-0",120000)=-0 → expiresAt===now → expired at GET
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@negzero:example.com',
            typing: true,
            timeout: '-0',
          }),
        })
      ...
      expect(negTyping.user_ids).not.toContain('@negzero:example.com');

      // "120000" → Math.min("120000",120000)=120000 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
      ...
      // "1e5" → Math.min("1e5",120000)=100000 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@exp:example.com',
            typing: true,
            timeout: '1e5',
          }),
        })
      );
      const midTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(midTyping.user_ids).toContain('@max:example.com');
      expect(midTyping.user_ids).toContain('@exp:example.com');
      expect(midTyping.user_ids).not.toContain('@negzero:example.com');
      ...
      expect(typing.user_ids).toContain('@max:example.com');
      expect(typing.user_ids).toContain('@exp:example.com');
      expect(typing.user_ids).toContain('@a:example.com');
      expect(typing.user_ids).not.toContain('@negzero:example.com');
    });
  }
  ...
  for (let i = 0; i < 8; i++) {
    it(`GET /receipts∥WS ping peer isolation flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      state.sockets.push(a);
      ...
      expect(body.receipts['$s19-seed']?.['m.read']?.['@seed:example.com']).toBeDefined();
      expect(a.sent).toContain(JSON.stringify({ type: 'pong' }));
    });
  }
});

describe('RoomDurableObject hibernation novemdenary receipt/ws leftovers after #387', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`WS read∥HTTP receipt both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      await Promise.all([
        wsMsg(room, a, JSON.stringify({ type: 'read', event_id: '$s19-ws-read' })),
        room.fetch(
          new Request('https://do/receipt', {
            method: 'PUT',
            body: JSON.stringify({
              user_id: '@c:example.com',
              event_id: '$s19-http-rcpt',
              receipt_type: 'm.read',
            }),
          })
        ),
  ...
  for (let i = 0; i < 8; i++) {
    it(`broadcast∥WS ping both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
  ...
  for (let i = 0; i < 8; i++) {
    it(`receipt private PUT∥webSocketError both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      await Promise.all([
        ...
        wsError(room, a, new Error('s19-err')),
      ]);

      expect(state.storage.map.has('receipt:@a:example.com:m.read.private:unthreaded')).toBe(
        true
      );
      const priv = state.storage.map.get(
        'receipt:@a:example.com:m.read.private:unthreaded'
      ) as { event_id: string };
      expect(priv.event_id).toBe('$s19-priv');
      expect(
        (room as unknown as { sessions: Map<FakeWebSocket, unknown> }).sessions.has(a)
      ).toBe(false);
      expect(
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON Infinity/NaN WS silent∥HTTP typing flood-${i}`, async () => {
      vi.useFakeTimers();
      ...
      // Infinity/NaN are invalid JSON → catch silent; only typing broadcast may arrive
      expect(
        a.sent.every((s) => {
          const m = JSON.parse(s) as { type: string };
          return m.type === 'typing';
 ...
 * private∥error / Infinity/NaN.
 */

describe('RoomDurableObject hibernation vicenary typing/upgrade leftovers after #400', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`typing timeout "+0"→expired∥"1e4" stays∥"119999" stays∥peer flood-${i}`, async () => {
      vi.useFakeTimers();
      ...
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@pluszero:example.com',
            typing: true,
            timeout: '+0',
          }),
        })
      );
      const plusTyping = (await (await room.fetch(new Request('https://do/typing'))).json()) as {
        user_ids: string[];
      };
      expect(plusTyping.user_ids).not.toContain('@pluszero:example.com');

      // "1e4" → Math.min("1e4",120000)=10000 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@exp4:example.com',
            typing: true,
            timeout: '1e4',
          }),
        })
      ...
      // "119999" → Math.min("119999",120000)=119999 → still active
      await room.fetch(
        new Request('https://do/typing', {
          method: 'PUT',
          body: JSON.stringify({
            user_id: '@nearmax:example.com',
            typing: true,
            timeout: '119999',
          }),
        })
      ...
      expect(midTyping.user_ids).toContain('@exp4:example.com');
      expect(midTyping.user_ids).toContain('@nearmax:example.com');
      expect(midTyping.user_ids).not.toContain('@pluszero:example.com');
      ...
      expect(typing.user_ids).toContain('@exp4:example.com');
      expect(typing.user_ids).toContain('@nearmax:example.com');
      expect(typing.user_ids).toContain('@a:example.com');
      expect(typing.user_ids).not.toContain('@pluszero:example.com');
    });
  }
  ...
  for (let i = 0; i < 8; i++) {
    it(`GET /state∥WS ping peer isolation flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      state.sockets.push(a);
      ...
      expect(stateRes.status).toBe(200);
      const body = (await stateRes.json()) as { connection_count: number };
      expect(body.connection_count).toBeGreaterThanOrEqual(1);
      expect(a.sent).toContain(JSON.stringify({ type: 'pong' }));
    });
  }
});

describe('RoomDurableObject hibernation vicenary receipt/ws leftovers after #400', () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  ...
  for (let i = 0; i < 8; i++) {
    it(`broadcast∥HTTP typing both visible flood-${i}`, async () => {
      ...
      expect(typing.user_ids).toContain('@t:example.com');
    });
  }

  for (let i = 0; i < 8; i++) {
    it(`receipt fully_read PUT∥webSocketError both settle flood-${i}`, async () => {
      const { state, do: room } = makeRacingRoomDo();
      const a = new FakeWebSocket();
      a.serializeAttachment({ userId: '@a:example.com', id: '1' });
      const b = new FakeWebSocket();
      ...
      await Promise.all([
        ...
        wsError(room, a, new Error('s20-err')),
      ]);

      expect(state.storage.map.has('receipt:@a:example.com:m.fully_read:unthreaded')).toBe(
        true
      );
      const fr = state.storage.map.get(
        'receipt:@a:example.com:m.fully_read:unthreaded'
      ) as { event_id: string };
      expect(fr.event_id).toBe('$s20-fully');
      expect(
        (room as unknown as { sessions: Map<FakeWebSocket, unknown> }).sessions.has(a)
      ).toBe(false);
      expect(
  ...
  for (let i = 0; i < 8; i++) {
    it(`JSON "undefined" WS silent∥HTTP typing flood-${i}`, async () => {
      vi.useFakeTimers();
      ...
      // "undefined" is invalid JSON → catch silent; only typing broadcast may arrive
      expect(
        a.sent.every((s) => {
          const m = JSON.parse(s) as { type: string };
          return m.type === 'typing';
