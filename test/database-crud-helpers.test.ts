...
describe('database CRUD TOKENMAXX residual senary leftovers after #315 tip-relaunch', () => {
  beforeEach(() => {
    vi.spyOn(Date, 'now').mockReturnValue(NOW);
  });
  afterEach(() => {
  ...
  it('createDevice ∥ deleteDevice same id: final state is absent or present coherently', async () => {
    const db = createCrudDb({
      devices: [
        {
          user_id: USER,
    ...
    await Promise.all([
      createDevice(db, USER, 'DEV-RACE', 'Phone2'),
      deleteDevice(db, USER, 'DEV-RACE'),
    ]);
    const got = await getDevice(db, USER, 'DEV-RACE');
    if (got) {
      expect(got).toMatchObject({ user_id: USER, device_id: 'DEV-RACE' });
    } else {
      expect(db._state.devices.filter((d) => d.device_id === 'DEV-RACE')).toHaveLength(0);
    }
  });

  it('getUserByLocalpart ∥ createUser sibling localpart stays isolated under race', async () => {
    const db = createCrudDb({
      users: [
        {
          user_id: USER,
    ...
    expect(got).toMatchObject({ user_id: USER, localpart: 'alice' });
  ...
  it('deleteRoomAlias ∥ createRoomAlias same alias: lookup coherent under race', async () => {
    const db = createCrudDb({
      aliases: [
        {
          alias: '#race:example.com',
    ...
    await Promise.all([
      deleteRoomAlias(db, '#race:example.com'),
      createRoomAlias(db, '#race:example.com', '!other:example.com', BOB),
    ]);
    const got = await getRoomByAlias(db, '#race:example.com');
    // delete-then-create → other; create-then-delete may wipe both or leave other
    if (got) {
      expect(['!r:example.com', '!other:example.com']).toContain(got);
    } else {
      expect(db._state.aliases.filter((a) => a.alias === '#race:example.com')).toHaveLength(0);
    }
  });

  it('getDevice missing ∥ createDevice: read sees null or new under race', async () => {
    const db = createCrudDb();
    const [got] = await Promise.all([
      getDevice(db, USER, 'NEWDEV'),
      createDevice(db, USER, 'NEWDEV', 'Tablet'),
    ]);
    expect(got === null || got.device_id === 'NEWDEV').toBe(true);
    await expect(getDevice(db, USER, 'NEWDEV')).resolves.toMatchObject({
      user_id: USER,
      device_id: 'NEWDEV',
      display_name: 'Tablet',
    });
  ...
  it('storeEvent soft-cap reject ∥ sibling ok insert stay isolated under race', async () => {
    const db = createCrudDb({ streamPosition: 1 });
    const overhead = JSON.stringify({ body: '' }).length;
    const soft = pdu({
      event_id: '$soft-rej',
      type: 'm.room.message',
      content: { body: 'z'.repeat(65_536 - overhead + 1) },
    });
    const ok = pdu({
      event_id: '$soft-ok',
      type: 'm.room.message',
    ...
    const [rej, stream] = await Promise.allSettled([
      storeEvent(db, soft),
      storeEvent(db, ok),
    ]);
    expect(rej.status).toBe('rejected');
    ...
    expect(stream.status).toBe('fulfilled');
    expect(db._state.events.map((e) => e.event_id)).toEqual(['$soft-ok']);
    expect(db._state.events.map((e) => e.event_id)).not.toContain('$soft-rej');
  });

  it('getMembership leave ∥ tryInsertJoinMembership upgrades under race', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [got, upgrade] = await Promise.all([
      getMembership(db, ROOM, USER),
      tryInsertJoinMembership(db, ROOM, USER, '$rejoin', 'Alice'),
    ]);
    expect(got).not.toBeNull();
    expect(['leave', 'join']).toContain(got!.membership);
    expect(upgrade.inserted).toBe(true);
    expect(upgrade.eventId).toBe('$rejoin');
    await expect(getMembership(db, ROOM, USER)).resolves.toEqual({
      membership: 'join',
      eventId: '$rejoin',
    });
  ...
  it('updateUserProfile display+avatar ∥ getPasswordHash hash stays stable under race', async () => {
    const db = createCrudDb({
      users: [
        {
          user_id: USER,
    ...
    expect(hash).toBe('stable-hash');
    await expect(getUserById(db, USER)).resolves.toMatchObject({
      display_name: 'New',
      avatar_url: 'mxc://example.com/new',
    });
    await expect(getPasswordHash(db, USER)).resolves.toBe('stable-hash');
  });

  it('createAccessToken ∥ deleteAllUserTokens: final lookup coherent under race', async () => {
    const db = createCrudDb();
    await Promise.all([
      createAccessToken(db, 'tid-1', 'hash-senary', USER, 'DEV1'),
      deleteAllUserTokens(db, USER),
    ]);
    const looked = await getUserByTokenHash(db, 'hash-senary');
    if (looked) {
      expect(looked).toEqual({ userId: USER, deviceId: 'DEV1' });
    } else {
