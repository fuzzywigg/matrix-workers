...
describe('database CRUD TOKENMAXX residual denary leftovers after #380', () => {
  beforeEach(() => {
    vi.spyOn(Date, 'now').mockReturnValue(NOW);
  });
  afterEach(() => {
  ...
  it('getUserById missing ∥ createUser: read null or settled under race', async () => {
    const db = createCrudDb();
    const [got] = await Promise.all([
      getUserById(db, USER),
      createUser(db, USER, 'alice', 'pbkdf2-denary', false),
    ]);
    expect(got === null || got?.user_id === USER).toBe(true);
    await expect(getUserById(db, USER)).resolves.toMatchObject({
      user_id: USER,
    ...
    await expect(getPasswordHash(db, USER)).resolves.toBe('pbkdf2-denary');
  });

  it('getRoom missing ∥ createRoom: lookup null or room under race', async () => {
    const db = createCrudDb();
    const [got] = await Promise.all([
      getRoom(db, ROOM),
      createRoom(db, ROOM, '10', USER, false),
    ]);
    expect(got === null || got?.room_id === ROOM).toBe(true);
    await expect(getRoom(db, ROOM)).resolves.toMatchObject({
      room_id: ROOM,
      room_version: '10',
      creator_id: USER,
      is_public: false,
  ...
  it('getRoomEvents(f) ∥ storeEvent: page length old or includes new only', async () => {
    const a = pdu({ event_id: '$a-den', type: 'm.room.message', content: { body: 'a' } });
    const db = createCrudDb({
      events: [eventRowFromPdu(a, 1)],
      streamPosition: 1,
    });
    const neu = pdu({ event_id: '$b-den', type: 'm.room.message', content: { body: 'b' } });
    const [page, stream] = await Promise.all([
      getRoomEvents(db, ROOM, undefined, 10, 'f'),
      storeEvent(db, neu),
    ]);
    ...
    expect(page.events.length).toBeLessThanOrEqual(2);
    expect(
      page.events.every((e) => e.event_id === '$a-den' || e.event_id === '$b-den')
    ).toBe(true);
    await expect(getEvent(db, '$b-den')).resolves.toMatchObject({ event_id: '$b-den' });
  });

  it('getUserRooms(leave) ∥ updateMembership leave→join: page sees room or empty', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [rooms] = await Promise.all([
      getUserRooms(db, USER, 'leave'),
      updateMembership(db, ROOM, USER, 'join', '$join-den'),
    ]);
    expect(rooms.length).toBeLessThanOrEqual(1);
    expect(rooms.every((r) => r === ROOM)).toBe(true);
    await expect(getUserRooms(db, USER, 'leave')).resolves.toEqual([]);
    await expect(getUserRooms(db, USER, 'join')).resolves.toEqual([ROOM]);
  });

  it('getStateEvent ∥ storeEventIdempotent topic: read old or new only', async () => {
    const old = pdu({
      event_id: '$old-topic',
      type: 'm.room.topic',
      state_key: '',
    ...
    const db = createCrudDb({
      events: [eventRowFromPdu(old, 1)],
      roomState: [
        { room_id: ROOM, event_type: 'm.room.topic', state_key: '', event_id: '$old-topic' },
      ],
      streamPosition: 1,
    });
    const neu = pdu({
    ...
    const [got, insert] = await Promise.all([
      getStateEvent(db, ROOM, 'm.room.topic', ''),
      storeEventIdempotent(db, neu),
    ]);
    expect(insert.inserted).toBe(true);
    expect(got).not.toBeNull();
    expect(['$old-topic', '$new-topic']).toContain(got!.event_id);
    await expect(getStateEvent(db, ROOM, 'm.room.topic', '')).resolves.toMatchObject({
      event_id: '$new-topic',
      content: { topic: 'New' },
    });
  });
  ...
  it('deleteAccessToken ∥ getUserByTokenHash: read sees token or null under race', async () => {
    ...
    const [got] = await Promise.all([
      getUserByTokenHash(db, 'hash-denary'),
      deleteAccessToken(db, 'hash-denary'),
    ]);
    expect(got === null || got?.userId === USER).toBe(true);
    await expect(getUserByTokenHash(db, 'hash-denary')).resolves.toBeNull();
  });

  it('tryInsertJoinMembership already-join KEEP ∥ getMembership under race', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [got, upgrade] = await Promise.all([
      getMembership(db, ROOM, USER),
      tryInsertJoinMembership(db, ROOM, USER, '$new-join-den', 'Alice'),
    ]);
    expect(upgrade.inserted).toBe(false);
    expect(upgrade.eventId).toBe('$keep-join');
    expect(got).toEqual({ membership: 'join', eventId: '$keep-join' });
    await expect(getMembership(db, ROOM, USER)).resolves.toEqual({
      membership: 'join',
      eventId: '$keep-join',
    });
  ...
  it('getRoomMembers(knock) ∥ updateMembership knock→join: filter sees knock or empty', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [knocked] = await Promise.all([
      getRoomMembers(db, ROOM, 'knock'),
      updateMembership(db, ROOM, USER, 'join', '$join-knock'),
    ]);
    expect(knocked.length).toBeLessThanOrEqual(1);
    expect(knocked.every((m) => m.membership === 'knock')).toBe(true);
    await expect(getRoomMembers(db, ROOM, 'knock')).resolves.toEqual([]);
    await expect(getMembership(db, ROOM, USER)).resolves.toEqual({
      membership: 'join',
      eventId: '$join-knock',
    });
  ...
  it('createDevice ∥ getDevice missing: read null or device; sibling KEEP untouched', async () => {
    const db = createCrudDb({
      devices: [
        {
          user_id: USER,
    ...
    const [got] = await Promise.all([
      getDevice(db, USER, 'NEW-DEN'),
      createDevice(db, USER, 'NEW-DEN', 'Phone'),
    ]);
    expect(got === null || got?.device_id === 'NEW-DEN').toBe(true);
    await expect(getDevice(db, USER, 'NEW-DEN')).resolves.toMatchObject({
      device_id: 'NEW-DEN',
      display_name: 'Phone',
    });
    await expect(getDevice(db, USER, 'KEEP')).resolves.toMatchObject({
...
describe('database CRUD TOKENMAXX residual undecenary leftovers after #395', () => {
  beforeEach(() => {
    vi.spyOn(Date, 'now').mockReturnValue(NOW);
  });
  afterEach(() => {
  ...
  it('getPasswordHash missing ∥ createUser: hash null or settled under race', async () => {
    const db = createCrudDb();
    const [hash] = await Promise.all([
      getPasswordHash(db, USER),
      createUser(db, USER, 'alice', 'pbkdf2-undecenary', false),
    ]);
    expect(hash === null || hash === 'pbkdf2-undecenary').toBe(true);
    await expect(getPasswordHash(db, USER)).resolves.toBe('pbkdf2-undecenary');
    await expect(getUserByLocalpart(db, 'alice')).resolves.toMatchObject({
      user_id: USER,
    });
  });

  it('deleteRoomAlias ∥ getRoomByAlias: lookup sees room or null under race', async () => {
    const db = createCrudDb({
      aliases: [
        {
          alias: '#und:example.com',
          room_id: ROOM,
          creator_id: USER,
          created_at: NOW,
        },
    ...
    const [got] = await Promise.all([
      getRoomByAlias(db, '#und:example.com'),
      deleteRoomAlias(db, '#und:example.com'),
    ]);
    expect(got === null || got === ROOM).toBe(true);
    await expect(getRoomByAlias(db, '#und:example.com')).resolves.toBeNull();
  });

  it('getRoomEvents(b) ∥ storeEvent: page length old or includes new only', async () => {
    const a = pdu({ event_id: '$a-und', type: 'm.room.message', content: { body: 'a' } });
    const db = createCrudDb({
      events: [eventRowFromPdu(a, 1)],
      streamPosition: 1,
    });
    const neu = pdu({ event_id: '$b-und', type: 'm.room.message', content: { body: 'b' } });
    const [page, stream] = await Promise.all([
      getRoomEvents(db, ROOM, undefined, 10, 'b'),
      storeEvent(db, neu),
    ]);
    ...
    expect(page.events.length).toBeLessThanOrEqual(2);
    expect(
      page.events.every((e) => e.event_id === '$a-und' || e.event_id === '$b-und')
    ).toBe(true);
    await expect(getEvent(db, '$b-und')).resolves.toMatchObject({ event_id: '$b-und' });
  });

  it('getUserRooms(invite) ∥ updateMembership invite→join: page sees room or empty', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [rooms] = await Promise.all([
      getUserRooms(db, USER, 'invite'),
      updateMembership(db, ROOM, USER, 'join', '$join-und'),
    ]);
    expect(rooms.length).toBeLessThanOrEqual(1);
    expect(rooms.every((r) => r === ROOM)).toBe(true);
    await expect(getUserRooms(db, USER, 'invite')).resolves.toEqual([]);
  ...
  it('createAccessToken ∥ getUserByTokenHash missing: read sees null or token', async () => {
    const db = createCrudDb();
    const [got] = await Promise.all([
      getUserByTokenHash(db, 'hash-undecenary'),
      createAccessToken(db, 'tid-und', 'hash-undecenary', USER, 'DEV-UND'),
    ]);
    expect(got === null || got?.userId === USER).toBe(true);
    await expect(getUserByTokenHash(db, 'hash-undecenary')).resolves.toEqual({
      userId: USER,
      deviceId: 'DEV-UND',
    });
  });

  it('updateUserProfile display-only ∥ getUserById: avatar stays; display old or new', async () => {
    const db = createCrudDb({
      users: [
        {
          user_id: USER,
    ...
    const [got] = await Promise.all([
      getUserById(db, USER),
      updateUserProfile(db, USER, 'Alice Undecenary', undefined),
    ]);
    expect(got).not.toBeNull();
    expect(got!.avatar_url).toBe('mxc://example.com/av');
    expect(['Alice', 'Alice Undecenary']).toContain(got!.display_name);
    await expect(getUserById(db, USER)).resolves.toMatchObject({
      display_name: 'Alice Undecenary',
      avatar_url: 'mxc://example.com/av',
    });
  });

  it('getEventsSince ∥ storeEventIdempotent duplicate: page/idempotent stay coherent', async () => {
    const seed = pdu({ event_id: '$seed-und', type: 'm.room.message', content: { body: 's' } });
    const db = createCrudDb({
      events: [eventRowFromPdu(seed, 3)],
      streamPosition: 3,
    });
    const dup = pdu({ event_id: '$seed-und', type: 'm.room.message', content: { body: 's' } });
    const [page, insert] = await Promise.all([
      getEventsSince(db, ROOM, 3, 10),
      storeEventIdempotent(db, dup),
    ]);
    expect(insert.inserted).toBe(false);
    expect(page).toEqual([]);
    await expect(getLatestStreamPosition(db)).resolves.toBe(3);
    await expect(getEvent(db, '$seed-und')).resolves.toMatchObject({ event_id: '$seed-und' });
  });

  it('tryInsertJoinMembership invite→join ∥ getRoomMembers(join): filter old or new', async () => {
    const db = createCrudDb({
      memberships: [
        {
          room_id: ROOM,
    ...
    const [joined, upgrade] = await Promise.all([
      getRoomMembers(db, ROOM, 'join'),
      tryInsertJoinMembership(db, ROOM, USER, '$join-und', 'Alice'),
    ]);
    expect(upgrade.inserted).toBe(true);
    expect(upgrade.eventId).toBe('$join-und');
  ...
  it('getDevice ∥ deleteDevice: read sees device or null; sibling KEEP untouched', async () => {
    const db = createCrudDb({
      devices: [
        {
          user_id: USER,
    ...
    const [got] = await Promise.all([
      getDevice(db, USER, 'DROP'),
      deleteDevice(db, USER, 'DROP'),
    ]);
    expect(got === null || got?.device_id === 'DROP').toBe(true);
    await expect(getDevice(db, USER, 'DROP')).resolves.toBeNull();
    await expect(getDevice(db, USER, 'KEEP')).resolves.toMatchObject({
      device_id: 'KEEP',
      display_name: 'Laptop',
    });
