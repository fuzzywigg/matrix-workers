/**
 * TOKENMAXX HEAVY leftovers after #317 — senary *admin* soft+concurrent-race
 * niches unsaturated by quinary create/keys/param exact (#317 / tip after #315–#319).
 *
 * Distinct from quinary (#317) create/keys-No-sig/param softs:
 *   Report not found resolve∥unresolve exact under soft+race;
 *   Admin access required ∥ Admin privileges required gate exact;
 *   login-token User not found ∥ rooms Room not found exact;
 *   IdP PUT discovery fail exact (quinary only POST create discovery);
 *   Identity provider not found GET∥DELETE under race with PUT discovery;
 *   keys Verified ∥ No self-signing ∥ No signature in DB ∥
 *     Signature not in device key object four-way under one Promise.all.
 *
...
describe('soft senary admin report/auth/404/IdP exact strings after #317', () => {
  for (let i = 0; i < 12; i++) {
    it(`resolve Report not found exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/admin/api/reports/999${i}/resolve`,
  ...
  for (let i = 0; i < 12; i++) {
    it(`unresolve Report not found exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        `/admin/api/reports/888${i}/unresolve`,
        jsonInit('POST', {}),
        env
      );
      expect(res.status).toBe(404);
  ...
  for (let i = 0; i < 12; i++) {
    it(`Admin access required exact flood-${i}`, async () => {
      authState.userId = undefined;
      const env = createEnv();
      const res = await jsonReq('/admin/api/registration', {}, env);
      expect(res.status).toBe(401);
      expect(res.body.errcode).toBe('M_UNAUTHORIZED');
      expect(res.body.error).toBe('Admin access required');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`Admin privileges required exact flood-${i}`, async () => {
  ...
  for (let i = 0; i < 12; i++) {
    it(`login-token User not found exact flood-${i}`, async () => {
      const env = createEnv();
      const missing = `@ghost${i}:example.com`;
      const res = await jsonReq(
        `/admin/api/users/${encodeURIComponent(missing)}/login-token`,
  ...
  for (let i = 0; i < 12; i++) {
    it(`rooms Room not found exact flood-${i}`, async () => {
      const env = createEnv();
      const missing = `!missing${i}:example.com`;
      const res = await jsonReq(`/admin/api/rooms/${encodeURIComponent(missing)}`, {}, env);
      expect(res.status).toBe(404);
      expect(res.body.errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`IdP PUT discovery fail exact soft flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/idp/providers/idp1',
        jsonInit('PUT', { issuer_url: 'https://bad-issuer.example.com' }),
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`Identity provider not found GET exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(`/admin/api/idp/providers/missing-${i}`, {}, env);
      expect(res.status).toBe(404);
      expect(res.body.errcode).toBe('M_NOT_FOUND');
...
describe('race senary admin report/auth/404/keys four-way after #317', () => {
  it('resolve∥unresolve Report not found isolation', async () => {
    const env = createEnv();
    const [resolve, unresolve] = await Promise.all([
      jsonReq('/admin/api/reports/999/resolve', jsonInit('POST', { note: 'x' }), env),
      jsonReq('/admin/api/reports/998/unresolve', jsonInit('POST', {}), env),
    ]);
    expect(statusesOf([resolve, unresolve])).toEqual([404, 404]);
    expect(resolve.body.error).toBe('Report not found');
    expect(unresolve.body.error).toBe('Report not found');
  });

  it('Report not found∥resolve ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq('/admin/api/reports/40404/resolve', jsonInit('POST', { note: 'nope' }), env),
      jsonReq('/admin/api/reports/1/resolve', jsonInit('POST', { note: 'done' }), env),
    ]);
    expect(miss.status).toBe(404);
    expect(miss.body.error).toBe('Report not found');
    expect(ok.status).toBe(200);
    expect(ok.body.success).toBe(true);
  });

  it('Admin access required dual registration∥stats isolation', async () => {
    authState.userId = undefined;
    const env = createEnv();
    ...
    expect(a.body.error).toBe('Admin access required');
    expect(b.body.error).toBe('Admin access required');
  });

  it('Admin privileges required dual registration∥stats isolation', async () => {
    authState.userId = BOB;
    const env = createEnv();
    const [a, b] = await Promise.all([
      jsonReq('/admin/api/registration', {}, env),
    ...
    expect(statusesOf([a, b])).toEqual([403, 403]);
    expect(a.body.error).toBe('Admin privileges required');
    expect(b.body.error).toBe('Admin privileges required');
  });

  it('login-token User not found∥Room not found isolation', async () => {
    const env = createEnv();
    const [userMiss, roomMiss] = await Promise.all([
      jsonReq(
        `/admin/api/users/${encodeURIComponent('@ghost:example.com')}/login-token`,
        jsonInit('POST', { ttl_minutes: 5 }),
        env
      ),
      jsonReq(`/admin/api/rooms/${encodeURIComponent('!ghost:example.com')}`, {}, env),
    ]);
    expect(statusesOf([userMiss, roomMiss])).toEqual([404, 404]);
    expect(userMiss.body.error).toBe('User not found');
    expect(roomMiss.body.error).toBe('Room not found');
  });

  it('login-token User not found∥mint ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq(
        `/admin/api/users/${encodeURIComponent('@nouser:example.com')}/login-token`,
        jsonInit('POST', {}),
        env
      ),
      jsonReq(
    ...
    expect(miss.status).toBe(404);
    expect(miss.body.error).toBe('User not found');
    expect(ok.status).toBe(200);
    expect(ok.body.user_id).toBe(BOB);
    expect(ok.body.token).toBe('mlt_pinned_login_token');
  });

  it('Room not found∥room detail ok isolation', async () => {
    const env = createEnv();
    const [miss, ok] = await Promise.all([
      jsonReq(`/admin/api/rooms/${encodeURIComponent('!nope:example.com')}`, {}, env),
      jsonReq(`/admin/api/rooms/${encodeURIComponent(ROOM)}`, {}, env),
    ]);
    expect(miss.status).toBe(404);
    expect(miss.body.error).toBe('Room not found');
    expect(ok.status).toBe(200);
    expect(ok.body.room_id).toBe(ROOM);
  });

  it('IdP PUT discovery fail∥GET Identity provider not found isolation', async () => {
    const env = createEnv();
    const [disc, miss] = await Promise.all([
      jsonReq(
        '/admin/api/idp/providers/idp1',
        jsonInit('PUT', { issuer_url: 'https://bad-issuer.example.com' }),
        env
      ),
      jsonReq('/admin/api/idp/providers/no-such-idp', {}, env),
    ]);
    expect(disc.status).toBe(400);
    expect(disc.body.error).toBe(
      'Failed to fetch OIDC discovery from issuer. Check the issuer URL is correct and accessible.'
    );
    expect(miss.status).toBe(404);
  ...
  it('IdP DELETE not found∥PUT discovery fail isolation', async () => {
    const env = createEnv();
    const [del, disc] = await Promise.all([
      jsonReq('/admin/api/idp/providers/gone', jsonInit('DELETE'), env),
      jsonReq(
        '/admin/api/idp/providers/idp1',
        jsonInit('PUT', { issuer_url: 'https://bad-issuer.example.com/upd' }),
        env
      ),
    ]);
    expect(del.status).toBe(404);
    expect(del.body.error).toBe('Identity provider not found');
    expect(disc.status).toBe(400);
    expect(disc.body.error).toBe(
      'Failed to fetch OIDC discovery from issuer. Check the issuer URL is correct and accessible.'
    );
  ...
  it('keys Verified∥No self-signing∥No signature in DB∥Signature not in device key four-way', async () => {
    const verifiedEnv = createEnv();

    const noSsDb = createAdminDb({
      users: [
        defaultAdmin(),
        { ...defaultBob(), user_id: '@carol:example.com', localpart: 'carol' },
      ],
      devices: [
        {
          user_id: ADMIN,
    ...
    const noSsEnv = createEnv({
      db: noSsDb,
      deviceKeys: mockKv({
        [`device:@carol:example.com:CAROLDEVICE`]: JSON.stringify({
          device_id: 'CAROLDEVICE',
          user_id: '@carol:example.com',
          keys: {},
        }),
    ...
    const noDbSigEnv = createEnv({ db: createAdminDb({ crossSigningSigs: [] }) });

    const badObjEnv = createEnv({
      deviceKeys: mockKv({
        [`device:${BOB}:BOBDEVICE`]: JSON.stringify({
          algorithms: ['m.olm.v1.curve25519-aes-sha2'],
          device_id: 'BOBDEVICE',
    ...
    const [verified, noSs, noDb, noObj] = await Promise.all([
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, verifiedEnv),
      jsonReq(
        `/admin/api/users/${encodeURIComponent('@carol:example.com')}/keys`,
        {},
        noSsEnv
      ),
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, noDbSigEnv),
      jsonReq(`/admin/api/users/${encodeURIComponent(BOB)}/keys`, {}, badObjEnv),
    ]);
    expect(statusesOf([verified, noSs, noDb, noObj])).toEqual([200, 200, 200, 200]);
    expect(
      (verified.body.verification_status as Record<string, { reason: string }>).BOBDEVICE.reason
    ).toBe('Verified');
    expect(
      (noSs.body.verification_status as Record<string, { reason: string }>).CAROLDEVICE.reason
  ...
  for (let i = 0; i < 12; i++) {
    it(`senary admin exact race flood-${i}`, async () => {
      const env = createEnv();
      const results = await Promise.all([
        jsonReq(`/admin/api/reports/7${i}/resolve`, jsonInit('POST', {}), env),
        jsonReq(
          `/admin/api/users/${encodeURIComponent(`@g${i}:example.com`)}/login-token`,
          jsonInit('POST', {}),
          env
        ),
        jsonReq(`/admin/api/rooms/${encodeURIComponent(`!g${i}:example.com`)}`, {}, env),
        jsonReq(`/admin/api/idp/providers/miss-${i}`, {}, env),
      ]);
      expect(statusesOf(results)).toEqual([404, 404, 404, 404]);
      expect(results[0].body.error).toBe('Report not found');
      expect(results[1].body.error).toBe('User not found');
      expect(results[2].body.error).toBe('Room not found');
      expect(results[3].body.error).toBe('Identity provider not found');
    });
  }
});
