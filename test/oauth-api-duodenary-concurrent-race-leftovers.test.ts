/**
 * TOKENMAXX HEAVY leftovers after #324 undecenary / tip past #315 — oauth
 * *duodenary* concurrent-race niches undecenary never mixed under Promise.all:
 *   login Invalid (NO period) ∥ Request Cancelled ∥ Missing Session ∥
 *     default approval desc (undecenary sensitive∥period∥no-period∥Approved;
 *     denary default∥cross-signing∥Expired∥Cancelled without login path),
 *   Authorization request expired JSON ∥ login Missing username ∥ UIA
 *     Invalid period ∥ Request Approved (denary expired∥Username required∥
 *     Missing; undecenary period∥Approved without expired+Missing username),
 *   OIDC-only Request Approved ∥ login no-period ∥ Cancelled ∥ Username
 *     required (undecenary OIDC∥mismatch∥sensitive∥parse; denary
 *     mismatch∥login without OIDC approve+cancel+Username required).
 *
 * Gap table (why leftover after #324):
 *   login no-period ∥ Cancelled ∥ Missing ∥ default approval
 *     | undecenary no-period with sensitive/Approved; denary Cancel/default without login
 *   expired JSON ∥ Missing username ∥ UIA period ∥ Approved
 *     | denary expired∥Missing without period/Approved
 *   OIDC approve ∥ login no-period ∥ Cancelled ∥ Username required
 *     | undecenary OIDC mega without login+cancel+Username required
 *
 * Distinct from #324 oauth undecenary, #315 denary, #306 nonary.
...
// ---------------------------------------------------------------------------
// login no-period ∥ Cancelled ∥ Missing Session ∥ default approval
// ---------------------------------------------------------------------------

describe('duodenary oauth no-period∥Cancelled∥Missing∥default after #324', () => {
  it('login no-period ∥ Request Cancelled ∥ Missing Session ∥ default approval under race', async () => {
    const cache = mockKv();
  ...
  it('no-period ∥ Cancelled ∥ Missing ∥ default ∥ credentials-desc penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-cancel');
    seedUiaSession(cache, 'uia-def');
  ...
  for (let i = 0; i < 8; i++) {
    it(`no-period ∥ Cancelled ∥ Missing ∥ default flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
...
// ---------------------------------------------------------------------------
// expired JSON ∥ Missing username ∥ UIA period ∥ Request Approved
// ---------------------------------------------------------------------------

describe('duodenary oauth expired∥Missing∥period∥Approved after #324', () => {
  it('Authorization request expired ∥ Missing username ∥ UIA period ∥ Request Approved under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    ...
    const results = await Promise.all([
      ...
      request(
        '/oauth/authorize',
        formInit({ auth_request_id: 'ar-miss' }),
        env
      ),
      request(
        '/oauth/authorize/uia',
    ...
    expect(String(results[1].body)).toContain(LOGIN_MISSING_CREDS);
    const htmls = htmlOf(results.slice(2));
    expect(hasUiaPeriod(htmls)).toBe(true);
    expect(htmls.some((h) => h.includes('Request Approved'))).toBe(true);
  });

  it('expired ∥ Missing ∥ period ∥ Approved ∥ Session Expired penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-bad');
    seedUiaSession(cache, 'uia-ok');
  ...
  for (let i = 0; i < 8; i++) {
    it(`expired ∥ Missing ∥ period ∥ Approved flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      ...
      const results = await Promise.all([
        ...
        request(
          '/oauth/authorize',
          formInit({ auth_request_id: `ar-miss-${i}` }),
          env
        ),
        request(
          '/oauth/authorize/uia',
...
// ---------------------------------------------------------------------------
// OIDC-only approve ∥ login no-period ∥ Cancelled ∥ Username required
// ---------------------------------------------------------------------------

describe('duodenary oauth OIDC-approve∥no-period∥Cancelled∥Username after #324', () => {
  it('OIDC-only Request Approved ∥ login no-period ∥ Cancelled ∥ Username required under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    ...
    expect(oidcStages).toEqual(
      expect.arrayContaining(['m.oauth', 'm.login.oauth', 'org.matrix.cross_signing_reset'])
    );
  });

  it('OIDC approve ∥ no-period ∥ Cancelled ∥ Username ∥ parse penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
      .mockRejectedValueOnce(new Error('duodenary-parse'));
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
    seedUiaSession(cache, 'uia-cancel');
  ...
  for (let i = 0; i < 8; i++) {
    it(`OIDC approve ∥ no-period ∥ Cancelled ∥ Username flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
  ...
  it('no-period∥Cancelled∥expired∥OIDC-approve∥Missing mega race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
    seedUiaSession(cache, 'uia-cancel');
