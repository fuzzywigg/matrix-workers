/**
 * TOKENMAXX HEAVY tip-relaunch after #332/#333/#334/#338 — oauth
 * *duodenary* concurrent-race niches undecenary (#324) never mixed under
 * Promise.all (fresh after closed conflicting #339):
 *   account-mismatch exact ∥ Session Expired ∥ Missing Session ∥
 *     Request Approved (denary Expired∥Cancelled∥default without
 *     mismatch+Missing+Approved; septenary mismatch∥approve only),
 *   cross-signing desc ∥ sensitive warning ∥ login Missing username ∥
 *     UIA Username required (denary cross-signing∥Expired∥Cancelled;
 *     undecenary sensitive without cross-signing+Missing+Username),
 *   Request Cancelled ∥ OIDC-only Approved ∥ UIA period ∥ login
 *     no-period (undecenary OIDC∥mismatch∥sensitive∥parse; never
 *     Cancelled∥OIDC∥period∥no-period).
 *
 * Gap table (why leftover after #324 / tip past #338):
 *   mismatch ∥ Expired ∥ Missing ∥ Approved
 *     | denary/septenary/senary pairs only
 *   cross-signing ∥ sensitive ∥ Missing username ∥ Username required
 *     | denary cross-signing quad; undecenary sensitive without these
 *   Cancelled ∥ OIDC approve ∥ period ∥ no-period
 *     | undecenary OIDC mega without Cancelled+period pair
 *
 * Distinct from #324 undecenary, #315 denary, #306 nonary.
