...
describe('notify / auth-chain TOKENMAXX residual octonary leftovers after #356', () => {
  beforeEach(() => {
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
  ...
  it('first Sync DO fail among three ∥ sibling room isolates fan-out', async () => {
    const env = makeNotifyEnv(
      {
        '!r:example.com': ['@a:example.com', '@b:example.com', '@c:example.com'],
        '!ok:example.com': ['@z:example.com'],
      },
      { failUsers: new Set(['@a:example.com']) }
    );
    await Promise.all([
      notifyUsersOfEvent(env, '!r:example.com', '$first', 'm.room.message'),
      notifyUsersOfEvent(env, '!ok:example.com', '$ok', 'm.room.member'),
    ]);
    expect(env.notifies.map((n: { userId: string }) => n.userId).sort()).toEqual([
      '@b:example.com',
  ...
  it('last Sync DO fail among three ∥ sibling isolates fan-out', async () => {
    const env = makeNotifyEnv(
      {
        '!r:example.com': ['@a:example.com', '@b:example.com', '@c:example.com'],
        '!ok:example.com': ['@z:example.com'],
      },
      { failUsers: new Set(['@c:example.com']) }
    );
    await Promise.all([
      notifyUsersOfEvent(env, '!r:example.com', '$last', 'm.room.message'),
      notifyUsersOfEvent(env, '!ok:example.com', '$ok', 'm.room.member'),
    ]);
    expect(env.notifies.map((n: { userId: string }) => n.userId).sort()).toEqual([
      '@a:example.com',
    ...
    expect(console.error).toHaveBeenCalledWith(
      '[database] Failed to notify user @c:example.com of event:',
      expect.any(Error)
    );
  });

  it('getStateAtEvent empty auth_events ∥ populated sibling stay isolated', async () => {
    const events = new Map<string, PDU>([
      [
        '$empty-auth',
        pdu('$empty-auth', [], {
    ...
    expect(populated[0].type).toBe('m.room.create');
  });

  it('getAuthChain duplicate root ids ∥ short sibling stay isolated', async () => {
    const events = new Map<string, PDU>([
      ['$root', pdu('$root', [])],
      ['$child', pdu('$child', ['$root'])],
    ]);
    const db = createAuthChainDb(events);
    const [dups, short] = await Promise.all([
      getAuthChain(db, ['$child', '$child', '$root']),
      getAuthChain(db, ['$root']),
    ]);
    expect(dups.map((e) => e.event_id).sort()).toEqual(['$child', '$root'].sort());
    expect(short.map((e) => e.event_id)).toEqual(['$root']);
  });

  it('getServersInRoomsWithUser no-colon subject ∥ multi under race', async () => {
    const binds: unknown[][] = [];
    const db = {
      prepare() {
        return {
    ...
    const [nocolon, multi] = await Promise.all([
      getServersInRoomsWithUser(db, '@nocolon'),
      getServersInRoomsWithUser(db, '@multi:example.com'),
    ]);
  ...
  it('notify two rooms with overlapping members fan-out distinct event_ids', async () => {
    const env = makeNotifyEnv({
      '!r1:example.com': ['@shared:example.com', '@only1:example.com'],
      '!r2:example.com': ['@shared:example.com', '@only2:example.com'],
    });
    await Promise.all([
      notifyUsersOfEvent(env, '!r1:example.com', '$e1', 'm.room.message'),
      notifyUsersOfEvent(env, '!r2:example.com', '$e2', 'm.room.member'),
    ...
    expect(byEvent['$e1'].sort()).toEqual(['@only1:example.com', '@shared:example.com']);
    expect(byEvent['$e2'].sort()).toEqual(['@only2:example.com', '@shared:example.com']);
  });
});

describe('notify / auth-chain TOKENMAXX residual denary leftovers after #380', () => {
  beforeEach(() => {
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
  ...
  it('getStateAtEvent missing leaf ∥ populated sibling stay isolated', async () => {
    const events = new Map<string, PDU>([
      [
        '$create',
        pdu('$create', [], {
    ...
    delete (events.get('$leaf') as { state_key?: string }).state_key;
    const db = createAuthChainDb(events);
    const [missing, populated] = await Promise.all([
      getStateAtEvent(db, '$gone'),
      getStateAtEvent(db, '$leaf'),
    ]);
    expect(missing).toEqual([]);
    expect(populated).toHaveLength(1);
  ...
  it('getAuthChain unknown id ∥ short root stay isolated', async () => {
    const events = new Map<string, PDU>([['$root', pdu('$root', [])]]);
    const db = createAuthChainDb(events);
    const [unknown, root] = await Promise.all([
      getAuthChain(db, ['$missing-den']),
      getAuthChain(db, ['$root']),
    ]);
    expect(unknown).toEqual([]);
    expect(root.map((e) => e.event_id)).toEqual(['$root']);
  });

  it('getServersInRoomsWithUser empty-results subject ∥ multi under race', async () => {
    const binds: unknown[][] = [];
    const db = {
      prepare() {
        return {
  ...
  it('notify membership boom room ∥ sibling success; outer catch logs', async () => {
    const env = makeNotifyEnv(
      {
        '!boom:example.com': ['@a:example.com'],
        '!ok:example.com': ['@z:example.com'],
      },
      { throwOnRooms: new Set(['!boom:example.com']) }
    );
  ...
  it('notify three rooms fan-out distinct event_ids under race', async () => {
    const env = makeNotifyEnv({
      '!r1:example.com': ['@a:example.com'],
      '!r2:example.com': ['@b:example.com'],
      '!r3:example.com': ['@c:example.com'],
    });
    await Promise.all([
      notifyUsersOfEvent(env, '!r1:example.com', '$e1-den', 'm.room.message'),
      notifyUsersOfEvent(env, '!r2:example.com', '$e2-den', 'm.room.member'),
      notifyUsersOfEvent(env, '!r3:example.com', '$e3-den', 'm.room.reaction'),
    ]);
    expect(env.notifies).toHaveLength(3);
    const byEvent = env.notifies.reduce(
      (acc: Record<string, string[]>, n: { userId: string; body: { event_id: string } }) => {
        const id = n.body.event_id;
        acc[id] = acc[id] ?? [];
    ...
    expect(byEvent['$e1-den']).toEqual(['@a:example.com']);
    expect(byEvent['$e2-den']).toEqual(['@b:example.com']);
    expect(byEvent['$e3-den']).toEqual(['@c:example.com']);
  });
});

describe('notify / auth-chain TOKENMAXX residual undecenary leftovers after #395', () => {
  beforeEach(() => {
    vi.spyOn(console, 'warn').mockImplementation(() => {});
    vi.spyOn(console, 'log').mockImplementation(() => {});
    vi.spyOn(console, 'error').mockImplementation(() => {});
  ...
  it('two Sync DO fails among three ∥ sibling room isolates fan-out', async () => {
    const env = makeNotifyEnv(
      {
        '!r:example.com': ['@a:example.com', '@b:example.com', '@c:example.com'],
        '!ok:example.com': ['@z:example.com'],
      },
      { failUsers: new Set(['@a:example.com', '@c:example.com']) }
    );
    await Promise.all([
      notifyUsersOfEvent(env, '!r:example.com', '$two-fail', 'm.room.message'),
      notifyUsersOfEvent(env, '!ok:example.com', '$ok', 'm.room.member'),
    ]);
    expect(env.notifies.map((n: { userId: string }) => n.userId).sort()).toEqual([
      '@b:example.com',
  ...
  it('getAuthChain empty seed ∥ populated sibling stay isolated', async () => {
    const events = new Map<string, PDU>([
      ['$root', pdu('$root', [])],
      ['$child', pdu('$child', ['$root'])],
    ]);
    ...
    const [empty, populated] = await Promise.all([
      getAuthChain(db, []),
      getAuthChain(db, ['$child']),
    ]);
    expect(empty).toEqual([]);
    expect(populated.map((e) => e.event_id).sort()).toEqual(['$child', '$root'].sort());
  });

  it('getStateAtEvent missing leaf ∥ create present stay isolated', async () => {
    const events = new Map<string, PDU>([
      [
        '$create',
        pdu('$create', [], {
  ...
  it('getServersInRoomsWithUser empty-string subject ∥ multi under race', async () => {
    const binds: unknown[][] = [];
    const db = {
      prepare() {
        return {
          bind(...args: unknown[]) {
            ...
            return {
              async all<T>() {
                if (subject === '') {
                  return { results: [{ server_name: null }] as T[] };
                }
                if (subject === '@multi:example.com') {
                  return {
                    results: [
    ...
    const [emptySubj, multi] = await Promise.all([
      getServersInRoomsWithUser(db, ''),
      getServersInRoomsWithUser(db, '@multi:example.com'),
    ]);
    expect(emptySubj).toEqual([]);
    expect(multi).toEqual(['a.example.com', 'b.example.com']);
    expect(binds).toHaveLength(2);
  });

  it('notify zero-members ∥ membership boom outer-catch both isolate under race', async () => {
    const env = makeNotifyEnv(
      {
        '!empty:example.com': [],
        '!boom:example.com': ['@x:example.com'],
    ...
    await Promise.all([
      notifyUsersOfEvent(env, '!empty:example.com', '$empty', 'm.room.message'),
      notifyUsersOfEvent(env, '!boom:example.com', '$boom', 'm.room.member'),
    ]);
    expect(env.notifies).toEqual([]);
    expect(console.log).toHaveBeenCalledWith(
      '[database] Notifying',
  ...
  it('notify same room two events fan-out distinct event_ids under race', async () => {
    const env = makeNotifyEnv({
      '!r:example.com': ['@a:example.com', '@b:example.com'],
    });
    await Promise.all([
      notifyUsersOfEvent(env, '!r:example.com', '$e1-und', 'm.room.message'),
      notifyUsersOfEvent(env, '!r:example.com', '$e2-und', 'm.room.member'),
    ]);
    expect(env.notifies).toHaveLength(4);
    const byEvent = env.notifies.reduce(
      (acc: Record<string, string[]>, n: { userId: string; body: { event_id: string } }) => {
    ...
    expect(byEvent['$e1-und'].sort()).toEqual(['@a:example.com', '@b:example.com']);
    expect(byEvent['$e2-und'].sort()).toEqual(['@a:example.com', '@b:example.com']);
  });
});
