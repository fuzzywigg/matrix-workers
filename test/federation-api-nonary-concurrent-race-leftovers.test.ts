/**
 * TOKENMAXX HEAVY leftovers after #342 / tip #350 — nonary *federation* soft+concurrent-race
 * niches unsaturated by octonary late-3pid exact (#342). Skip FederationCatchupWorkflow
 * and knock (own leftovers file). Relaunch of closed #351 onto tip after #350; lands
 * closed septenary #335 niches that never merged.
 *
 * Distinct from senary (#328) device_keys/one_time_keys/3pid-early/openid triple:
 * Distinct from octonary (#342) late-3pid Room-not-found/No-token/Invalid-content/Unverifiable:
 *   invite exact Event is not an invite / Invalid state_key / User is not local /
 *     Sender does not belong / missing room_version∥event soft+race;
 *   send PDU exact Content hash mismatch / Invalid PDU structure /
 *     Invalid sender format soft+race (never soft-flooded in numbered);
 *   Media not found ∥ Federation authentication required exact under soft+race.
 *
 * Distinct from #332–#350 oauth/media/filters/crypto/room-cache siblings. Tests-only.
 * example.com only. No product inventing. Reversible by deleting this file.
 */
...
// ---------------------------------------------------------------------------
// Nonary soft floods + concurrent races after tip #342 / post-#328–#342 wave
// ---------------------------------------------------------------------------

describe('soft nonary federation invite/PDU/media/auth exact after #342', () => {
  for (let i = 0; i < 12; i++) {
    it(`invite Event is not an invite exact flood-${i}`, async () => {
  ...
  for (let i = 0; i < 12; i++) {
    it(`invite Invalid state_key exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const keys = ['', 'nocolon', 'also-bad'];
      const res = await req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent(`$isk${i}`)}`,
        env,
        {
          type: 'm.room.member',
          content: { membership: 'invite' },
          sender: REMOTE_USER,
          state_key: keys[i % keys.length],
        }
      );
      expect(res.status).toBe(400);
  ...
  for (let i = 0; i < 12; i++) {
    it(`invite User is not local exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const res = await req(
        'PUT',
        ...
        env,
        {
          ...
          content: { membership: 'invite' },
          sender: REMOTE_USER,
          state_key: `@x:other${i}.example.com`,
        }
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`invite Sender does not belong exact flood-${i}`, async () => {
      ...
      const res = await req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent(`$sb${i}`)}`,
        env,
        {
          type: 'm.room.member',
          content: { membership: 'invite' },
          sender: `@evil:other.example.com`,
          state_key: LOCAL_USER,
        }
      );
      expect(res.status).toBe(403);
  ...
  for (let i = 0; i < 10; i++) {
    it(`v2 invite missing room_version exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const res = await req(
        'PUT',
        `/_matrix/federation/v2/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent(`$nrv${i}`)}`,
        env,
        {
          event: {
            type: 'm.room.member',
  ...
  for (let i = 0; i < 10; i++) {
    it(`v2 invite missing event exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const res = await req(
        'PUT',
        `/_matrix/federation/v2/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent(`$nev${i}`)}`,
        env,
        { room_version: '10' }
      );
      expect(res.status).toBe(400);
      expect((res.body as { error: string }).error).toBe('Missing required parameter: event');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`send Invalid PDU structure exact flood-${i}`, async () => {
      const env = makeEnv(
        createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
      );
      const eid = `$struct${i}`;
      const pdus = [
        { event_id: eid, room_id: ROOM },
        { event_id: eid, room_id: ROOM, sender: REMOTE_USER },
        { event_id: eid, room_id: ROOM, sender: REMOTE_USER, type: 'm.room.message' },
      ];
      const res = await req('PUT', `/_matrix/federation/v1/send/txn-struct-${i}`, env, {
        pdus: [pdus[i % pdus.length]],
      });
      expect(res.status).toBe(200);
      expect((res.body as { pdus: Record<string, { error: string }> }).pdus[eid].error).toBe(
  ...
  for (let i = 0; i < 12; i++) {
    it(`send Invalid sender format exact flood-${i}`, async () => {
      const env = makeEnv(
        createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
      );
      const eid = `$sender${i}`;
      const res = await req('PUT', `/_matrix/federation/v1/send/txn-sender-${i}`, env, {
        pdus: [
          {
            event_id: eid,
            room_id: ROOM,
  ...
  for (let i = 0; i < 12; i++) {
    it(`send Content hash mismatch exact flood-${i}`, async () => {
      ...
      const eid = `$hash${i}`;
      const res = await req('PUT', `/_matrix/federation/v1/send/txn-hash-${i}`, env, {
        pdus: [
          {
            event_id: eid,
            room_id: ROOM,
  ...
  for (let i = 0; i < 12; i++) {
    it(`Media not found exact flood-${i}`, async () => {
      const env = makeEnv(createFedDb({ media: [] }), { media: mockR2() });
      const res = await req(
        'GET',
        `/_matrix/federation/v1/media/download/missing_media_${i}`,
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`Federation authentication required exact flood-${i}`, async () => {
      ...
      const res = await req('PUT', `/_matrix/federation/v1/send/txn-noauth-${i}`, env, {
        pdus: [],
      });
...
describe('race nonary federation invite/PDU/media/auth exact after #342', () => {
  beforeEach(() => {
    federationOrigin = FED_ORIGIN;
    verifyRemoteSignature.mockResolvedValue(true);
    verifyContentHash.mockResolvedValue(false);
  ...
  it('not-invite∥Invalid state_key isolation', async () => {
    const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
    const [ni, isk] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_ni')}`,
        env,
        {
          type: 'm.room.member',
      ...
      req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_isk')}`,
        env,
        {
          type: 'm.room.member',
          content: { membership: 'invite' },
    ...
    expect((ni.body as { error: string }).error).toBe('Event is not an invite event');
    expect((isk.body as { error: string }).error).toBe('Invalid state_key for invite');
  });

  it('not-local∥Sender does not belong isolation', async () => {
    const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
    const [nl, sb] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_nl')}`,
        env,
        {
          type: 'm.room.member',
          content: { membership: 'invite' },
          sender: REMOTE_USER,
          state_key: '@x:elsewhere.example.com',
        }
      ),
      req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_sb')}`,
        env,
        {
          type: 'm.room.member',
          content: { membership: 'invite' },
          sender: '@evil:elsewhere.example.com',
          state_key: LOCAL_USER,
        }
      ),
    ]);
    expect((nl.body as { error: string }).error).toBe('User is not local to this server');
    expect((sb.body as { error: string }).error).toBe(
      'Sender does not belong to the authenticated origin server'
    );
  });

  it('missing room_version∥missing event isolation', async () => {
    const env = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
    const [nrv, nev] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v2/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_nrv')}`,
        env,
        {
          event: {
            type: 'm.room.member',
      ...
      req(
        'PUT',
        `/_matrix/federation/v2/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$r_nev')}`,
        env,
        { room_version: '10' }
      ),
    ]);
    expect((nrv.body as { error: string }).error).toBe(
      'Missing required parameter: room_version'
    );
    expect((nev.body as { error: string }).error).toBe('Missing required parameter: event');
  });

  it('Invalid PDU∥Invalid sender∥Content hash triple', async () => {
    const env = makeEnv(
      createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
    );
    const [struct, sender, hash] = await Promise.all([
      req('PUT', '/_matrix/federation/v1/send/txn-r-struct', env, {
        pdus: [{ event_id: '$rstruct', room_id: ROOM }],
      }),
      req('PUT', '/_matrix/federation/v1/send/txn-r-sender', env, {
        pdus: [
          {
            event_id: '$rsender',
            room_id: ROOM,
      ...
      req('PUT', '/_matrix/federation/v1/send/txn-r-hash', env, {
        pdus: [
          {
            event_id: '$rhash',
            room_id: ROOM,
    ...
    expect((struct.body as { pdus: Record<string, { error: string }> }).pdus['$rstruct'].error).toBe(
      'Invalid PDU structure'
    );
    expect((sender.body as { pdus: Record<string, { error: string }> }).pdus['$rsender'].error).toBe(
      'Invalid sender format'
    );
    expect((hash.body as { pdus: Record<string, { error: string }> }).pdus['$rhash'].error).toBe(
      'Content hash mismatch'
    );
  });

  it('Media not found then Federation authentication required serial dual', async () => {
    const mediaEnv = makeEnv(createFedDb({ media: [] }), { media: mockR2() });
    const authEnv = makeEnv(createFedDb());
    const media = await req('GET', '/_matrix/federation/v1/media/download/nope_race', mediaEnv);
    expect(media.status).toBe(404);
    expect((media.body as { error: string }).error).toBe('Media not found');
    federationOrigin = undefined;
    const auth = await req('PUT', '/_matrix/federation/v1/send/txn-r-noauth', authEnv, {
      pdus: [],
    });
    federationOrigin = FED_ORIGIN;
  ...
  it('not-invite∥Content hash∥Media not found∥Invalid sender four-way', async () => {
    const inviteEnv = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
    const sendEnv = makeEnv(
      createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
    );
    const mediaEnv = makeEnv(createFedDb({ media: [] }), { media: mockR2() });
    const [ni, hash, media, sender] = await Promise.all([
      req(
        'PUT',
        `/_matrix/federation/v1/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent('$4ni')}`,
        inviteEnv,
        {
          type: 'm.room.member',
      ...
      req('PUT', '/_matrix/federation/v1/send/txn-4way-hash', sendEnv, {
        pdus: [
          {
            event_id: '$4hash',
            room_id: ROOM,
      ...
      req('GET', '/_matrix/federation/v1/media/download/4way_miss', mediaEnv),
      req('PUT', '/_matrix/federation/v1/send/txn-4way-sender', sendEnv, {
        pdus: [
          {
            event_id: '$4sender',
            room_id: ROOM,
    ...
    expect((ni.body as { error: string }).error).toBe('Event is not an invite event');
    expect((hash.body as { pdus: Record<string, { error: string }> }).pdus['$4hash'].error).toBe(
      'Content hash mismatch'
    );
    expect((media.body as { error: string }).error).toBe('Media not found');
    expect(
      (sender.body as { pdus: Record<string, { error: string }> }).pdus['$4sender'].error
    ).toBe('Invalid sender format');
  });

  for (let i = 0; i < 8; i++) {
    it(`nonary federation exact race flood-${i}`, async () => {
      const inviteEnv = makeEnv(createFedDb({ users: [{ user_id: LOCAL_USER }] }));
      const sendEnv = makeEnv(
        createFedDb({ rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }] })
      );
      ...
      const results = await Promise.all([
        ...
        req('PUT', `/_matrix/federation/v1/send/txn-ff-struct-${i}`, sendEnv, {
          pdus: [{ event_id: `$fs${i}`, room_id: ROOM }],
        }),
        req('GET', `/_matrix/federation/v1/media/download/ff_miss_${i}`, mediaEnv),
        req(
          'PUT',
          `/_matrix/federation/v2/invite/${encodeURIComponent(ROOM)}/${encodeURIComponent(`$fnrv${i}`)}`,
          inviteEnv,
          {
            event: {
              type: 'm.room.member',
      ...
      expect(
        (results[1].body as { pdus: Record<string, { error: string }> }).pdus[`$fs${i}`].error
      ).toBe('Invalid PDU structure');
