...
describe('crypto TOKENMAXX residual undecenary leftovers after #372', () => {
  it('verifyPassword wrong-salt ∥ empty-digest ∥ ok stay isolated under race', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const real = await hashPassword('undecenary-ok-1');
    const parts = real.split('$');
    ...
    const [ok, saltBad, emptyDig, emptyStored] = await Promise.all([
      verifyPassword('undecenary-ok-1', real),
      verifyPassword('undecenary-ok-1', wrongSalt),
      verifyPassword('undecenary-ok-1', emptyDigest),
      verifyPassword('undecenary-ok-1', ''),
    ]);
    expect(ok).toBe(true);
  ...
  for (let i = 0; i < 6; i++) {
    it(`hashPassword∥verify wrong-salt∥timingSafeEqual∥hashToken flood-${i}`, async () => {
      const [hash, tok, eq] = await Promise.all([
        hashPassword(`und-flood-${i}-1`),
        hashToken(`syt_und_${i}`),
        Promise.resolve(timingSafeEqual(`u-${i}`, `u-${i}`)),
      ]);
      const parts = hash.split('$');
      const wrongSalt = `$pbkdf2-sha256$100000$${btoa(`salt-${i}-pad!!!!`)}$${parts[4]}`;
      const [ok, saltBad] = await Promise.all([
        verifyPassword(`und-flood-${i}-1`, hash),
        verifyPassword(`und-flood-${i}-1`, wrongSalt),
      ]);
      expect(ok).toBe(true);
      expect(saltBad).toBe(false);
      expect(tok).toBe(await sha256(`syt_und_${i}`));
      expect(eq).toBe(true);
    });
  }
});

describe('federation signing TOKENMAXX residual undecenary leftovers after #372', () => {
  let restore: (() => void) | undefined;
  ...
  it('same-server dual-keyId merge ∥ wrong pubkey for right keyId false under race', async () => {
    const a = await generateSigningKeyPair();
    const b = await generateSigningKeyPair();
    const base = { type: 'm.test', content: { path: 'und-dual' }, unsigned: { age: 1 } };
    const once = await signJson(base, 'ex.com', a.keyId, a.privateKeyJwk);
    const twice = await signJson(once, 'ex.com', b.keyId, b.privateKeyJwk);
