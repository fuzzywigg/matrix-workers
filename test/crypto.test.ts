...
describe('crypto TOKENMAXX residual quaternary leftovers after #303', () => {
  ...
  it('timingSafeEqual + hashToken + generateRandomString stay isolated under Promise.all', async () => {
    const [eq, neq, len, tokA, tokB, empty, one, def] = await Promise.all([
      ...
      hashToken('syt_quaternary_a'),
      hashToken('syt_quaternary_b'),
...
describe('federation signing TOKENMAXX residual quaternary leftovers after #303', () => {
  let restore: (() => void) | undefined;
