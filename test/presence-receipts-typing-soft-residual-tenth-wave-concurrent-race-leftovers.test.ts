/**
 * TOKENMAXX HEAVY tip-relaunch deepen after merged #397 — niche
 * presence/receipts/typing soft residual *tenth-wave* concurrent-race leftovers
 * (tip past #397 ninth / 0d60aaa).
 *
 * Unsaturated by waves #301/#305/#318/#327/#334/#341/#357/#371/#397:
 *   never ONline / oFFLINE / UNAVAILAble / onliNE case pins,
 *   never Focusing/InMeeting/BeRightBack/NotAvailable/Sleeping invalid pins,
 *   never BOM / thin-space / trailing-space whitespace (`\ufeffonline`,
 *     `\u2009online`, `offline `),
 *   never typing root-string `"TRUE"` / root-number `-1` → Missing typing ∥ boolean ok,
 *   never typing knock-membership Not a member ∥ join ok,
 *   never receipt case-mutated types M.READ.private/m.Fully_READ/M.fully_read/
 *     m.read.PRIVATE under soft residual PA.
 *
 * Gap table (why leftover after #397 ninth):
 *   ONline/oFFLINE/UNAVAILAble + Focusing/InMeeting/Sleeping + BOM/thin/trail-sp ∥ ok
 *   typing root-string/number Missing typing + knock-membership Not a member ∥ ok
 *   receipt case-mutated Invalid type matrix ∥ m.read ok
 *   cross-domain: ONline + root-string `"TRUE"` typing + M.READ.private ∥ oks
 *
...
describe('presence tenth-wave concurrent case/whitespace soft after #397', () => {
  const cases: Array<{ label: string; body: unknown; error: string }> = [
    ...
    {
      label: 'Focusing-title',
      body: { presence: 'Focusing' },
      error: 'Invalid presence state: Focusing. Must be one of: online, offline, unavailable',
    },
    {
      label: 'InMeeting-title',
      body: { presence: 'InMeeting' },
      error: 'Invalid presence state: InMeeting. Must be one of: online, offline, unavailable',
    },
    {
      label: 'BeRightBack-title',
      body: { presence: 'BeRightBack' },
      error: 'Invalid presence state: BeRightBack. Must be one of: online, offline, unavailable',
    },
    {
      label: 'NotAvailable-title',
      body: { presence: 'NotAvailable' },
      error: 'Invalid presence state: NotAvailable. Must be one of: online, offline, unavailable',
    },
    {
    ...
    {
      label: 'thin-space-whitespace',
      body: { presence: '\u2009online' },
      error: 'Invalid presence state: \u2009online. Must be one of: online, offline, unavailable',
    },
    {
      label: 'trailing-space-whitespace',
      body: { presence: 'offline ' },
      error: 'Invalid presence state: offline . Must be one of: online, offline, unavailable',
    },
  ];
  ...
  it('ONline ∥ oFFLINE ∥ Focusing ∥ Sleeping ∥ offline under race', async () => {
    const env = createEnv();
    const results = await Promise.all([
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
      ...
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
        jsonInit('PUT', { presence: 'Focusing' }),
        env
      ),
      presenceReq(
        `/_matrix/client/v3/presence/${USER_ENC}/status`,
    ...
    expect(
      ...
      [
        ...
        'Invalid presence state: Focusing. Must be one of: online, offline, unavailable',
...
describe('typing tenth-wave concurrent root-string-number + knock-membership soft after #397', () => {
  ...
  it('knock membership Not a member ∥ join ok under race', async () => {
    const roomDO = createRoomDOStub();
    const softRoom = '!knock-mem:example.com';
    const env = createEnv({
      roomDO,
      db: createSharedDb({
        memberships: [
  ...
  it('root-string + knock-membership ∥ two oks under race', async () => {
    const roomDO = createRoomDOStub();
    const softRoom = '!knock:example.com';
    const env = createEnv({
      roomDO,
      db: createSharedDb({
        memberships: [
  ...
  for (let i = 0; i < 6; i++) {
    it(`typing root-string-number / knock-mem flood-${i} ∥ ok`, async () => {
      const roomDO = createRoomDOStub();
      const softRoom = `!knock${i}:example.com`;
      const env = createEnv({
        roomDO,
        db: createSharedDb({
          memberships: [
...
describe('receipts tenth-wave concurrent case-mutated Invalid type soft after #397', () => {
  for (const badType of ['M.READ.private', 'm.Fully_READ', 'M.fully_read', 'm.read.PRIVATE'] as const) {
  ...
  it('M.READ.private ∥ m.Fully_READ ∥ M.fully_read ∥ m.read + m.read.private under race', async () => {
    const roomDO = createRoomDOStub();
    const env = createEnv({ roomDO });
    const results = await Promise.all([
  ...
  for (let i = 0; i < 6; i++) {
    it(`receipt case-mutated type flood-${i}`, async () => {
      const roomDO = createRoomDOStub();
      const env = createEnv({ roomDO });
      const badType = (['M.READ.private', 'm.Fully_READ', 'M.fully_read', 'm.read.PRIVATE'] as const)[i % 4];
      const results = await Promise.all([
...
// ---------------------------------------------------------------------------
// Cross-domain tenth-wave soft isolation
// ---------------------------------------------------------------------------

describe('presence/receipts/typing tenth-wave cross-domain soft isolation after #397', () => {
  ...
  for (let i = 0; i < 6; i++) {
    it(`cross-domain tenth-wave soft isolation flood-${i}`, async () => {
      const roomDO = createRoomDOStub();
      const env = createEnv({ roomDO });
      const badPresence = (['ONline', 'oFFLINE', 'Focusing'] as const)[i % 3];
      const badType = (['M.READ.private', 'm.Fully_READ', 'M.fully_read'] as const)[i % 3];
      const results = await Promise.all([
        presenceReq(
          `/_matrix/client/v3/presence/${USER_ENC}/status`,
