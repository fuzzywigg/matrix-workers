/**
 * TOKENMAXX HEAVY tip-relaunch deepen after merged #403 — oauth
 * *quattuordecenary* concurrent-race niches tridecenary (#366) never
 * mixed under Promise.all (tip past #403 media/presence tenth / f933abf;
 * fresh after closed #406/#399/#394 tip-relaunches):
 *   parse fail ∥ Request Cancelled ∥ Authorization request expired ∥
 *     Username required (tridecenary Username∥Cancelled∥AUTH∥Missing
 *     without parse; default∥parse∥Expired∥no-period without Cancelled
 *     +AUTH JSON),
 *   cross-signing desc ∥ OIDC Approved ∥ Session Expired ∥ login
 *     no-period (duodenary cross-signing∥sensitive∥Missing∥Username;
 *     tridecenary mismatch∥OIDC∥Missing∥credentials — never
 *     cross-signing∥OIDC∥Expired∥no-period),
 *   sensitive ∥ Invalid with period ∥ login Missing ∥ credentials-desc
 *     (duodenary Cancelled∥OIDC∥period∥no-period; undecenary
 *     sensitive∥period without Missing+credentials — never
 *     sensitive∥period∥Missing∥credentials).
 *
 * Gap table (why leftover after #366 / tip past #403):
 *   parse ∥ Cancelled ∥ AUTH expired JSON ∥ Username required
 *     | tridecenary Cancelled+AUTH without parse; parse without Cancelled
 *   cross-signing ∥ OIDC approve ∥ Expired ∥ no-period
 *     | duodenary cross-signing without OIDC+Expired+no-period
 *   sensitive ∥ period ∥ login Missing ∥ credentials
 *     | undecenary/duodenary sensitive without this quartet
 *
 ...
 * No invent-product / secrets / DNS / workflow edits.
 */
...
// ---------------------------------------------------------------------------
// parse fail ∥ Cancelled ∥ AUTH expired JSON ∥ Username required
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth parse∥Cancelled∥AUTH-expired∥Username after #366', () => {
  it('parse fail ∥ Request Cancelled ∥ Authorization request expired ∥ Username required under race', async () => {
    const spy = vi
  ...
  it('parse ∥ Cancelled ∥ AUTH expired ∥ Username ∥ login Missing penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
  ...
  for (let i = 0; i < 8; i++) {
    it(`parse ∥ Cancelled ∥ AUTH expired ∥ Username flood-${i}`, async () => {
      const spy = vi
        .spyOn(HonoRequest.prototype, 'parseBody')
...
// ---------------------------------------------------------------------------
// cross-signing ∥ OIDC Approved ∥ Session Expired ∥ login no-period
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth cross-signing∥OIDC∥Expired∥no-period after #366', () => {
  it('cross-signing desc ∥ OIDC Approved ∥ Session Expired ∥ login no-period under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
  ...
  it('cross-signing ∥ OIDC ∥ Expired ∥ no-period ∥ mismatch ∥ Missing ∥ default', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
  ...
  for (let i = 0; i < 8; i++) {
    it(`cross-signing ∥ OIDC ∥ Expired ∥ no-period flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
...
describe('quattuordecenary oauth sensitive∥period∥Missing∥credentials after #366', () => {
  it('sensitive ∥ Invalid with period ∥ login Missing ∥ credentials-desc under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
  ...
  it('sensitive ∥ period ∥ Missing ∥ credentials ∥ Cancelled penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-bad');
  ...
  for (let i = 0; i < 8; i++) {
    it(`sensitive ∥ period ∥ Missing ∥ credentials flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      seedUiaSession(cache, `uia-xr-${i}`);
  ...
  it('parse∥Cancelled∥cross-signing∥OIDC∥sensitive∥period mega race', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
    ...
    const htmls = htmlOf(results.filter((_, i) => i !== 5));
    expect(htmls.some((h) => h.includes(UIA_PARSE_BODY))).toBe(true);
    expect(htmls.some((h) => h.includes('Request Cancelled'))).toBe(true);
    expect(htmls.some((h) => h.includes(UIA_CROSS_SIGNING_DESC))).toBe(true);
    expect(htmls.some((h) => h.includes(UIA_SENSITIVE))).toBe(true);
