/**
 * TOKENMAXX HEAVY tip-relaunch at tip ~320fea0 after #366 oauth tridecenary
 * + #380 room-cache (fresh after closed conflicting tips) — oauth
 * *quattuordecenary* concurrent-race niches tridecenary (#366) never mixed
 * under Promise.all:
 *   cross-signing desc ∥ Authorization request expired ∥ parse fail ∥
 *     password Request Approved (duodenary cross-signing∥sensitive∥
 *     Missing∥Username; tridecenary AUTH with Username∥Cancelled∥Missing;
 *     parse with default∥Expired∥no-period — never cross-signing∥AUTH∥
 *     parse∥Approved),
 *   sensitive warning ∥ Request Cancelled ∥ Session Expired ∥ login
 *     Missing (undecenary sensitive∥period∥no-period∥Approved;
 *     tridecenary Cancelled with Username∥AUTH; Expired with default∥
 *     parse — never sensitive∥Cancelled∥Expired∥Missing),
 *   Username required ∥ UIA period ∥ OIDC Approved ∥ default desc
 *     (tridecenary Username∥Cancelled∥AUTH∥Missing; duodenary
 *     Cancelled∥OIDC∥period∥no-period — never Username∥period∥OIDC∥
 *     default).
 *
 * Gap table (why leftover after #366 / tip past #380):
 *   cross-signing ∥ AUTH expired ∥ parse ∥ Approved
 *     | duodenary/tridecenary pairs only
 *   sensitive ∥ Cancelled ∥ Expired ∥ login Missing
 *     | undecenary sensitive without Cancelled+Expired+Missing
 *   Username ∥ period ∥ OIDC approve ∥ default
 *     | tridecenary Username quad; duodenary OIDC∥period without Username+default
 *
...
// ---------------------------------------------------------------------------
// cross-signing ∥ AUTH expired ∥ parse ∥ Request Approved
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth cross-signing∥AUTH-expired∥parse∥Approved after #366', () => {
  it('cross-signing desc ∥ Authorization request expired ∥ parse fail ∥ Request Approved under race', async () => {
    const spy = vi
    ...
    const htmls = htmlOf(results.filter((_, i) => i !== 1));
  ...
  it('cross-signing ∥ AUTH expired ∥ parse ∥ Approved ∥ credentials-desc penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
  ...
  for (let i = 0; i < 8; i++) {
    it(`cross-signing ∥ AUTH expired ∥ parse ∥ Approved flood-${i}`, async () => {
      const spy = vi
        .spyOn(HonoRequest.prototype, 'parseBody')
      ...
      const htmls = htmlOf(results.filter((_, i2) => i2 !== 1));
      expect(htmls.some((h) => h.includes(UIA_CROSS_SIGNING_DESC))).toBe(true);
      expect(htmls.some((h) => h.includes(UIA_PARSE_BODY))).toBe(true);
      expect(htmls.some((h) => h.includes('Request Approved'))).toBe(true);
    });
...
// ---------------------------------------------------------------------------
// sensitive ∥ Cancelled ∥ Session Expired ∥ login Missing
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth sensitive∥Cancelled∥Expired∥Missing after #366', () => {
  it('sensitive warning ∥ Request Cancelled ∥ Session Expired ∥ login Missing under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
  ...
  it('sensitive ∥ Cancelled ∥ Expired ∥ Missing ∥ credentials-desc penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-cancel');
  ...
  for (let i = 0; i < 8; i++) {
    it(`sensitive ∥ Cancelled ∥ Expired ∥ Missing flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      seedUiaSession(cache, `uia-xr-${i}`);
...
describe('quattuordecenary oauth Username∥period∥OIDC-approve∥default after #366', () => {
  it('Username required ∥ UIA period ∥ OIDC Approved ∥ default desc under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-cred');
  ...
  it('Username ∥ period ∥ OIDC approve ∥ default ∥ login Missing penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-cred');
    seedUiaSession(cache, 'uia-bad');
  ...
  for (let i = 0; i < 8; i++) {
    it(`Username ∥ period ∥ OIDC approve ∥ default flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
  ...
  it('cross-signing∥AUTH∥parse∥sensitive∥Cancelled∥Username∥OIDC∥default mega race', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
