 ...
 * media — oauth *quattuordecenary* concurrent-race niches duodenary
 * (#340) / open tridecenary (#354) never mixed under Promise.all:
 *   cross-signing desc ∥ Request Cancelled ∥ Authorization request
 *     expired ∥ login no-period (duodenary cross-signing∥sensitive∥
 *     Missing∥Username; tridecenary Username∥Cancelled∥AUTH∥Missing;
 *     duodenary Cancelled∥OIDC∥period∥no-period — never cross-signing∥
 *     Cancelled∥AUTH∥no-period),
 *   sensitive warning ∥ OIDC Approved ∥ Session Expired ∥ Username
 *     required (undecenary sensitive∥period∥Approved; tridecenary
 *     mismatch∥OIDC∥Missing∥credentials; duodenary mismatch∥Expired∥
 *     Missing∥Approved — never sensitive∥OIDC∥Expired∥Username),
 *   account-mismatch ∥ parse fail ∥ password Approved ∥ default desc
 *     (duodenary mismatch∥Expired∥Missing∥Approved; tridecenary
 *     default∥parse∥Expired∥no-period — never mismatch∥parse∥
 *     Approved∥default).
 *
 * Gap table (why leftover after #340 / tip past #341; niches #354
 * tridecenary still open but disjoint):
 *   cross-signing ∥ Cancelled ∥ AUTH expired JSON ∥ no-period
 *     | Cancelled+AUTH with Username/Missing only; no-period with
 *       Cancelled+OIDC+period
 *   sensitive ∥ OIDC approve ∥ Expired ∥ Username required
 *     | sensitive without OIDC+Expired+Username; OIDC without Expired
 *   mismatch ∥ parse ∥ password Approved ∥ default
 *     | mismatch without parse+default; parse without mismatch+Approved
 *
 * Distinct from #354 tridecenary, #340 duodenary, #324 undecenary.
...
// ---------------------------------------------------------------------------
// cross-signing ∥ Cancelled ∥ AUTH expired JSON ∥ login no-period
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth cross-signing∥Cancelled∥AUTH∥no-period after #340', () => {
  it('cross-signing desc ∥ Request Cancelled ∥ Authorization request expired ∥ login no-period under race', async () => {
    const cache = mockKv();
  ...
  it('cross-signing ∥ Cancelled ∥ AUTH expired ∥ no-period ∥ Missing Session penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-cancel');
  ...
  for (let i = 0; i < 8; i++) {
    it(`cross-signing ∥ Cancelled ∥ AUTH ∥ no-period flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      ...
      const htmls = htmlOf(results.filter((_, j) => j !== 2));
      expect(htmls.some((h) => h.includes(UIA_CROSS_SIGNING_DESC))).toBe(true);
      expect(htmls.some((h) => h.includes('Request Cancelled'))).toBe(true);
      expect(hasLoginNoPeriod(htmls)).toBe(true);
    });
...
// ---------------------------------------------------------------------------
// sensitive ∥ OIDC Approved ∥ Session Expired ∥ Username required
// ---------------------------------------------------------------------------

describe('quattuordecenary oauth sensitive∥OIDC-approve∥Expired∥Username after #340', () => {
  it('sensitive warning ∥ OIDC Approved ∥ Session Expired ∥ Username required under race', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
  ...
  it('sensitive ∥ OIDC approve ∥ Expired ∥ Username ∥ credentials-desc penta', async () => {
    const cache = mockKv();
    seedClient(cache, 'cid-1');
    seedUiaSession(cache, 'uia-xr');
    seedUiaSession(cache, 'uia-oidc', { user_id: OIDC_ID });
  ...
  for (let i = 0; i < 8; i++) {
    it(`sensitive ∥ OIDC approve ∥ Expired ∥ Username flood-${i}`, async () => {
      const cache = mockKv();
      seedClient(cache, 'cid-1');
      seedUiaSession(cache, `uia-xr-${i}`);
...
describe('quattuordecenary oauth mismatch∥parse∥Approved∥default after #340', () => {
  it('account-mismatch ∥ parse fail ∥ password Approved ∥ default desc under race', async () => {
    // parse reject steals the first POST under Promise.all (undecenary
    // pattern) — put parse first so mismatch/Approved siblings survive.
    const spy = vi
  ...
  it('mismatch ∥ parse ∥ Approved ∥ default ∥ Expired body penta', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
  ...
  for (let i = 0; i < 8; i++) {
    it(`mismatch ∥ parse ∥ Approved ∥ default flood-${i}`, async () => {
      const spy = vi
        .spyOn(HonoRequest.prototype, 'parseBody')
  ...
  it('cross-signing∥Cancelled∥sensitive∥OIDC∥mismatch∥parse mega race', async () => {
    const spy = vi
      .spyOn(HonoRequest.prototype, 'parseBody')
