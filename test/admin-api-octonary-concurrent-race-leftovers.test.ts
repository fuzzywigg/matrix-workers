/**
 * TOKENMAXX HEAVY leftovers after #328 / complementary to in-flight septenary #335 —
 * octonary *admin* soft+concurrent-race niches unsaturated by senary (#328) and
 * septenary IdP create/missing/test-fail/delete/link (#335).
 *
 * Distinct from senary (#328) Report/Admin-access/login-token/IdP-PUT-discovery/keys:
 * Distinct from septenary (#335) IdP POST missing/create-fail/test-fail/deleted/link:
 *   IdP PUT Identity provider updated success exact soft+race;
 *   IdP PUT No changes (empty body) success exact soft+race;
 *   IdP POST /test Connection successful exact soft+race;
 *   IdP POST∥PUT Bad JSON Could not parse request body exact soft+race.
 *
 * Distinct from #332–#339 oauth/media/filters/crypto/room-cache siblings.
...
// ---------------------------------------------------------------------------
// Octonary soft floods + concurrent races — complementary to septenary #335
// ---------------------------------------------------------------------------

describe('soft octonary admin IdP updated/No-changes/test-ok/Bad-JSON after #328/#335', () => {
  for (let i = 0; i < 12; i++) {
    it(`IdP PUT Identity provider updated exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/idp/providers/idp1',
        jsonInit('PUT', { name: `Updated-${i}` }),
        env
      );
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.message).toBe('Identity provider updated');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`IdP PUT No changes exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', {}), env);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
  ...
  for (let i = 0; i < 12; i++) {
    it(`IdP POST /test Connection successful exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
      ...
      expect(res.body.discovery.issuer).toBe('https://idp.example.com');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`IdP POST Bad JSON exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
  ...
  for (let i = 0; i < 12; i++) {
    it(`IdP PUT Bad JSON exact flood-${i}`, async () => {
      const env = createEnv();
      const res = await jsonReq(
        '/admin/api/idp/providers/idp1',
        {
...
describe('race octonary admin IdP updated/No-changes/test-ok/Bad-JSON after #328/#335', () => {
  it('Identity provider updated∥No changes isolation', async () => {
    const env = createEnv();
    const [upd, none] = await Promise.all([
    ...
    expect(none.body.message).toBe('No changes');
  });

  it('Connection successful∥Identity provider not found isolation', async () => {
    const env = createEnv();
    const [ok, miss] = await Promise.all([
      ...
      jsonReq(
        '/admin/api/idp/providers/missing-oct/test',
        { method: 'POST', headers: AUTH },
        env
      ),
    ]);
  ...
  it('updated∥Connection successful∥No changes three-way', async () => {
    const env = createEnv();
    const [upd, testOk, none] = await Promise.all([
      jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', { enabled: true }), env),
      jsonReq(
        '/admin/api/idp/providers/idp1/test',
        { method: 'POST', headers: AUTH },
        env
    ...
    expect(upd.body.message).toBe('Identity provider updated');
    expect(testOk.body.message).toBe('Connection successful');
    expect(none.body.message).toBe('No changes');
  });

  it('POST Bad JSON∥PUT Bad JSON isolation', async () => {
    const env = createEnv();
    const [postBad, putBad] = await Promise.all([
      jsonReq(
        '/admin/api/idp/providers',
        {
          method: 'POST',
    ...
    expect(postBad.body.error).toBe('Could not parse request body as JSON');
    expect(putBad.status).toBe(400);
    expect(putBad.body.error).toBe('Could not parse request body as JSON');
  });

  it('updated∥Bad JSON∥Connection successful∥not found four-way', async () => {
    const env = createEnv();
    const [upd, bad, testOk, miss] = await Promise.all([
      jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', { name: 'Four' }), env),
      jsonReq(
        '/admin/api/idp/providers/idp1',
        {
          method: 'PUT',
      ...
      jsonReq('/admin/api/idp/providers/gone-oct', {}, env),
    ]);
    expect(upd.body.message).toBe('Identity provider updated');
  ...
  it('No changes∥updated not-found isolation', async () => {
    const env = createEnv();
    const [none, miss] = await Promise.all([
      jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', {}), env),
      jsonReq('/admin/api/idp/providers/missing-upd', jsonInit('PUT', { name: 'x' }), env),
    ]);
    expect(none.body.message).toBe('No changes');
    expect(miss.status).toBe(404);
    expect(miss.body.error).toBe('Identity provider not found');
  ...
  for (let i = 0; i < 12; i++) {
    it(`octonary admin IdP success exact race flood-${i}`, async () => {
      const env = createEnv();
      const results = await Promise.all([
        jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', { name: `F${i}` }), env),
        jsonReq('/admin/api/idp/providers/idp1', jsonInit('PUT', {}), env),
        jsonReq(
          '/admin/api/idp/providers/idp1/test',
          { method: 'POST', headers: AUTH },
        ...
        jsonReq(
          `/admin/api/idp/providers/miss-${i}`,
          {
            method: 'PUT',
            headers: {
              ...AUTH,
      ...
      expect(results[2].body.message).toBe('Connection successful');
      // missing id returns 404 before JSON parse... actually body is parsed first
      // for PUT: bad JSON is checked before existence — so M_BAD_JSON
