/**
 * TOKENMAXX HEAVY leftovers after #279 — quaternary *admin* soft+concurrent-race
 * niches unsaturated by tertiary IdP (#279) and second-wave (#275).
 * Reclaims zero/exact-string gaps that closed #284 claimed but never merged.
 *
 * Distinct from #279 tertiary IdP soft/race and admin-mutate write TOCTOU:
 *   exact Cannot remove your own admin privileges;
 *   exact Cannot delete your own account;
 *   exact enabled (boolean) required;
 *   exact User is deactivated under soft/race;
 *   exact Admin privileges required to query other users under race;
 *   keys Verified ∥ No self-signing key soft+race (routes once only).
 *
...
describe('soft quaternary admin exact forbid/param strings after #279', () => {
  for (let i = 0; i < 12; i++) {
  ...
  for (let i = 0; i < 12; i++) {
    it(`keys Verified soft flood-${i}`, async () => {
      const env = createEnv();
      ...
      const vs = (res.body.verification_status as Record<string, { verified: boolean; reason: string }>)
        .BOBDEVICE;
      expect(vs).toEqual({ verified: true, reason: 'Verified' });
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`keys No self-signing key soft flood-${i}`, async () => {
      const db = createAdminDb({
        crossSigningKeys: [
          {
            user_id: BOB,
      ...
      const vs = (res.body.verification_status as Record<string, { verified: boolean; reason: string }>)
        .BOBDEVICE;
      expect(vs).toEqual({ verified: false, reason: 'No self-signing key' });
    });
  }
});

describe('race quaternary admin exact strings + keys after #279', () => {
  ...
  it('keys Verified∥No self-signing key isolation', async () => {
    ...
    const [verified, noSs] = await Promise.all([
    ...
    expect(statusesOf([verified, noSs])).toEqual([200, 200]);
    expect(
  ...
  for (let i = 0; i < 10; i++) {
    it(`quaternary admin exact race flood-${i}`, async () => {
      const env = createEnv();
      ...
      expect(
        (results[3].body.verification_status as Record<string, { reason: string }>).BOBDEVICE
          .reason
      ).toBe('Verified');
    });
  }
