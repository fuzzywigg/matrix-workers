/**
 * TOKENMAXX HEAVY leftovers after #297 quaternary / tip past #303 — *federation*
 * *quinary* soft+concurrent-race niches: exact product strings never leftover-
 * asserted under soft/race (routes status/errcode only). Skip catchup.
 *   `Requesting server has no users in this room` (backfill + get_missing_events);
 *   `Room alias not found` (query/directory);
 *   `User not found` (query/profile);
 *   `Room not found` exact (backfill miss);
 *   `Key not found` (GET /_matrix/key/v2/server/:keyId);
 *   `Invalid server name` (GET key query SSRF reject);
 *   `No keys found` (own-server query with empty server_keys).
 *
 * Gap table (why leftover after #297):
 *   backfill/gme membership forbid | status 403 only in routes/races
 *   directory/profile miss | status 404 only in leftovers races
 *   Key not found / Invalid server name / No keys found | absent from leftovers
 *
 * Distinct from #297 quaternary empty-ts/leave/v2/crop, #278 knock, #279 invite.
 * New file. Tests-only. example.com fixtures. Reversible by delete.
...
function stubNotaryKeys(): ServerKeyRow[] {
  return [
    {
      key_id: 'ed25519:notary_q5',
      public_key: 'cHVibGljX3E1X2tleQ',
      private_key_jwk: JSON.stringify({ kty: 'OKP', crv: 'Ed25519', d: 'd', x: 'x' }),
      key_version: 2,
      valid_from: 1_000,
...
describe('soft quinary federation membership/keys/directory exact after #297', () => {
  for (let i = 0; i < 12; i++) {
    it(`backfill Requesting server has no users exact flood-${i}`, async () => {
      ...
      const res = await req(
        'GET',
        `/_matrix/federation/v1/backfill/${encodeURIComponent(ROOM)}?limit=${(i % 3) + 1}`,
        env
      );
  ...
  for (let i = 0; i < 12; i++) {
    it(`get_missing_events Requesting server has no users exact flood-${i}`, async () => {
      const db = createFedDb({
        rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }],
        memberships: [],
      });
      ...
      const res = await req(
        ...
        env,
        { earliest_events: [], latest_events: [`$e${i}`], limit: 5 }
      );
      expect(res.status).toBe(403);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_FORBIDDEN');
      expect((res.body as { error: string }).error).toBe(
  ...
  for (let i = 0; i < 12; i++) {
    it(`backfill Room not found exact flood-${i}`, async () => {
      const db = createFedDb({ rooms: [], memberships: [] });
      const env = makeEnv(db);
      const res = await req(
        'GET',
        `/_matrix/federation/v1/backfill/${encodeURIComponent(`!missing${i}:example.com`)}`,
        env
      );
      expect(res.status).toBe(404);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_NOT_FOUND');
      expect((res.body as { error: string }).error).toBe('Room not found');
    });
  }

  for (let i = 0; i < 12; i++) {
    it(`query/directory Room alias not found exact flood-${i}`, async () => {
      const db = createFedDb({ aliases: { '#room:example.com': ROOM } });
      const env = makeEnv(db);
      const res = await req(
        'GET',
        `/_matrix/federation/v1/query/directory?room_alias=${encodeURIComponent(`#no${i}:example.com`)}`,
        env
      );
      expect(res.status).toBe(404);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`query/profile User not found exact flood-${i}`, async () => {
      const db = createFedDb({
        users: [{ user_id: LOCAL_USER, display_name: 'Alice', avatar_url: null }],
      });
      const env = makeEnv(db);
      const res = await req(
        'GET',
        `/_matrix/federation/v1/query/profile?user_id=${encodeURIComponent(`@nobody${i}:example.com`)}`,
        env
      );
      expect(res.status).toBe(404);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`key/v2/server Key not found exact flood-${i}`, async () => {
      const db = createFedDb({ serverKeys: stubNotaryKeys() });
      const env = makeEnv(db);
      const res = await req('GET', `/_matrix/key/v2/server/ed25519:missing${i}`, env);
      expect(res.status).toBe(404);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_NOT_FOUND');
  ...
  for (let i = 0; i < 12; i++) {
    it(`key query Invalid server name exact flood-${i}`, async () => {
      const db = createFedDb({ serverKeys: stubNotaryKeys() });
      const env = makeEnv(db);
      const bad = ['127.0.0.1', 'localhost', '0.0.0.0', '::1', '169.254.1.1', '10.0.0.1'][i % 6];
  ...
  for (let i = 0; i < 12; i++) {
    it(`key query No keys found for server exact flood-${i}`, async () => {
      getRemoteKeysWithNotarySignature.mockResolvedValueOnce([]);
      const db = createFedDb({ serverKeys: stubNotaryKeys() });
      const env = makeEnv(db);
      const res = await req(
        'GET',
        `/_matrix/key/v2/query/remote${i}.example.com?minimum_valid_until_ts=0`,
        env
      );
      expect(res.status).toBe(404);
      expect((res.body as { errcode: string; error: string }).errcode).toBe('M_NOT_FOUND');
...
describe('race quinary federation membership/keys/directory exact after #297', () => {
  beforeEach(() => {
    federationOrigin = FED_ORIGIN;
  ...
  it('backfill no-users exact∥member-ok isolation', async () => {
    const path = `/_matrix/federation/v1/backfill/${encodeURIComponent(ROOM)}?limit=2`;
    const forbidDb = createFedDb({
      rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }],
      memberships: [],
      events: [
        makeEvent({
    ...
    const [forbid, ok] = await Promise.all([
      req('GET', path, makeEnv(forbidDb)),
      req('GET', path, makeEnv(okDb)),
    ]);
    ...
    expect(statusesOf([forbid, ok])).toEqual([200, 403]);
  });

  it('get_missing_events no-users exact∥Room not found isolation', async () => {
    const forbidDb = createFedDb({
      rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }],
      memberships: [],
    });
    const missDb = createFedDb({ rooms: [], memberships: [] });
    const [forbid, miss] = await Promise.all([
      req(
        'POST',
        `/_matrix/federation/v1/get_missing_events/${encodeURIComponent(ROOM)}`,
        makeEnv(forbidDb),
        { earliest_events: [], latest_events: ['$x'], limit: 3 }
      ),
      req(
        'POST',
        `/_matrix/federation/v1/get_missing_events/${encodeURIComponent('!no:example.com')}`,
        makeEnv(missDb),
        { earliest_events: [], latest_events: ['$x'], limit: 3 }
      ),
    ]);
    ...
    expect((miss.body as { error: string }).error).toBe('Room not found');
  });

  it('Room alias not found∥directory ok isolation', async () => {
    ...
    expect(miss.status).toBe(404);
    expect((miss.body as { error: string }).error).toBe('Room alias not found');
    expect(ok.status).toBe(200);
    expect((ok.body as { room_id: string }).room_id).toBe(ROOM);
  });

  it('User not found∥profile ok isolation', async () => {
    const db = createFedDb({
    ...
    const [miss, ok] = await Promise.all([
      req(
        'GET',
        `/_matrix/federation/v1/query/profile?user_id=${encodeURIComponent('@no:example.com')}`,
        env
      ),
      req(
        'GET',
    ...
    expect(miss.status).toBe(404);
    expect((miss.body as { error: string }).error).toBe('User not found');
    expect(ok.status).toBe(200);
    expect((ok.body as { displayname: string }).displayname).toBe('Alice');
  });

  it('Key not found∥Invalid server name isolation', async () => {
    const db = createFedDb({ serverKeys: stubNotaryKeys() });
    const env = makeEnv(db);
    ...
    expect((missing.body as { error: string }).error).toBe('Key not found');
    expect(invalid.status).toBe(400);
    expect((invalid.body as { error: string }).error).toBe('Invalid server name');
    expect(statusesOf([missing, invalid])).toEqual([400, 404]);
  });

  it('No keys found for server∥Invalid server name isolation', async () => {
    getRemoteKeysWithNotarySignature.mockResolvedValue([]);
    const db = createFedDb({ serverKeys: stubNotaryKeys() });
    const env = makeEnv(db);
    const [empty, invalid] = await Promise.all([
      req('GET', '/_matrix/key/v2/query/peer.example.com', env),
      req('GET', '/_matrix/key/v2/query/localhost', env),
    ]);
    expect(empty.status).toBe(404);
    expect((empty.body as { error: string }).error).toBe('No keys found for server');
    expect(invalid.status).toBe(400);
    expect((invalid.body as { error: string }).error).toBe('Invalid server name');
  });

  for (let i = 0; i < 10; i++) {
    it(`quinary federation exact race flood-${i}`, async () => {
      getRemoteKeysWithNotarySignature.mockResolvedValue([]);
      const forbidDb = createFedDb({
        rooms: [{ room_id: ROOM, room_version: '10', is_public: 1, created_at: 1 }],
        memberships: [],
        serverKeys: stubNotaryKeys(),
        aliases: {},
        users: [{ user_id: LOCAL_USER, display_name: 'Alice', avatar_url: null }],
      });
      const env = makeEnv(forbidDb);
      const results = await Promise.all([
        req('GET', `/_matrix/federation/v1/backfill/${encodeURIComponent(ROOM)}?limit=1`, env),
        req(
          'GET',
          `/_matrix/federation/v1/query/directory?room_alias=${encodeURIComponent('#no:example.com')}`,
          env
        ),
        req(
          'GET',
        ...
        req('GET', `/_matrix/key/v2/server/ed25519:miss${i}`, env),
        req('GET', '/_matrix/key/v2/query/127.0.0.1', env),
        req('GET', '/_matrix/key/v2/query/other.example.com', env),
      ]);
      expect((results[0]!.body as { error: string }).error).toBe(
        'Requesting server has no users in this room'
      );
      expect((results[1]!.body as { error: string }).error).toBe('Room alias not found');
      expect((results[2]!.body as { error: string }).error).toBe('User not found');
      expect((results[3]!.body as { error: string }).error).toBe('Key not found');
      expect((results[4]!.body as { error: string }).error).toBe('Invalid server name');
      expect((results[5]!.body as { error: string }).error).toBe('No keys found for server');
    });
  }
});
