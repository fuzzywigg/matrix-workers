/**
 * TOKENMAXX HEAVY tip-relaunch deepen after merged #403 — niche media soft
 * residual *eleventh-wave* concurrent-race leftovers (tip past #403 tenth / f933abf).
 *
 * Unsaturated by waves #301/#305/#318/#327/#334/#341/#357/#371/#397/#403:
 *   never 127.32.0.1 / 10.32.0.1 / 192.168.32.1 / 172.22.0.1 /
 *     ::ffff:192.168.32.1 / ::ffff:c0a8:2001 / ::ffff:169.254.32.1 /
 *     ff06 / ff0c / fe8c / feb4 / fd20 / 2001:db8:b /
 *     scheduler.kubernetes.default.svc.cluster.local / sandbox.metadata.google.internal /
 *     proxy.internal / quux.local / 0.32.0.1 under soft PA,
 *   never preview nonstandard 1111/2222/3333/6666/7778/19000,
 *   never amqp/vnc/ipfs/ipns/zoom/skype/discord/steam schemes,
 *   never case-mutated supported MIME IMAGE/Jpeg|AUDIO/Ogg|Video/Mp4|
 *     Application/JSON|IMAGE/Svg+xml|Audio/WebM.
 *
 * Gap table (why leftover after #403 tenth):
 *   residual SSRF (low-mid loopback / 10.32 / 192.168.32 / 172.22 /
 *     mapped 192.168.32 + link-local / ff06 / ff0c / fe8c / feb4 / fd20 /
 *     2001:db8:b / k8s cluster.local scheduler + sandbox metadata + .internal +
 *     .local / 0.32.0.1) ∥ cached
 *   residual preview nonstandard ports (1111/2222/3333/6666/7778/19000) ∥ cached
 *   residual schemes (amqp / vnc / ipfs / ipns / zoom / skype / discord / steam) ∥ cached
 *   case-mutated supported MIME (IMAGE/Jpeg, AUDIO/Ogg, Video/Mp4, …) ∥ ok
...
function seedRow(partial: Partial<MediaRow> & { media_id: string }): MediaRow {
  return {
    ...
    content_type: partial.content_type ?? 'image/jpeg',
...
function seedLocalDownload(mediaId: string, body = 'PNGDATA') {
  const db = createMediaDb({
    rows: [seedRow({ media_id: mediaId, content_type: 'image/jpeg', content_length: body.length })],
  });
  const media = createMediaBucket({
    [mediaId]: { body, httpMetadata: { contentType: 'image/jpeg' } },
  });
  return { db, media };
}
...
// ---------------------------------------------------------------------------
// Residual SSRF (low-mid loopback / 10.32 / mapped 192.168.32 / ff06 / fe8c / …) ∥ cached
// ---------------------------------------------------------------------------

describe('media eleventh-wave concurrent residual SSRF ∥ cached after #403', () => {
  const cases: Array<{ label: string; url: string; error: string }> = [
    {
      label: 'loopback 127.32.0.1',
      url: 'http://127.32.0.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'rfc1918 10.32.0.1',
      url: 'http://10.32.0.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'rfc1918 192.168.32.1',
      url: 'http://192.168.32.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'rfc1918 172.22',
      url: 'http://172.22.0.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv4-mapped dotted ::ffff:192.168.32.1',
      url: 'http://[::ffff:192.168.32.1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv4-mapped hex ::ffff:c0a8:2001',
      url: 'http://[::ffff:c0a8:2001]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv4-mapped dotted ::ffff:169.254.32.1',
      url: 'http://[::ffff:169.254.32.1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv6 multicast ff06',
      url: 'http://[ff06::1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv6 multicast ff0c',
      url: 'http://[ff0c::1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv6 link-local fe8c',
      url: 'http://[fe8c::1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv6 link-local feb4',
      url: 'http://[feb4::1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
    ...
    {
      label: 'docs prefix 2001:db8:b',
      url: 'http://[2001:db8:b::1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'scheduler.kubernetes.default.svc.cluster.local subdomain',
      url: 'http://scheduler.kubernetes.default.svc.cluster.local/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'sandbox.metadata.google.internal subdomain',
      url: 'http://sandbox.metadata.google.internal/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'proxy.internal hostname',
      url: 'http://proxy.internal/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'quux.local mDNS',
      url: 'http://quux.local/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'current-network 0.32.0.1',
      url: 'http://0.32.0.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
  ];
  ...
  it('multi residual SSRF softs ∥ one cached under race', async () => {
    ...
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://127.32.0.1/')}`,
        {},
        env
      ),
      request(
        `/_matrix/client/v1/media/preview_url?url=${encodeURIComponent('http://[ff06::1]/')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://scheduler.kubernetes.default.svc.cluster.local/')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://172.22.0.1/')}`,
        {},
        env
      ),
      request(
...
describe('media eleventh-wave concurrent residual ports ∥ cached after #403', () => {
  const cases: Array<{ label: string; url: string; error: string }> = [
    {
    ...
    {
      label: 'preview nonstandard 2222',
      url: 'https://example.org:2222/',
      error: 'Only standard HTTP ports (80, 443, 8080, 8443) are allowed for URL preview',
    },
    {
    ...
    {
      label: 'preview nonstandard 6666',
      url: 'https://example.org:6666/',
      error: 'Only standard HTTP ports (80, 443, 8080, 8443) are allowed for URL preview',
    },
    {
      label: 'preview nonstandard 7778',
      url: 'https://example.org:7778/',
      error: 'Only standard HTTP ports (80, 443, 8080, 8443) are allowed for URL preview',
    },
    {
      label: 'preview nonstandard 19000',
      url: 'https://example.org:19000/',
      error: 'Only standard HTTP ports (80, 443, 8080, 8443) are allowed for URL preview',
    },
  ];
  ...
  it('multi port softs ∥ one cached under race', async () => {
    ...
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('https://example.org:1111/')}`,
        {},
        env
      ),
      request(
        `/_matrix/client/v1/media/preview_url?url=${encodeURIComponent('https://example.org:2222/')}`,
        {},
        env
      ),
      request(
...
describe('media eleventh-wave concurrent residual schemes ∥ cached after #403', () => {
  const cases: Array<{ label: string; url: string }> = [
    { label: 'amqp', url: 'amqp://example.org/' },
    { label: 'vnc', url: 'vnc://example.org/' },
    { label: 'ipfs', url: 'ipfs://bafybeigdyrzt' },
    { label: 'ipns', url: 'ipns://example.org/' },
    { label: 'zoom', url: 'zoommtg://example.org/join' },
    { label: 'skype', url: 'skype:echo123?call' },
    { label: 'discord', url: 'discord://example.org/channels' },
    { label: 'steam', url: 'steam://run/440' },
  ];
  ...
  it('multi residual schemes ∥ cached under race', async () => {
    ...
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('amqp://example.org/')}`,
        {},
        env
      ),
      request(
        `/_matrix/client/v1/media/preview_url?url=${encodeURIComponent('vnc://example.org/')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('zoommtg://example.org/join')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('steam://run/440')}`,
        {},
        env
      ),
      request(
...
// ---------------------------------------------------------------------------
// Case-mutated *supported* MIME soft (leftovers after #403 tenth) ∥ ok
// ---------------------------------------------------------------------------

describe('media eleventh-wave concurrent case-mutated supported MIME soft ∥ ok after #403', () => {
  it('v3 IMAGE/Jpeg ∥ image/jpeg — case-sensitive whitelist pin', async () => {
    const db = createMediaDb();
    const media = createMediaBucket();
  ...
  it('v1 AUDIO/Ogg ∥ v3 audio/ogg under race', async () => {
    const db = createMediaDb();
    const media = createMediaBucket();
    const env = envFor({ db, media });
    const results = await Promise.all([
      request('/_matrix/client/v1/media/upload', {
        method: 'POST',
        headers: { 'Content-Type': 'AUDIO/Ogg', 'Content-Length': '2' },
        body: bytesOf('no'),
      }, env),
      request('/_matrix/media/v3/upload', {
        method: 'POST',
...
// ---------------------------------------------------------------------------
// Multi-error eleventh-wave soft isolation
// ---------------------------------------------------------------------------

describe('media eleventh-wave concurrent multi-error soft isolation after #403', () => {
  it('mapped-192.168.32-SSRF + port + scheme + case-MIME ∥ oks', async () => {
    const { db, media } = seedLocalDownload('loc11');
    const cache = seedPreviewCache();
    const env = envFor({ db, media, cache });
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://[::ffff:192.168.32.1]/')}`,
        {},
        env
      ),
      request(
      ...
      request(`/_matrix/media/v3/download/${SERVER}/loc11`, {}, env),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent(PREVIEW_OK_URL)}`,
        {},
        env
  ...
  for (let i = 0; i < 6; i++) {
    it(`eleventh-wave multi soft isolation flood-${i}`, async () => {
      const cache = seedPreviewCache();
      const db = createMediaDb();
      const media = createMediaBucket();
      const env = envFor({ db, media, cache });
      const results = await Promise.all([
        request(
          `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://[feb4::1]/')}`,
          {},
          env
        ),
        request('/_matrix/media/v3/upload', {
