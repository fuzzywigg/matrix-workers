/**
 * TOKENMAXX HEAVY tip-relaunch deepen after merged #341 — niche media soft
 * residual *seventh-wave* concurrent-race leftovers (tip past #341 sixth).
 *
 * Unsaturated by waves #301/#305/#318/#327/#334/#341:
 *   never 127.0.0.2 / 10.255.0.1 / 192.168.255.1 / ::ffff:192.168.1.1 /
 *     ::ffff:c0a8:101 / ff0e / fee0 / fea0 / feb0 / fcff / 2001:0db8 /
 *     expanded :: / api.kubernetes.default / prod.metadata.google.internal /
 *     foo.localhost.localdomain / bar.ip6-localhost / 0.0.255.1 under soft PA,
 *   never preview nonstandard 8000/5173/10000/7000/8081/4443,
 *   never wss/ssh/git/redis schemes,
 *   never case-mutated supported MIME Image/JPEG|IMAGE/JPEG|Audio/MP3|
 *     Audio/WebM|Application/JSON|VIDEO/WEBM.
 *
 * Gap table (why leftover after #341 sixth):
 *   residual SSRF (loopback .2 / 10.255 / 192.168.255 / mapped 192.168 /
 *     ff0e / fee0 / fea0 / feb0 / fcff / 2001:0db8 / expanded :: /
 *     k8s+metadata subdomains / localhost.localdomain+ip6 subdomains /
 *     0.0.255.1) ∥ cached
 *   residual preview nonstandard ports (8000/5173/10000/7000/8081/4443) ∥ cached
 *   residual schemes (wss / ssh / git / redis) ∥ cached
 *   case-mutated supported MIME (Image/JPEG, IMAGE/JPEG, Audio/MP3, …) ∥ ok
...
// ---------------------------------------------------------------------------
// Residual SSRF (loopback.2 / 10.255 / mapped 192.168 / ff0e / fee0 / …) ∥ cached
// ---------------------------------------------------------------------------
...
describe('media seventh-wave concurrent residual SSRF ∥ cached after #341', () => {
  const cases: Array<{ label: string; url: string; error: string }> = [
    ...
    {
      label: 'ipv4-mapped dotted ::ffff:192.168.1.1',
      url: 'http://[::ffff:192.168.1.1]/',
      error: 'Access to internal IP addresses is not allowed',
    },
    {
      label: 'ipv4-mapped hex ::ffff:c0a8:101',
    ...
    {
      label: 'api.kubernetes.default subdomain',
      url: 'http://api.kubernetes.default/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'prod.metadata.google.internal subdomain',
      url: 'http://prod.metadata.google.internal/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'foo.localhost.localdomain subdomain',
      url: 'http://foo.localhost.localdomain/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'bar.ip6-localhost subdomain',
      url: 'http://bar.ip6-localhost/',
      error: 'Access to internal hostnames is not allowed',
    },
    {
      label: 'current-network 0.0.255.1',
      url: 'http://0.0.255.1/',
      error: 'Access to internal IP addresses is not allowed',
    },
  ];
  ...
  it('multi residual SSRF softs ∥ one cached under race', async () => {
    ...
    const results = await Promise.all([
      ...
      request(
        `/_matrix/client/v1/media/preview_url?url=${encodeURIComponent('http://[ff0e::1]/')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://api.kubernetes.default/')}`,
        {},
        env
      ),
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://192.168.255.1/')}`,
        {},
        env
      ),
      request(
...
describe('media seventh-wave concurrent residual ports ∥ cached after #341', () => {
  const cases: Array<{ label: string; url: string; error: string }> = [
    ...
    {
      label: 'preview nonstandard 5173',
      url: 'https://example.org:5173/',
      error: 'Only standard HTTP ports (80, 443, 8080, 8443) are allowed for URL preview',
    },
    {
  ...
  it('multi port softs ∥ one cached under race', async () => {
    ...
    const results = await Promise.all([
      ...
      request(
        `/_matrix/client/v1/media/preview_url?url=${encodeURIComponent('https://example.org:5173/')}`,
        {},
        env
      ),
      request(
...
describe('media seventh-wave concurrent residual schemes ∥ cached after #341', () => {
  const cases: Array<{ label: string; url: string }> = [
    { label: 'wss', url: 'wss://example.org/' },
    { label: 'ssh', url: 'ssh://example.org/' },
    { label: 'git', url: 'git://example.org/' },
    { label: 'redis', url: 'redis://example.org/' },
  ];
  ...
  it('multi residual schemes ∥ cached under race', async () => {
    ...
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('wss://example.org/')}`,
        {},
        env
      ),
      request(
      ...
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('redis://example.org/')}`,
        {},
        env
      ),
      request(
...
describe('media seventh-wave concurrent case-mutated supported MIME soft ∥ ok after #341', () => {
  it('v3 Image/JPEG ∥ image/jpeg — case-sensitive whitelist pin', async () => {
    ...
    const results = await Promise.all([
      ...
      request('/_matrix/media/v3/upload?filename=ok.jpg', {
        method: 'POST',
  ...
  it('v1 IMAGE/JPEG ∥ v3 image/jpeg under race', async () => {
    const db = createMediaDb();
    const media = createMediaBucket();
    const env = envFor({ db, media });
    const results = await Promise.all([
...
describe('media seventh-wave concurrent multi-error soft isolation after #341', () => {
  it('mapped-SSRF + port + scheme + case-MIME ∥ oks', async () => {
    const { db, media } = seedLocalDownload('loc7');
    const cache = seedPreviewCache();
    const env = envFor({ db, media, cache });
    const results = await Promise.all([
      request(
        `/_matrix/media/v3/preview_url?url=${encodeURIComponent('http://[::ffff:192.168.1.1]/')}`,
        {},
        env
      ),
      request(
