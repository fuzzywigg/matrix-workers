/**
 * TOKENMAXX HEAVY tip-relaunch at tip ~bd0cefc after #349 turn + #350
 * filters + #353 room-cache (fresh after closed #354) — residual
 * *devices + keys* soft→*concurrent-race* tenth-wave binds unsaturated
 * by:
 *   #340 ninth-wave (SSO *callback*∥devices; SSO∥token∥not-approved;
 *        OAuth approved∥Unrecognized∥list; token∥keys∥PUT — never
 *        SSO callback ∥ token soft ∥ Unrecognized ∥ PUT; never SSO
 *        redirect ∥ OAuth approved ∥ DELETE challenge ∥ Missing devices;
 *        never SSO callback no-code ∥ keys challenge ∥ No password ∥
 *        Invalid password; never token expired ∥ delete_devices
 *        challenge ∥ GET list ∥ Device not found),
 *   #324 eighth-wave (SSO redirect∥devices; token∥PUT∥Unrecognized;
 *        approved∥DELETE challenge; list∥keys — without callback+token,
 *        without redirect+approved+Missing),
 *   #315 seventh-wave (challenge∥Unrecognized∥not-approved;
 *        keys∥delete_devices challenge without token expired∥list).
 *
 * Gap table (why leftover after ninth-wave / tip past #349/#350/#353):
 *   SSO callback ∥ token soft ∥ Unrecognized ∥ PUT ok
 *     | ninth callback∥devices; eighth token∥PUT∥Unrecognized
 *   SSO redirect ∥ OAuth approved ∥ DELETE challenge ∥ Missing devices
 *     | eighth redirect∥devices + approved∥challenge; ninth approved∥list
 *   SSO callback no-code ∥ keys challenge ∥ No password ∥ Invalid password
 *     | ninth callback without keys; ninth No password without callback
 *   token expired ∥ delete_devices challenge ∥ list ∥ Device not found
 *     | ninth token∥keys∥PUT; seventh delete_devices; eighth list∥keys
 *
...
describe('devices soft residual tenth-wave SSO callback∥token∥Unrecognized∥PUT after #340', () => {
  it('SSO callback Missing state ∥ expired HTML ∥ token missing ∥ Unrecognized ∥ PUT ok under race', async () => {
    const cache = mockKv();
    ...
    const [missingState, expired, tokenMissing, unrec, putOk] = await Promise.all([
  ...
  it('SSO callback Missing∥expired∥no-code ∥ token expired ∥ Unrecognized ∥ PUT ok', async () => {
    const cache = mockKv({
    ...
    const [missingState, expired, noCode, tokenExpired, unrec, putOk] = await Promise.all([
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO callback ∥ token soft ∥ Unrecognized ∥ PUT flood-${i}`, async () => {
...
describe('devices soft residual tenth-wave SSO∥approved∥DELETE∥Missing after #340', () => {
  it('SSO redirect Missing∥expired ∥ OAuth approved ∥ DELETE challenge ∥ Missing devices under race', async () => {
    const cache = mockKv({
    ...
    const [ssoMissing, ssoExpired, ok, challenge, missing] = await Promise.all([
  ...
  it('SSO redirect ∥ OAuth approved ∥ DELETE challenge ∥ Missing devices ∥ Device not found', async () => {
    const cache = mockKv({
      'uia_session:ok': JSON.stringify({
        user_id: USER,
    ...
    const [ssoMissing, ok, challenge, missing, notFound] = await Promise.all([
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO redirect ∥ OAuth approved ∥ DELETE challenge ∥ Missing flood-${i}`, async () => {
...
// ---------------------------------------------------------------------------
// SSO callback no-code ∥ keys UIA challenge ∥ No password ∥ Invalid password
// ---------------------------------------------------------------------------

describe('devices soft residual tenth-wave SSO callback∥keys-challenge∥NoPw after #340', () => {
  it('SSO callback no-code ∥ keys UIA challenge ∥ No password ∥ Invalid password under race', async () => {
    const cache = mockKv({
      'uia_session:race-nocode': JSON.stringify({
        user_id: USER,
        completed_stages: [],
    ...
    const [noCode, challenge, noPw, badPw] = await Promise.all([
      keysRequest(keysEnv, `${SSO_CALLBACK}?state=race-nocode`),
  ...
  it('SSO callback no-code∥Missing state ∥ keys challenge ∥ No password ∥ Invalid password ∥ badJson', async () => {
    const cache = mockKv({
      'uia_session:race-nocode': JSON.stringify({
        user_id: USER,
        completed_stages: [],
    ...
    const [noCode, missingState, challenge, noPw, badPw, badJson] = await Promise.all([
      keysRequest(keysEnv, `${SSO_CALLBACK}?state=race-nocode`),
      keysRequest(keysEnv, `${SSO_CALLBACK}?code=x`),
      keysRequest(keysEnv, DEVICE_SIGNING, jsonInit('POST', masterKeyBody())),
      keysRequest(
  ...
  for (let i = 0; i < 8; i++) {
    it(`SSO callback no-code ∥ keys challenge ∥ No password ∥ Invalid password flood-${i}`, async () => {
      const cache = mockKv({
...
// ---------------------------------------------------------------------------
// token expired ∥ delete_devices challenge ∥ GET list ∥ Device not found
// ---------------------------------------------------------------------------

describe('devices soft residual tenth-wave token∥delete_devices∥list after #340', () => {
  it('token expired ∥ delete_devices challenge ∥ GET list ok ∥ Device not found under race', async () => {
    const keysEnv = createKeysEnv({
      db: createKeysDb({
        crossSigningKeys: [existingMasterKey()],
        passwordHashes: new Map([[USER, `mockok:${PASS}`]]),
    ...
    const [tokenExpired, challenge, list, notFound] = await Promise.all([
      keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', { session: 'gone' })),
  ...
  it('token missing∥expired ∥ delete_devices challenge ∥ list ∥ Device not found ∥ Missing devices', async () => {
    const keysEnv = createKeysEnv({
      db: createKeysDb({
        crossSigningKeys: [existingMasterKey()],
        passwordHashes: new Map([[USER, `mockok:${PASS}`]]),
    ...
    const [missing, expired, challenge, list, notFound, missingDevices] = await Promise.all([
      keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', {})),
      keysRequest(keysEnv, TOKEN_SUBMIT, jsonInit('POST', { session: 'gone' })),
      devicesRequest(db, DELETE_DEVICES, jsonInit('POST', { devices: ['PHONE'] })),
      devicesRequest(db, DEVICES, { method: 'GET', headers: AUTH }),
  ...
  for (let i = 0; i < 8; i++) {
    it(`token expired ∥ delete_devices challenge ∥ list ∥ Device not found flood-${i}`, async () => {
      const keysEnv = createKeysEnv({
        db: createKeysDb({
          crossSigningKeys: [existingMasterKey()],
          passwordHashes: new Map([[USER, `mockok:${PASS}`]]),
